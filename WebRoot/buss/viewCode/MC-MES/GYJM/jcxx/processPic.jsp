...
<body>
		...
		<div class="user-drop" id="picList" style="height:calc(100% - 41px);">
			...
			<s:iterator value="dataList" id="picdto" status="index">
				...
				<div class="item">
					<div>
						<img alt=""
							src="${path}plf/page/fp/img/process/${picdto.picName};"
							width="100px" height="100px" onclick="chosenPic('<s:property value="#index.index"/>');" style="cursor:pointer" id="${picdto.picName}" >
					</div>

                    
					<input type="radio" name="picNameRadio" value="${picdto.picName}"
						id="radio<s:property value="#index.index"/>" class="radio" style="width:15px;margin-top: 3px;"
						onclick="setPicId(this)"/>
						<p onclick="chosenPic('<s:property value="#index.index"/>');" id="${picdto.picName}" style="float:right;font-size:12px;width:80px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;margin-top: 1px;" title="${picdto.picName}">${picdto.picName}</p> 
					 	<%-- <s:hidden id="picId_%{#index.index}" name="${picdto.picName}"></s:hidden>--%>
	...
	<script type="text/javascript">
        ...
        function chosenPic(obj){

         //$("#radio"+obj).attr("checked","checked");
         $("#radio"+obj).click();
        }
		...
		function commit(thisObj) {
			...
			var parentPicIdId = parentWindows.document.getElementById('serverfileName');//通过父iframe找到图片ID这个输入框
			var parentPicUrlId = parentWindows.document.getElementById('preview53bb92ed69a846788fb8f2cd0ef98609');//通过父iframe找到图片URL
			if (isClick) {
