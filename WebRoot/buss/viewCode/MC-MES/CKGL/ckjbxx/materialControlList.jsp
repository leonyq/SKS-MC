    ...
    <div class="content-in">
        <div class="dlist">
            <div class="hd">
				    		...
				    		<bu:func viewId="b2a9defd739b4cf4a88fbb9d31949eb4" />
...
//var createTables = "${path}buss/bussModel_exeFunc.ms?funcMId=bcf05dc427294db98098819856698fbc"; //
    	var createTables = "${path}buss/bussModel_exeFunc.ms?funcMId=1d868d02358a4e96b9310e456abb5032";//VM-物料管控 操作功能 显示人员信息（旧）
        ...
        success:function(data){
            ...
            if(null!=data.ajaxPage.dataList){
                ...
                for(var i=0;i<list.length;i++){
							...
							$("#tbody_465bfc3dfbec493d8bf3eab7cd53bef4 tr:last").append("<td class='datagrid-cell datagrid-cell-3' style='width:30px;text-align: center;'><input id='"+i+"' type='checkbox' name='isSelect3' class='_selectdata' value='"+list[i].ID+"' onclick=\"showItemDetail('"+list[i].ID+"','"+list[i].AUTH_VAL+"');\"value='"+list[i].LOGIN_NAME+"'  /></td>");
							...
							//$("#tbody_465bfc3dfbec493d8bf3eab7cd53bef4 tr:last").append("<td class='datagrid-cell datagrid-cell-4' style='width:100px;text-align: center;' title='"+list[i].DATA_AUTH+"'>"+list[i].NAMES+"<input type='hidden' id='DATA_AUTH_ID_"+list[i].ID+"' value='"+list[i].DATA_AUTH2+"' /></td>");
							$("#tbody_465bfc3dfbec493d8bf3eab7cd53bef4 tr:last").append("<td class='datagrid-cell datagrid-cell-4' style='width:100px;text-align: center;' title='"+list[i].DATA_AUTH2+"'>"+list[i].NAMES+"<input type='hidden' id='DATA_AUTH_ID_"+list[i].ID+"' value='"+list[i].DATA_AUTH2+"' /></td>");
							$("#tbody_465bfc3dfbec493d8bf3eab7cd53bef4 tr:last").append("<td class='datagrid-cell datagrid-cell-5' style='width:100px;text-align: center;'>"+list[i].LOGIN_NAME+"<input type='hidden' id='LOGIN_NAME"+(thCount+1)+"' name='WIC_CTRL_EMP_NAME' value='"+list[i].LOGIN_NAME+"'/><input type='hidden' class='loginName' value='"+list[i].LOGIN_NAME+"'/></td>");
...
//查询分页
		...
		function  SearchDetailFun(paginationImpl,url){
			...
			var GetEmpPages = "${path}buss/bussModel_exeFunc.ms?funcMId=1d868d02358a4e96b9310e456abb5032";//VM-物料管控 操作功能 人员分页
			$.ajax({
				    type: "POST",
			    	dataType: "json",
			    	url: GetEmpPages,
            ...
            if(null!=data.ajaxPage.dataList){
                ...
                for(var i=0;i<list.length;i++){
						...
						$("#tbody_465bfc3dfbec493d8bf3eab7cd53bef4").append("<tr style='height:30px;' onclick=\"showItemDetail('"+list[i].LOGIN_NAME+"','"+list[i].AUTH_VAL+"');\" id='tr1_"+((currentPage-1)*pageRecord+(i+1))+"'></tr>");
							...
							$("#tbody_465bfc3dfbec493d8bf3eab7cd53bef4 tr:last").append("<td class='datagrid-cell datagrid-cell-2' style='width:30px;text-align: center;'><input id='"+i+"' type='checkbox' name='isSelect3' class='_selectdata' value='"+list[i].ID+"' onclick=\"showItemDetail('"+list[i].LOGIN_NAME+"','"+list[i].AUTH_VAL+"');\" value='"+list[i].LOGIN_NAME+"'  /></td>");
...
<%@ include file="/plf/common/fp/pub_dom_fp.jsp"%>	
    	<script type="text/javascript">
    	    ...
    	    function showItemDetail(itemIds,authVal){
    	    	    $("#itemIds").val(itemIds);
    	    	    WIC_CTRL_EMP=itemIds;
    	    	    $("#authVal").val(authVal);
    	    	   $("#WIC_CTRL_EMPS").val(itemIds);
    	    	   var CheckDetail= "${path}buss/bussModel_exeFunc.ms?funcMId=e4db471e8f1f4fb08b32c571645e9ef4";//VM-物料管控 操作功能 通过人员看物料
    	    	     $.ajax({
    	    	             type: "POST",
			                dataType: "json",
			                url: CheckDetail,
    	    	      ...
    	    	      for(var i=0;i<list.length;i++){
                           ...
                           if(list[i].CI_ITEM_CODE==null)list[i].CI_ITEM_CODE="";
							...
							$("#addItemDetail tr:last").append("<td class='datagrid-cell datagrid-cell-2' style='width:150px;text-align: center;' title='"+list[i].CI_ITEM_CODE+"'><span onclick='itemJump1(\""+list[i].CI_ITEM_CODE+"\")' style='display: inline;border-bottom: 1px solid #004098;'>"+list[i].CI_ITEM_CODE+"</span></td>");
    ...
    function searchItemInfo(paginationImpl,url){
            ...
            var dataAuth = $("#DATA_AUTH").val();
            var GetSuppliesPages ="${path}buss/bussModel_exeFunc.ms?funcMId=5c0f10bdc9c5481f8d39783391024fa2";//VM-物料管控 操作功能 物料分页
            $.ajax({
                type:"post",
                dataType:"json",
