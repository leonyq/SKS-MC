...
<body>
	...
	<input type="hidden" id="printIndex" name="printIndex" value="" />
	<input type="hidden" id="lableCount" name="lableCount" value="" />
		<s:set name="_$viewId" value="'c47bab52c37b4845b8cdd584940e34dd'" />
		<s:set name="_$formId_1" value="'fd43d63021be4eadb08e73d0ac80b243'" />
		<s:set name="_$type" value="'add'" />
		<tr style="display:none;">
...
<script type="text/javascript">
	    ...
	    $(function() {
            ...
            getWmsSel();
            localLabelId=localStorage.getItem("LOCAL_LABLE_ID2");
		...
		$("#paraMap1_WRC_TEMPLATE_ID").change(function(){
    	    ...
    	    localStorage.setItem("LOCAL_LABLE_ID2",labelId);
    	    labelIdStr=labelId;
					...
					if(null != data.ajaxList) {
						...
						if(labelId!=""){
							...
							for(var i = 0; i < rccList.length; i++) {
							    ...
							    $("#itemTable tr:last").append("<td class='name' style='width:105px;display:none;'>"+rccList[i].BARCODE_ITEM+"</td>	");
							    $("#itemTable tr:last").append("<td>&nbsp;</td>	");
							}
						}
    			 			...
    			 			success: function(data){
    			 				...
    			 				if(dataStr=="ERROR"){
                                    ...
                                    return;
    			 				    ...
    			 				    //物料流水最后一位
    			 				    //////////////////////////////////////////////////////////////////////
    			 				    $("#printIndex").val(dataMap.MSS_MAX_NUM);
    			 				    $("#lableCount").val(dataMap.DATA_ITEMS_NUM)
    			 				    //console.log($("#printIndex").val());
    			 				    getSNInfoFlag=true;
    			 				    var wmsOption="";
...
　　    function changeNum(index)
		...
		function saveInfo(){
		    ...
		    var lableCount=$("#lableCount").val();
		    if(lableCount==""||lableCount==null){
		        lableCount="数量";
		    }
            ...
            //console.log("splitNum="+splitNum);
                var tempStr=changeNum((parseInt(printIndex)+i+1));
                if(tempStr=="false"){
                    _alertValMsg($("#WRC_DISK_NUM"), "<dict:lang value="超出三位流水号" />");
                    $("#resDiv").html("");
        	    	return;
                }
                tempStr="-"+tempStr
                var tempSn=$("#itemSn").val()+tempStr;
                var dataSource3={};
                for(var j=0;j<table1Trs.length;j++){
    				//var labelName=$("#itemTable tbody tr:eq(" + j + ")").find("td:eq(1)").text();
    				var labelName=$.trim($("#itemTable tbody tr:eq(" + j + ")").find("td:eq(1)").text());
    	            var labelVal=$.trim($("#itemTable tbody tr:eq(" + j + ")").find("td:eq(3) input").val());
    	            var labelFlag=$.trim($("#itemTable tbody tr:eq(" + j + ")").find("td:eq(5)").text());
    	            var labelStyle=$.trim($("#itemTable tbody tr:eq(" + j + ")").find("td:eq(3) input").attr("disabled"));
    	            //if(labelName=="数量"){
    	            if(labelName==lableCount){
    				    itemNum=parseInt(labelVal);	
                        if(itemNum<(diskNum*splitNum)){
    				...
    				if(labelFlag=='Y' && labelVal != "" && labelStyle=="disabled"){
    				    dataSource3[labelName]=tempSn;
    				}else{
    				    dataSource3[labelName]=labelVal;
    				}
	                dataSource3[labelName]=labelVal;
    	        }
    	        //var labelName2="唯一SN";
    	        //dataSource3[labelName2]=tempSn;
    	        changeDataSourceInfo(dataSource3);
    	        textAlignDetail();
                printPreview();
            }
            /////////////////////////////////////////////////////////////////////////////////////////////////////
