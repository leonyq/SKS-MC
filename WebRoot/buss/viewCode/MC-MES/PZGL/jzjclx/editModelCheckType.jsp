...
function getModel(){
            ...
            var jsondata={"checkType":checkType,"dataAuth":dataAuth,"sampleType":sampleType};
			util.showLoading("处理中...");
			$.ajax({
				type: "POST",
			    dataType: "json",
			    url: "${path}buss/bussModel_exeFunc.ms?funcMId=a0dc7811626c42b391ab265406b59f0d",//VM-机种检测类型-修改 操作功能 检验类型信息弹窗
			    data: jsondata,
				success: function(data){
						...
						if(null != data.ajaxPage.dataList){
							...
							for(var i = 0;i < rccList.length; i++){
								...
								$("#modelTbody tr:last").append("<td style='width: 127px;text-align: center;' title='"+rccList[i].QST_THRESHOLD+"'>"+rccList[i].QST_THRESHOLD+"<input type='hidden' id='QST_THRESHOLD"+rccList[i].ID+"' value='"+rccList[i].QST_THRESHOLD+"' /></td>");
							}
							//init();
							pageFun(data.ajaxPage,"formId3");
 ...
 function searchModelInfo(paginationImpl){
        var url = "${path}buss/bussModel_exeFunc.ms?funcMId=04874ae67fc6445ea54ac4c9e251cacf";//VM-机种检测类型-修改 操作功能 校验类型信息分页
...
function SearchModelInfo(paginationImpl,url){
			...
			var dataAuth = $("#deptLs_data_auth").val();
			var jsondata={"type":"1","paraMap.checkType":checkType,"paraMap.sampleType":sampleType,"paraMap.modelType":modelType,"paraMap.dataAuth":dataAuth,"page.currentPage":_GLO_FORM_PAGE_MAP[paginationImpl].currentPage,"page.pageRecord":_GLO_FORM_PAGE_MAP[paginationImpl].pageRecord};
			util.showLoading("处理中...");
			$.ajax({
				type: "POST",
			    dataType: "json",
			 ...
			 $("#modelTbody :checkbox").each(function(){
					if($(this).attr("checked")){
						...
						if (!$("#trr"+$(this).attr("id")).length) {
                            ...
                            if(QST_ALQ_VALUE==null||QST_ALQ_VALUE=="null"||QST_ALQ_VALUE==""){
                                QST_ALQ_VALUE="0";
                            }
                            if(QST_THRESHOLD==null||QST_THRESHOLD=="null"||QST_THRESHOLD==""){
                                QST_THRESHOLD="0";
                            }
							var samplSel;
							var errorSel;
                            ...
                            $("#addModelList tr:last").append("<td style='width: 120px;text-align: center;padding:0;'><input type='text' style='width: 100%;padding:0;' maxlength='9'  class=' input isSubmit'  name='threshold' value='"+QST_THRESHOLD+"' onkeyup='value=value.replace(/[^\\d.]/g,\"\")'/></td>");
	...
	//ajax回填值
	function setValue(){
	var getModelUrl="${path}buss/bussModel_exeFunc.ms?funcMId=b3dcf7b84a784cd0aff27ec432513840";//VM-机种检测类型-修改 操作功能 Ajax回填类型信息
				...
				success: function(data){
							...
							if(modList!=""){
								for(var i=0;i<modList.length;i++){
									...
									$("#addModelList tr:last").append("<td style='width: 120px;text-align: center;padding:0;'><input type='text' style='width: 100%;padding:0;'  maxlength='9'  class=' input isSubmit' name='threshold' value='"+modList[i].QMT_THRESHOLD+"' onkeyup='value=value.replace(/[^\\d.]/g,\"\")'/></td>");
	...
	function getSelVal(){
		var testStep = $("#paraMapObj_QMT_TEST_STEP").val();
		var dataAuth=$("#deptLs_data_auth").val();
		var jsondata = { "testStep": testStep, "dataAuth":dataAuth};
		$.ajax({
			    type: "POST",
			    dataType: "json",
			    url: "${path}buss/bussModel_exeFunc.ms?funcMId=e0dc5bed321f4bdeac4c96c76dfd0a75",//VM-机种检测类型-新增 操作功能 获取检验项目信息所需下拉框
			    data: jsondata,
				success: function(data){
					...
					<div class="datagrid-div2 datagrid-div6 datagrid-view"  style="overflow-x:hidden;border-bottom:0;">
							...
							<div class="datagrid-header-inner head2"  id="">
								<table  class="datagrid-htable fixedTable datagrid-div6 datagrid-htable-kz  " id="datagrid-htable123456">
									...
									<td style="width: 90px;text-align: center;"><dict:lang value="组织机构" /></td>
									<td style="width: 120px;text-align: center;"><dict:lang value="项目类型" /></td>
									<td style="width: 112px;text-align: center;"><dict:lang value="抽样方案" /></td>
									<td style="width: 127px;text-align: center;"><dict:lang value="缺陷等级" /></td>
									<td style="width: 89px;text-align: center;"><dict:lang value="检查水平" /></td>
									<td style="width: 57px;text-align: center;"><dict:lang value="水平等级" /></td>
									<td style="width: 57px;text-align: center;"><dict:lang value="AQL值" /></td>
									<td style="width: 57px;text-align: center;"><dict:lang value="全检标志" /></td>
									<td style="width: 127px;text-align: center;"><dict:lang value="全检阈值" /></td>					
									</tr>
									</tbody>		
								</table>
							</div>
