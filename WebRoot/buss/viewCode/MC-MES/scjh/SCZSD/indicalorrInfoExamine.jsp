...
<body>
            ...
            <div class="hd">
                ...
                <div style="margin-left:40px;">
	               <s:if test="${isDataAuth == '1'}">
			    	...
			    	<div class="optn">
			    	        <button type="button" onclick="checkInfo();"><i class="ico ico-save"></i><dict:lang value="审核" /></button>
	...
	<input type="hidden" id ="proCodeStr" name="proCodeStr" value=""/>
	<input type="hidden" id ="indicatorSn" name="indicatorSn" value=""/>
		<s:set name="_$viewId" value="'020a86444e4a4465a0f0e0de0f387920'" />
...
<script type="text/javascript">
				...
				success: function(data){
					...
					if(null != data.ajaxList){
						...
						for(var i=0;i<rccList.length;i++){
        					...
        					$("#datagrid-btable-ajax831b2dbf0dfb494695b7ce66538a1234 tr:last").append("<td class='datagrid-cell datagrid-cell-3' style='width:120px;text-align: left;' title='"+rccList[i].CO_PROJECT_NAME+"'>" + rccList[i].CO_PROJECT_NAME + "<input type='hidden' id='CO_PROJECT_CODE"+(trrCount+1)+"' name='PROJECT_CODE' value='" + rccList[i].CO_PROJECT_CODE + "'/></td>");
    		                ...
    		                //$("#itemInfo tr:last").append("<td class='datagrid-cell datagrid-cell-12' style='width:30px;text-align: center;display:none;'><input type='hidden' name='' id='CO_PROJECT_CODE"+(trrCount+1)+"' value='"+rccList[i].CO_PROJECT_CODE+"'/></td>");
    		                trrCount++;
						}	
		...
		function checkInfo(){
		    ...
		    var indicatorSn=$("input[name='paraMapObj.CO_INDICATORR_SN']").val();
		    $("#indicatorSn").val(indicatorSn);
			...
			$("#addItemInfo :checkbox").each(function() {
				if($(this).attr("checked")) {
					...
					var proCodeId="#CO_PROJECT_CODE"+trid;	
                    var confirmId="#CONFIRM_ID"+trid;    
                    var checkId="#CHECK_ID"+trid;
                    var coState="#CO_STATE"+trid;
                    if($.trim($(checkId).val())!=$.trim($("#curUser").val())){
                        tempFlag=false;
                    }
                    if($.trim($(coState).val())=="1"){
                        tempFlag2=false;
                    }
					...
					proCodeStr=proCodeStr+$(proCodeId).val()+",";
					console.log($(proCodeId).val());
				}
			});
			
			if(tempFlag==false){
			    utilsFp.confirmIcon(3,"","","", "<dict:lang value="审核人与当前登录用户不一致" />",0,"260","");
               	return;
			}
			if(tempFlag2==false){
			    utilsFp.confirmIcon(3,"","","", "<dict:lang value="项目未确认" />",0,"260","");
               	return;
			}
			if(tempFlag3==false){
            ...
            document.forms.editForm.action = "${path}buss/bussModel_exeFunc.ms?funcMId=284e93b4584848bf8b2833b52c83edbe";
