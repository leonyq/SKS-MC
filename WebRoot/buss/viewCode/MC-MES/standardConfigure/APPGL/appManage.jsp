...
<body>
    <div class="content-in">
		...
		<bu:tr cssClass="datagrid-header-row datagrid-title-ajax" id="datagrid-title-ajax">
		    ...
		    <td class="" style="width:30px; text-align:center"><span><input type="checkbox" id="SelectAll" onclick="selectAll();" style=" width:30px; cursor: pointer;"title="全选"/></span></td> 
			...
			<td class="datagrid-cell" style="width:191.25px;"><bu:uitn colNo="TAS_MENU_ID" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td>
			...
			<td class="datagrid-cell"  style="width:191.25px;"><bu:uitn colNo="TAS_MOUDLE_ID" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td>
			<td class="datagrid-cell"  style="width:191.25px;"><bu:uitn colNo="TAS_SHOW_TYPE" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td>
			<td class="datagrid-cell"  style="width:191.25px;"><bu:uitn colNo="TAS_FUN_NAME" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td>
			<td class="datagrid-cell"  style="width:191.25px;"><dict:lang value="数据源" /></td>
			<td class="datagrid-cell"  style="width:191.25px;"><dict:lang value="图片" /></td>
		<!--	<td class="datagrid-cell"  style="width:191.25px;"><bu:uitn colNo="TAS_IS_DEFAULT" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td> -->
			<td class="datagrid-cell"  style="width:191.25px;"><bu:uitn colNo="TAS_SEQ" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td>
		<!--	<td class="datagrid-cell"  style="width:191.25px;"><bu:uitn colNo="TAS_STATUS" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td> -->
			
		<!--	<td class="datagrid-cell"  style="width:30px;"><bu:uitn colNo="TAS_DISPLAY_MODE" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td> -->
			<td class="datagrid-cell"  style="width:191.25px;"><bu:uitn colNo="TAS_FUNCTION_LINK" formId="cde7667fb3bc47d1b64355437e9de4ef" /></td>
    ...
    //var intid2 = sid.replace("smenu","intmenu");
    	...
    	$.ajax({
				...
				success: function(data){
				    ...
				    $("#"+sid).empty();
				    $("#"+sid).append("<select id='"+ssid+"' name='"+ssid+"' style='width:191.25px;' onchange='chgAselect1(this)'></select>");
    ...
    $("#"+mid).removeAttr('onclick');
    	$.ajax({
				...
				success: function(data){
				    var list = data.ajaxList;
				    $("#"+mid).empty();
				    $("#"+mid).append("<select id='"+ssid+"' style='width:191.25px;' onchange='chgAselect2(this)'></select>");
				    $("#"+ssid).append("<option value=''>--请选择--</option>");
				    for(var i=0;i<list.length;i++){
    ...
    $("#"+sid).removeAttr('onclick');
    	$.ajax({
				...
				success: function(data){
				    ...
				    $("#"+sid).empty();
				    $("#"+sid).append("<select id='"+ssid+"' style='width:191.25px;' onchange='chgAselect2(this)'></select>");
				    $("#"+ssid).append("<option value=''>--请选择--</option>");
				    for(var i=0;i<list.length;i++){
				    $("#"+ssid).append("<option value="+list[i].ID+">"+list[i].TAM_MOUDLE_NAME+"</option>");    
				    }
    ...
    $("#"+sid).removeAttr('onclick');
    	$.ajax({
				...
				success: function(data){
				    var list = data.ajaxList;
				    $("#"+sid).empty();
				    $("#"+sid).append("<select id='"+ssid+"' style='width:191.25px;' onchange='chgAselect3(this)'></select>");
				    $("#"+ssid).append("<option value=''>--请选择--</option>");
				    for(var i=0;i<list.length;i++){
				    $("#"+ssid).append("<option value="+list[i].CODE+">"+list[i].VALUE+"</option>");    
				    }
						    ...
						    //	var isStatusSel = "<select id='isStatus"+(i+1)+"' name='isStatus' class=' input isSubmit 'style=' cursor: pointer;width:100%;height:100%;border:0;background-color:transparent;' onchange='setModify(this);'>"+ isCheckOption +"</select>";
						     ...
						     //	if(rccList[i].TAS_UNIT==null) rccList[i].TAS_UNIT=""; 
							   ...
							   // arrHtml.push("<td class='datagrid-cell ' style='width:200px;' title='"+rccList[i].DATA_AUTH+"'>"+rccList[i].DATA_AUTH+"</td>");
							    ...
							    arrHtml.push("<td  class='datagrid-cell ' style='width:191.25px;text-align:center;padding:0;'>");
							    arrHtml.push("<span id='menu_"+rccList[i].ID+"' style='width:191.25px;' onclick='setAselect1(this)'>"+rccList[i].FUNCNAME+"</span><input type='hidden'  name='menuId' id='intmenu_"+rccList[i].ID+"' value='"+rccList[i].TAS_MENU_ID+"' >");
							    arrHtml.push("</td>");	
							    arrHtml.push("<td  class='datagrid-cell ' style='display:none; text-align:center;padding:0;'>");
							    arrHtml.push("</td>");
							    arrHtml.push("<td class='datagrid-cell ' style='width:191.25px; text-align:center;padding:0;'>");
							    arrHtml.push("<span  id='moudle_"+rccList[i].ID+"' style='width:191.25px;' onchange='setModify(this);' onclick='setAselect2(this)'>"+rccList[i].TAM_MOUDLE_NAME+"</span><input type='hidden' name='moudleId' id='intmoudle_"+rccList[i].ID+"' value='"+rccList[i].TAS_MOUDLE_ID+"' >");//
							    arrHtml.push("</td>");
							    arrHtml.push("<td class='datagrid-cell ' style='width:191.25px; text-align:center;padding:0;'>");
							    arrHtml.push("<span  id='type_"+rccList[i].ID+"' style='width:191.25px;' onchange='setModify(this);' onclick='setAselect3(this)'>"+rccList[i].TYPENAME+"</span><input type='hidden' name='showType' id='inttype_"+rccList[i].ID+"' value='"+rccList[i].TAS_SHOW_TYPE+"' >");
							    arrHtml.push("</td>");
							    arrHtml.push("<td class='datagrid-cell ' style='width:191.25px; text-align:center;'>");
							    ...
							    arrHtml.push("<td class='_data' style='width:191.25px; text-align:center;'>");
							    ...
							    arrHtml.push("<textarea name='commSql' cols='' rows='' wrap='off' id='COMM_SQL"+(i+1)+"' class='isSubmit' style='width:191.25px; display:none;word-wrap: normal;'>"+jsObj.commql+"</textarea>");
							    arrHtml.push("<textarea name='sqlserverSql' cols='' rows='' wrap='off' id='SQLSERVER_SQL"+(i+1)+"' class='isSubmit' style='width:191.25px; display:none;word-wrap: normal;'>"+jsObj.sqlserver+"</textarea>");
							    arrHtml.push("<textarea name='oracleSql' cols='' rows='' wrap='off' id='ORACLE_SQL"+(i+1)+"' class='isSubmit' style='width:191.25px; display:none;word-wrap: normal;'>"+jsObj.oracle+"</textarea>");
							    arrHtml.push("<textarea name='mysqlSql' cols='' rows='' wrap='off' id='MYSQL_SQL"+(i+1)+"' class='isSubmit' style='width:191.25px; display:none;word-wrap: normal;'>"+jsObj.mysql+"</textarea>");
...
function addMyRow(){
    ...
    arrHtml.push("<td class='' style='width:30px;text-align:center;' ><span class='_noedit'>");
    arrHtml.push("<input type='checkbox' name='selectName' value='' checked='true' class='isSubmit _selectdata' style='width:30px; cursor: pointer;margin:0'>");
    arrHtml.push("</td>");
    arrHtml.push("<td class='datagrid-cell ' style='width:191.25px; text-align:center;padding:0;'>");
 ...
 // $('.scroll').mCustomScrollbar("destroy");
   $('.scroll').mCustomScrollbar("update");
    $('.scroll').mCustomScrollbar("scrollTo","last");
