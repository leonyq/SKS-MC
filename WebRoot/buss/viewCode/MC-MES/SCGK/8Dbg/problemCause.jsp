...
<div class="mod-hd">		
  <h3 class="tit"><dict:lang value="基本信息" /></h3>	
	...
	<input type="hidden" name="paraMap1@__DATA_COUNT__" value="1" />
		<s:set name="_$viewId" value="'bdba083f0b094f938cb39e55ddc9c1cf'" />
	...
	<div class="mod-hd">		
        <h3 class="tit"><dict:lang value="原因分析" /></h3>	
        	<div class="extend-btn" style="float: right;margin-top:14px;margin-bottom:5px;">
			...
			<div class="mod-bd" style="margin-top:5px;">
				<div class="panel datagrid datagrid-div1">
					<div class="datagrid-div2 datagrid-div6 datagrid-view"   >
							...
							<div class="datagrid-header-inner head1" id="" >
								<table  class="datagrid-htable fixedTable  " id="causeAnalysis1">
									...
									<tr class="thcolor">
										...
										<td class="datagrid-cell" align="center" style="width:30px;text-align: center"><input type="checkbox" id="allSelect" name="allSelect" onchange="getAllCause();"/></td >
						...
						<div class="datagrid-body datagrid-div3 scorl1"   style="height:151px;max-width:100%">
				...
				success: function(data){
						...
						if(null != data.ajaxList){
						    ...
						    for(var i=0;i<rcList.length;i++){
                            	...
                            	var causeTypeSel = "<select id='causeTypeSel_"+(i+1)+"' name='causeTypeSelect' class='_VAL_NULL input isSubmit _VAL_OjbsData dept_select' style='display: none;'><option value=''>--请选择--</option>"+ causeTypeOption +"</select>";
                            	
                            	$("#causeAnalysis  tbody:last").append("<tr id='tre"+(i+1)+"'></tr>");
                            	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-1' style='width:30px;text-align:center;'>"+(i+1)+"</td>");
                            	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-2' style='width:30px;text-align:center;'><input type='checkbox'/></td>");
                            	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-3' style='width:230px;padding:0;'>"+causeTypeSel+"</td>");
                            	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-4' style='padding:0;'><input type='text' style='width:100%;' maxlength='500' class='_VAL_NULL input isSubmit _VAL_DATABASE' name='CPC_CAUSE_ANALYSIS' value='"+rcList[i].CPC_CAUSE_ANALYSIS+"'/></td>");
                            	setChosen();//设置下拉框样式
...
var ttCount=0;
function addMyRow(){
	var len = $("#addCauseAnalysis tr").length;
	var causeTypeOpt = $("#causeType").html();
	var causeTypeOption = causeTypeOpt.substring(causeTypeOpt.indexOf('<option'));
	var causeTypeSel = "<select id='causeTypeSel_"+(ttCount+len+1)+"' name='causeTypeSelect' class='_VAL_NULL input isSubmit _VAL_OjbsData dept_select' style='display: none;'><option value=''>--请选择--</option>"+ causeTypeOption +"</select>";
	
	$("#causeAnalysis  tbody:last").append("<tr id='tre"+(ttCount+1+len)+"'></tr>");
	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-1' style='width:30px;text-align:center;'>"+(len+1)+"</td>");
	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-2' style='width:30px;text-align:center;'><input type='checkbox'/></td>");
	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-3' style='width:230px;padding:0;'>"+causeTypeSel+"</td>");
	$("#causeAnalysis  tr:last").append("<td class='datagrid-cell datagrid-cell-4' style='padding:0;'><input type='text' style='width:100%;' maxlength='500' class='_VAL_NULL input isSubmit _VAL_DATABASE' name='CPC_CAUSE_ANALYSIS'/></td>");
	setChosen();//设置下拉框样式
	$("#causeTypeSel_"+(ttCount+1)+"_chosen").attr("style","width:230px;");
	ttCount++;
	clickTr('causeAnalysis');
}
function delMyRow(){
	$("#causeAnalysis :checkbox").each(function(){
	...
	reSort("causeAnalysis");
	var tableTrJs = $("#causeAnalysis tbody tr");
...
function reSort(objid){
    ...
    //全选
     	function getAllCause(){
		if($("#allSelect").attr("checked")=="checked"){
			$("#addCauseAnalysis :checkbox").attr("checked",true);
		}else{
			$("#addCauseAnalysis :checkbox").each(function(){
...
function editCal(){
  if(!val.valNullData()){
        	...
        	var len = $("#addCauseAnalysis tr").length;
        	if(len<=0){
        	    utilsFp.confirmIcon(3,"","","", "<dict:lang value="请添加原因分析" />",0,"260","");
        	    return ;
        	}
        	$("#docNum").val($("#paraMapObj_CPR_SN").val());
  document.forms.editForm.action = "${path}buss/bussModel_exeFunc.ms?funcMId=f573e9f2dc4649b295256fb82346d615";
...
$('#addCauseAnalysis').on('focus','input',function(){
