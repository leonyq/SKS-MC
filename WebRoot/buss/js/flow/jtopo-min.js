...
(JTopo), function (a) {
	...
	function c(c) {
		...
		function l(a) {
			var b = d(a);
			b.wheelDelta = -b.wheelDelta; //2019/5/10
            b.wheelDeltaY = -b.wheelDeltaY; //2019/5/10
			...
			if (null != b && "" != b) {
				...
				if(bs.length>1){
					for(var j= 0;j<bs.length;j++){
						var tmp=bs[j].split('&');
						var _x=tmp[1]==null||tmp[1]==""?e.x:tmp[1];
						var _y=tmp[2]==null||tmp[2]==""?e.y/bs.length*j:tmp[2];
						a.fillText(tmp[0],_x,_y);
					}
...
(JTopo), function (a) {
	...
	function f(b, c, g) {
		...
		this.paintText = function (a, b) {
			...
			if (4 == b.length && (c = b[1], d = b[2]), this.text && this.text.length > 0) {
				...
				if (a.fillStyle = "rgba(" + this.fontColor + ", " + this.alpha + ")", this.nodeA === this.nodeZ) {
					...
					var bs=this.text.split('@');
					if(bs.length>1){
						for(var j= 0;j<bs.length;j++){
							var tmp=bs[j].split('&');
							var _x=tmp[1]==null||tmp[1]==""?e:tmp[1];
							var _y=tmp[2]==null||tmp[2]==""?f/bs.length*j:tmp[2];
							a.fillText(tmp[0],_x,_y);
						}
		...
		this.isInBound = function (b, c) {
			if ((this.nodeA!=undefined)&&(this.nodeA === this.nodeZ)) {
