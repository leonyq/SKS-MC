...
<body scroll="auto" style="background-color:#fff;">
							...
							<table class="func_table">
								<tr>
									<td width="100%" align="right">
										...
										<button type="button"  class="botton_img_add" onclick="searchOpen();" style="display: none;">
                      ...
                      <div class="search-box">
                    	<form id="searchForm" name="searchForm" action="" method="post">
							...
							<%-- <table class="search_table">
								...
								<tr> 
									<td><div class="group"  style="width:300px;"><div class="name" title="等于"><dict:lang value="部门名称" /></div><div class="dec"  style="width:200px;"><input class="input inputsd" id="name" name="paraMap2.name" style="width:200px;"></div></div></td>
								</tr>
								</tbody>
							</table> --%>
						</form>
								...
								<table  id="datagrid-htable" class="datagrid-htable fixedTable  " style="height: 25px; ">
									<tbody id="title-tbody">
										<tr>
											...
											<td style="text-align: center;"><dict:lang value="部门名称" /></td>
											<td style="width: 120px;text-align: center;"><dict:lang value="上级部门" /></td>
											<td style="width: 120px;text-align: center;"><dict:lang value="默认角色" /></td>
											<td style="width: 120px;text-align: center;"><dict:lang value="默认工种" /></td>
										</tr>
									</tbody>
								  </table>
								</div>
							</div>
							<div class="datagrid-body datagrid-div3" id="datagrid-body" onscroll="tableScroll()" style="height:366px;overflow-x: hidden;">
								<table class=" dlist-table table-line" id="datagrid-btable">
    ...
    <script type="text/javascript">
    	...
    	function backFill(popId,shadow,thisId){
    		...
    		parentWindow.backFillDeptVal(trObj,thisId);
    		parent.closePopWinFp(popId,shadow);
    	}
       	
       	// 清空值
    	function clearSel(pop,shadow,thisId){
    		parentWindow.clearDeptVal(thisId);
    	...
    	function getDataTable(paginationImpl){
    	 ...
    	 util.showLoading("处理中...");
   				...
   				success: function(data){
   					util.closeLoading();
						...
						if(null != data.ajaxPage.dataList){
							...
							if (rccList[i].PID==-1) {
								$("#table-tbody").append("<tr id='"+rccList[i].KEY+"' ></tr>");
							}else{
								if(i % 2== 1){
					
									$("#table-tbody").append("<tr id='"+rccList[i].KEY+"' pId='"+rccList[i].PID+"' class='tr-hover'></tr>");
								}else{
									$("#table-tbody").append("<tr id='"+rccList[i].KEY+"' pId='"+rccList[i].PID+"' ></tr>");
								}
							}
								
								$("#table-tbody tr:last").append("<td style='width: 30px;text-align: center;'><input type='radio' name='selectValue' id='cb_"+rccList[i].KEY+"' name='"+rccList[i].VAL+"' value='"+rccList[i].KEY+"'/></td>");
								if(rccList[i].hasChild==1){
									$("#table-tbody tr:last").append("<td style='text-align: left;'><span controller='true'>"+rccList[i].VAL+"</td>");
								}else{
									$("#table-tbody tr:last").append("<td style='text-align: left;'>"+rccList[i].VAL+"</td>");
								}
								if (rccList[i].PNAME==null) {
									rccList[i].PNAME = "";
								}
								if (rccList[i].ROLENAME==null) {
									rccList[i].ROLENAME = "";
								}
								if (rccList[i].JOBNAME==null) {
									rccList[i].JOBNAME = "";
								}
								$("#table-tbody tr:last").append("<td style='width: 120px;text-align: left;'>"+rccList[i].PNAME+"</td>");
								$("#table-tbody tr:last").append("<td style='width: 120px;text-align: left;'>"+rccList[i].ROLENAME+"</td>");
								$("#table-tbody tr:last").append("<td style='width: 120px;text-align: left;'>"+rccList[i].JOBNAME+"</td>");
								$("#table-tbody tr:last").append("<td style='display: none;'>{\"KEY\":\""+rccList[i].KEY+"\",\"VAL\":\""
								+rccList[i].VAL+"\",\"PID\":\""+rccList[i].PID+"\",\"PNAME\":\""+rccList[i].PNAME+"\",\"ROLENAME\":\""
								+rccList[i].ROLENAME+"\",\"JOBNAME\":\""+rccList[i].JOBNAME+"\"}</td>");
								}
            var option = {
                theme:'vsStyle',
            ...
            $("#datagrid-btable").treeTable(option);
								}
   					...
   					createTableModel(arrDataList.tableData,arrDataList.setHiddenCol);
    	...
    	function createTableTitleModel(titleData,hiddenCol){
    		if(!$.isEmptyObject(titleData)){
    			var arr = [];
    			arr.push("<tr>");
				arr.push("<td width=\"40px;\"></td>");
    			arr.push("<td width=\"40px;\" align=\"center\"></td>");
    	...
    	// 创建表格内容
    	function createTableModel(tableData,hiddenCol){
    		var count = 1;
    		var arr = [];
    		...
    		if(!$.isEmptyObject(tableData)){
    			$.each(tableData,function(i,v){
    				...
    				arr.push("<td class=\"datagrid-cell\" style=\"width:40px;text-align: center;\">"+count+"</td>");
    				arr.push("<td class=\"datagrid-cell\" style=\"width:40px;text-align: center;\">");
    				if(v.KEY==selVal){
    				...
    				for(var key in v){
    					if(hiddenCol.length>0){
    						...
    						if(flag==true){
    							arr.push("<td colName="+key+" class=\"datagrid-cell\" align=\"center\">");
    	            			arr.push(v[key]);
    	            			arr.push("</td>");
    						}
    					}else{
