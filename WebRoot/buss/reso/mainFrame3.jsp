...
<body>
		...
		<div class="topbar">
			<div class="topbar-in">
				...
				<div class="user">
					...
					<div class="info" id="infoDiv" onclick="toggleUserDrop();">
						<img
							...
							id="photoId" width="25" height="25" alt="" /> <span class="name">
							<s:property
				...
				<div class="state">
					...
					<div style="width:15px;height:12px;padding:3px 0;background-position:0 3px;background:url(${path}buss/images/setting.png) no-repeat center;cursor:pointer;position:relative;display:block;float:left;margin-right:14px;" onclick="showCardListPage('9c7ba6626bec46ab8c994ffcb461a980','配置中心');" title="<%=CommMethodMc.getText("配置中心") %>"><!-- cardListPage -->
						<!-- 换肤 -->
						<!--
						<div class="s1-drop">
							<span class="green"
								onclick="switchTheSkin('/plf/page/fp/themes/colorType/green/theme.css')"></span>
							<span class="red"
								onclick="switchTheSkin('/plf/page/fp/themes/colorType/red/theme.css')"></span>
							<span class="deep"
								onclick="switchTheSkin('/plf/page/fp/themes/colorType/deep/theme.css')"></span>
						</div>
						-->
					</div>
                    ...
                    <a href="javascript:void(0);" title="<dict:lang value="我的消息" />"><span class="count">10</span><dict:lang value="我的消息" /></a>--%>
							<a href="javascript:void(0);" onclick="showMsgData();"
								...
								onclick="showUserSetting();" title="<dict:lang value="用户设置" />">
								<i class="ico" style="	width:15px;	height:15px;margin-bottom:2px;background: url(${path}buss/images/user_setting.png) center no-repeat;"></i> <dict:lang value="用户设置" /> </a> <a
	...
	<script type="text/javascript">
	    ...
	    function addUserTab(url,name,id){
			...
			//卡片菜单
			function showCardListPage(id,name){
				//buss/bussModel.ms?exeid=883135130ace48bc9db3a1e91f57913b
				//addTab("${path}sys/ConfigureCardAction_listUserFuncTree.ms?id="+id,name,id,"0");
				addTab("${path}buss/bussModel.ms?exeid=ed960edbf22649fe9d0e0fcbd7955703&id="+id,name,id,"0");
			}
			...
			function getFpMenu(){
				...
				$.ajax({
					...
					success: function(data){
							...
							if(null != data.ajaxList){
								...
								for(var i = 0;i < aList.length; i++){
									...
									if(aList[i].level == "1"){
										...
										menuHtml+="<li class=\"item\" style=\"cursor:pointer;\" id=\"m_"+aList[i].id+"\"  onmouseenter=\"showSub('"+aList[i].id+"','sub_',this);\" onmouseleave=\"hideSub('"+aList[i].id+"','sub_',this)\" onClick=\"showCardListPage('"+aList[i].id+"','"+aList[i].name+"');\">"
							...
							$("#m_9c7ba6626bec46ab8c994ffcb461a980").attr("style","display:none;");
						},
					error: function(msg){
							util.closeLoading();
...
/* 			        $('.js-nav').slickAdd(tabTitle);//添加tab
			...
			function switchTag(tag, content,url) {
					...
					for (i = 1; i < (tab_len+1); i++) {<%-- 下标从1开始 --%>
						...
						document.getElementById("content").className = content;
							...
							//调用content引用的页面中的js方法，实现滚动条回到切换页面之前的位置；
							if(document.getElementById(iFrameNum).contentWindow.backScroll){
								document.getElementById(iFrameNum).contentWindow.backScroll();
							}
	        ...
	        //换肤
	        /*
	       	function switchTheSkin(url){
	       		event.stopPropagation();
	       		if(util.isChrome()){
					$(window).unbind('beforeunload');
				}
	       		$.post("${path}buss/bussModel_switchTheSkin.ms",{"themePath":url},function(data){
	       			//console.log(data);
