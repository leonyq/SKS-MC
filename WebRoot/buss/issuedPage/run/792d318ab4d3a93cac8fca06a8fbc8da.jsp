...
//	var endTime = $('T2#WPD_DELIVERY_DATE_END').val();
	...
	*/
	 var getItemDetailUrl="buss/bussModel_exeFunc.ms?funcMId=97f0a52fa234453f8e4c430d50081b74";
	util.showLoading("处理中...");
	$.ajax({
			    type: "POST",
			    dataType: "json",
        ...
        $.each(uiTxt,function(i,v){
          ...
          //  arrHtml.push("</td>");
           ...
           arrHtml.push("<td colno='DATA_AUTH' class='datagrid-cell TD_DATA_AUTH"+ms.tdClass(formId, 'DATA_AUTH')+"' style='width:200px;text-align:center;"+ms.tdStyle(formId, 'DATA_AUTH')+"' "+ms.tdEvent(formId, 'DATA_AUTH')+">")
            ...
            arrHtml.push("<span colno='T7#WDO_DISPATCH_NAME' class='"+ms.uiClass(formId, 'T7#WDO_DISPATCH_NAME')+"' onclick='itemJump2(\""+v['T6#WDT_TYPE_NAME']+"\",\""+v['T7#WDO_DISPATCH_NAME']+"\")' style='display: inline;border-bottom: 1px solid #004098;' "+ms.uiEvent(formId, 'T7#WDO_DISPATCH_NAME')+" title='"+ms.titleAttr(formId,'T7#WDO_DISPATCH_NAME','IS_TITLE_ATTR',v)+"' >"+v['T7#WDO_DISPATCH_NAME']+"</span>")
            arrHtml.push("</td>");
...
function searchReceive(){
    var url = "${path}buss/bussModel_exeFunc.ms?funcMId=8757d5235ac54762b3f57ddfbcedbd3f";
    var dataAuth = $("#DATA_AUTH").val();
	var itemCode = $.trim($("#WRI_ITEM_CODE_SHOW").val());
	var docNum = $.trim($("#WRI_DOC_NUM").val());
	//var supCode = $(document.getElementById('T2#WRD_SUP_CODE')).val();
                ...
                var list = eval(data.ajaxPage.dataList);
                    for(var i=0; i<list.length;i++){
                       ...
                       // if(i!=0 && list[i].WRD_DOC_NUM!=list[i-1].WRD_DOC_NUM ){
						...
						$("#addReceive tr:last").append("<td class='datagrid-cell datagrid-cell-3' style='text-align: left;width:140px;' title='"+list[i].WRD_DOC_NUM+"'>"+list[i].WRD_DOC_NUM+"</td>");
						$("#addReceive tr:last").append("<td class='datagrid-cell datagrid-cell-4' style='text-align: center;width:100px;' title='"+list[i].WDT_TYPE_NAME+"'><span onclick='itemJump1(\""+list[i].WDT_TYPE_NAME+"\")' style='display: inline;border-bottom: 1px solid #004098;'>"+list[i].WDT_TYPE_NAME+"</span></td>");
						$("#addReceive tr:last").append("<td class='datagrid-cell datagrid-cell-5' style='text-align: center;width:110px;' >"+tempType+"</td>");
...
function searchItemInfo(paginationImpl){
    var url = "${path}buss/bussModel_exeFunc.ms?funcMId=88beb5544f904338978860e84a53dbdd";
	SearchItemInfo(paginationImpl,url);
	SearchItemInfo(paginationImpl,url);
}
...
function itemJump1(obj){
      var title = "<dict:lang value="单据配置" />";
      var formId = $("#formId1").val();
      var url = "${path}buss/bussModel.ms?&formId="+formId+"&exeid=5ae9940df103418ea8badc0d07d006f6&FUNC_CODE=F6529&frameId="+_currentFrame+"&jumpId="+obj;
...
function itemJump2(obj1,obj2){
    var title = "<dict:lang value="仓库单据信息" />";
  var formId = $("#formId1").val();
  var url = "${path}buss/bussModel.ms?&formId="+formId+"&exeid=f4c3cfb840234a24b5ac7135d783b7e3&FUNC_CODE=F7011&frameId="+_currentFrame+"&jumpId="+obj1+"&jumpId2="+obj2;
    judgIsOpen(title,url);
} 
function itemJump3(obj){
    var title = "<dict:lang value="物料追溯" />";
  var formId = $("#formId1").val();
  var url = "${path}buss/bussModel.ms?&formId="+formId+"&exeid=0b0276d9a6e3422592e30a48ad93b418&FUNC_CODE=F10790&frameId="+_currentFrame+"&jumpId="+obj;
    judgIsOpen(title,url);
} 

function itemJump4(obj){
    var title = "<dict:lang value="物料信息" />";
  var formId = $("#formId1").val();
  var url = "${path}buss/bussModel.ms?&formId="+formId+"&exeid=15e211df6f9d435088ad89c31837ef5f&FUNC_CODE=F3428&frameId="+_currentFrame+"&jumpId="+obj;
    judgIsOpen(title,url);
} 
function itemJump5(obj){
    var title = "<dict:lang value="采购管理" />";
  var formId = $("#formId1").val();
  var url = "${path}buss/bussModel.ms?&formId="+formId+"&exeid=3465c822bb554b43af180b6def3cd860&FUNC_CODE=F6646&frameId="+_currentFrame+"&jumpId="+obj;
    judgIsOpen(title,url);
} 
</script>
<script>(function($){
