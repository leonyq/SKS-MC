...
<script>
	//加载父ifream中的js\css文件
		...
		//点击行寄选中该行
	/*$("#loadPoint tr").on("click",function(e){
		...
		if ($("#"+trId+" :checkbox").attr("checked")) {
			...
			$("#loadPoint input[type='checkbox']").each(function(){
	  ...
	  $("#loadPoint tr input[type=checkbox]").click(function(e) {
		  e.stopPropagation();
					...
					for(var i=1;i<=loadPointCount;i++){
						...
						if (count2>0) {
							for ( var k = 0; k < count2; k++) {
								if ($("#CDL_TABLE1").val().trim() == tableWork2.rows[k].cells[2].innerHTML.trim() && ($("#CDL_LOADPOINT_PREFIX").val().trim()+(parseInt($("#CDL_STAR").val())+i-1)) == tableWork2.rows[k].cells[3].innerHTML.trim()) {
								_alertValMsg(CDL_TABLE1,"<dict:lang value="相同的Table里料站不能相同" />");
								return false;
								}
							}
...
<body onchange="changeWay();" style="background-color: #fff;" onload="clickTr('loadPoint');">
	<DIV class="multilist comwrap content-in">
       ...
       <div class="hd">
				...
				<div class="js-tab">
					...
					<form id="editCalForm" name="editCalForm" action="${path}buss/bussModel.ms" method="post" <bu:formet exeid="%{exeid}" /> target="submitFrame" style="margin-top: 20px;">
						...
						<DIV class="content1" id="content">
							<DIV id="content1">
									...
									<table class="basic-table">
										...
										<s:set name="_$viewId"
											value="'4edf9182b3d149a8a43eaffcfbf7e6c4'" />
		...
		<tr>
			...
			<bu:td cssClass="dec" colNo="CD_DEVICE_SPEC" formId="99338dfb95df4eec8d5714a4e773b127"  colspan="3"><bu:ui colNo="CD_DEVICE_SPEC" formId="99338dfb95df4eec8d5714a4e773b127" dataId="${dataId}" formIndex="1" /></bu:td>
		</tr>
		<tr>
			<td class="name" /><bu:uitn colNo="CD_MEMO" formId="99338dfb95df4eec8d5714a4e773b127" /></td>
			<bu:td cssClass="dec" colNo="CD_MEMO" formId="99338dfb95df4eec8d5714a4e773b127" style="height:80px;" colspan="3"><bu:ui colNo="CD_MEMO" formId="99338dfb95df4eec8d5714a4e773b127" dataId="${dataId}" formIndex="1" style="width:100%;height:100%;"/></bu:td>
		</tr>
		</table>
...
<c:forEach items="${dataList}" var="cdlList">
	...
	<div class="datagrid-div2 datagrid-div6 datagrid-view" id="datagrid-view" style="width:100%;height:100%">
			<div class="datagrid-header">
				...
				<table  class="datagrid-htable fixedTable  " id="loadPoint1">        
					...
					<tr class="thcolor">
						...
						<td class="datagrid-cell" align="center" style="width:30px;text-align: center"><input type="checkbox" id="allSelect" name="allSelect"/></th >
						<td class="datagrid-cell" align="center" width="141px;" ><dict:lang value="Table" /></th >
						<td class="datagrid-cell" align="center" width="144px;" ><dict:lang value="料站位" /></th >
						<td class="datagrid-cell" align="center" width="147px;" ><dict:lang value="轨道" /></th >
						<td class="datagrid-cell" align="center" ><dict:lang value="说明" /></th >
					</tr>
					</tbody>
				</table>
				</div>
		...
		<table class="dlist-table" id="loadPoint" style="width:100%"><!--datagrid-btable-->
					<tbody id="addLoadPoint">
					<c:forEach items="${dataList}" var="cdlList">
								<tr id="${cdlList.rownum}" name="${cdlList.rownum}">
									<td class="datagrid-cell datagrid-cell-0" style="width:30px;text-align: center;">${cdlList.rownum}</td>
									...
									<td class="datagrid-cell datagrid-cell-2" width="141px;">
										${cdlList.cdl_table}
									</td>
									<td class="datagrid-cell datagrid-cell-3" width="144px;">
										${cdlList.cdl_loadpoint}
									</td>
									<td class="datagrid-cell datagrid-cell-4" width="147px;">
										${cdlList.cdl_track}
									</td>
...
<bu:script viewId="4edf9182b3d149a8a43eaffcfbf7e6c4" />
