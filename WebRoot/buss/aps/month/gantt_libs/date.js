...
Date.prototype.isSpecialDate=function(areaSn){
	return checkSpecialDate(this,areaSn);
}
...
Date.prototype.incrementDateByWorkingDays=function (days,areaSn) {
  ...
  if(typeof(areaSn)=="undefined"){
    ...
    var specialCheck={duration:1};
	  while (q > 0) {
	    //this.setDate(this.getDate() + (days > 0 ? 1 : -1));
	    this.setMinutes(this.getMinutes()+parseInt(specialCheck.duration));
      if (specialRestType.indexOf((specialCheck=this.isSpecialDate(areaSn)).type)<0)
...
//low performances in case of long distance
// bicch 22/4/2016: modificato per far ritornare anche valori negativi, cos矛 come la controparte Java in CompanyCalendar.
// attenzione che prima tornava 1 per due date uguali adesso torna 0
Date.prototype.distanceInWorkingDays= function (toDate,areaSn){
  if(typeof(areaSn)=="undefined"){
	  var pos = new Date(Math.min(this,toDate));
	  //pos.setHours(12, 0, 0, 0);
	  pos.setMinutes(pos.getMinutes(), 0, 0);
	  //var days = 0;
	  var minutes=0;
	  var nd=new Date(Math.max(this,toDate));
	  //nd.setHours(12, 0,0, 0);
	  nd.setMinutes(nd.getMinutes(), 0, 0);
	  while (pos < nd) {
	    //days = days + (isHoliday(pos) ? 0 : 1);
	    //pos.setDate(pos.getDate() + 1);
		  minutes=minutes+ (isHoliday(pos) ? 0 : 1);
		  pos.setMinutes(pos.getMinutes()+1);
	  }
	  //days=days*(this>toDate?-1:1);
	  minutes=minutes*(this>toDate?-1:1);
    ...
    var actduration=0;
      ...
      //pos.setDate(pos.getDate() + 1);
      //2018 by cc
      specialCheck=checkSpecialDate(pos,areaSn);
      actduration=(nd.getTime()-pos.getTime())/(1000*60);
      actduration=actduration<=parseInt(specialCheck.duration)?actduration:parseInt(specialCheck.duration);
      /////
		  minutes=minutes+ (specialRestType.indexOf(specialCheck.type)>=0 ? 0 : actduration);
		  pos.setMinutes(pos.getMinutes()+actduration);
	  }
	  //days=days*(this>toDate?-1:1);
	  minutes=minutes*(this>toDate?-1:1);
...
function initDateLang(lang){
	//初始化多语言
	Date.dayAbbreviations.forEach(function(item,index,array){
		Date.dayAbbreviations[index]=lang[item];
	});
	
	Date.monthAbbreviations.forEach(function(item,index,array){
		Date.monthAbbreviations[index]=lang[item];
	});
	
	Date.monthNames.forEach(function(item,index,array){
		Date.monthNames[index]=lang[item];
	});
}
