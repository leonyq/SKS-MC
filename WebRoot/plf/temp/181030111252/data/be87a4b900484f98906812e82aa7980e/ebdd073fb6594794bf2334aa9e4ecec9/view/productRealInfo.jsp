...
<body>
    <div class="content-in">
		...
		<bu:table cssClass="datagrid-htable fixedTable datagrid-div6 datagrid-htable-kz" id="datagrid-htableba0b1e72aae14ee38a2c1272e136dcd5"  style="min-width:1800px;">
		...
		<bu:tr cssClass="datagrid-header-row datagrid-title-ajax" id="datagrid-title-ajax">
			...
			<td class="datagrid-cell"><bu:uitn colNo="PRODUCT_NAME" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
			<td class="datagrid-cell"><bu:uitn colNo="PRODUCT_STANDARD" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
			<td class="datagrid-cell"><bu:uitn colNo="PRODUCT_COUNT" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
			<td class="datagrid-cell"><bu:uitn colNo="PM_PROCESS_FACE" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
			<td class="datagrid-cell"><bu:uitn colNo="PM_AREA_SN" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
			<td class="datagrid-cell"><bu:uitn colNo="PM_TARGET_QTY" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
			<td class="datagrid-cell"><bu:uitn colNo="PM_INPUT_COUNT" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
			<td class="datagrid-cell"><bu:uitn colNo="PM_FINISH_COUNT" formId="ba0b1e72aae14ee38a2c1272e136dcd5" /></td>
		...
		<table class="datagrid-btable fixedTable dblClick"  id="datagrid-btable-ajaxba0b1e72aae14ee38a2c1272e136dcd5" style="min-width:1800px;">
		<tbody id="tbody_ba0b1e72aae14ee38a2c1272e136dcd5" class="isParents">
		</tbody>
		</table>		</div>
		</div>
		</div>
...
function showGroupStatistic(moNumber){
				...
				success: function(data){
						...
						if(null != data.ajaxList){
							...
							for(var i = 0;i < rccList.length; i++){
								...
								if(rccList[i].PASSNUM==null) rccList[i].PASSNUM="";
								if(rccList[i].MGS_T_OK_PCS==null) rccList[i].MGS_T_OK_PCS="";
								if(rccList[i].WIP_NUM==null) rccList[i].WIP_NUM="";
								if(rccList[i].MGS_NG_PCS==null) rccList[i].MGS_NG_PCS="";
								if(rccList[i].MGS_T_NG_PCS==null) rccList[i].MGS_T_NG_PCS="";
								if(rccList[i].MGS_SCRAP_QTY==null) rccList[i].MGS_SCRAP_QTY="";
								$("#addGroupStatistic").append("<tr id='"+(i+1)+"' name='"+(i+1)+"' onclick=\"showTracking('"+(i+1)+"');\" ></tr>");
...
util.showLoading("处理中...");
			$.ajax({
			    type: "POST",
			    dataType: "json",
			    url: getTracking+"&moNumber="+moNumber+"&groupCode="+groupCode+"&groupSeq="+groupSeq,
			    data: "",
				success: function(data){
						util.closeLoading();
						...
						if(null != data.ajaxPage.dataList){
							...
							for(var i = 0;i < rccList.length; i++){
								...
								$("#addTracking tr:last").append("<td class='datagrid-cell datagrid-cell-2' style='width:100px;' title='"+rccList[i].WT_SN+"'>"+rccList[i].WT_SN+"</td>");
...
var isFirstLoad = firstLoadThisPage(paginationImpl);
	...
	util.showLoading("处理中...");
			$.ajax({
			    ...
			    dataType: "json",
			    url: url,
			    data: "paraMap.myMoNumber="+myMoNumber+"&paraMap.myGroupCode="+myGroupCode+"&paraMap.myGroupSeq="+myGroupSeq+"&page.currentPage="+_GLO_FORM_PAGE_MAP[paginationImpl].currentPage+"&page.pageRecord=" + _GLO_FORM_PAGE_MAP[paginationImpl].pageRecord,
				success: function(data){
						util.closeLoading();
						//console.log(data.ajaxPage.dataList);
						$("#addTracking").empty();
				...
				success: function(data){
						...
						if(null != data.ajaxPage.dataList){
							...
							for(var i = 0;i < rccList.length; i++){
								...
								var trHtml = "<tr class='datagrid-row "+backgroudColor+"' id='"+rccList[i].ID+"' onclick=\"showGroupStatistic('"+rccList[i].PM_MO_NUMBER+"');\" abbr='"+rccList[i].ID+"' style='cursor: pointer;' >"
			 					+"<input type='hidden' name='ID' value='"+rccList[i].ID+"' />"
			 					+"<td class='datagrid-cell' style='width:60px;text-align:center;' ><span class='_noedit'>"+rccList[i].ROWNUM+"</span></td>"
						...
						if(null != data.ajaxPage.dataList){
							...
							for(var i = 0;i < rccList.length; i++){
								...
								var trHtml = "<tr class='datagrid-row "+backgroudColor+"' id='"+rccList[i].ID+"' onclick=\"showGroupStatistic('"+rccList[i].PM_MO_NUMBER+"');\" abbr='"+rccList[i].ID+"' style='cursor: pointer;' >"
			 					...
			 					+"<td class='datagrid-cell' style='width:30px;text-align:center;' ><span class='_noedit'>"
			 					+"<input type='checkbox' name='"+formId+"$ID' value='"+rccList[i].ID+"' class='isSubmit _selectdata' style='cursor: pointer;margin:0'></td>"
			 					+"<td colno='PM_MO_NUMBER' class='datagrid-cell' >"
			 					+"<span colno='PM_MO_NUMBER' title='"+rccList[i].PM_MO_NUMBER+"'>"+rccList[i].PM_MO_NUMBER+"</span>"
			 					+"</td>"
			 					+"<td colno='PM_PROJECT_ID' class='datagrid-cell' >"
			 					+"<span colno='PM_PROJECT_ID' title='"+rccList[i].PM_PROJECT_ID+"'>"+rccList[i].PM_PROJECT_ID+"</span>"
			 					+"</td>"
			 					+"<td colno='DESTROY_NO' class='datagrid-cell' >"
			 					+"<span colno='DESTROY_NO' title='"+rccList[i].DESTROY_NO+"'>"+rccList[i].DESTROY_NO+"</span>"
			 					+"</td>"
			 					+"<td colno='PM_MODEL_CODE' class='datagrid-cell' >"
			 					+"<span colno='PM_MODEL_CODE' title='"+rccList[i].PM_MODEL_CODE+"'>"+rccList[i].PM_MODEL_CODE+"</span>"
			 					+"</td>"
								+"<td colno='PRODUCT_NAME' class='datagrid-cell' style='text-align:center;'>"
			 					+"<span colno='PRODUCT_NAME' title='"+rccList[i].PRODUCT_NAME+"'>"+rccList[i].PRODUCT_NAME+"</span>"
			 					+"</td>"
								+"<td colno='PRODUCT_STANDARD' class='datagrid-cell' >"
			 					+"<span colno='PRODUCT_STANDARD' title='"+rccList[i].PRODUCT_STANDARD+"'>"+rccList[i].PRODUCT_STANDARD+"</span>"
								...
								+"<td colno='PRODUCT_COUNT' class='datagrid-cell' >"
			 					+"<span colno='PRODUCT_COUNT' title='"+rccList[i].PRODUCT_COUNT+"'>"+rccList[i].PRODUCT_COUNT+"</span>"
			 					+"</td>"
								+"<td colno='PM_PROCESS_FACE' class='datagrid-cell' >"
			 					+"<span colno='PM_PROCESS_FACE' title='"+rccList[i].FACE_NAME+"'>"+rccList[i].FACE_NAME+"</span>"
			 					+"</td>"
								+"<td colno='PM_AREA_SN' class='datagrid-cell' >"
			 					+"<span colno='PM_AREA_SN' title='"+rccList[i].PM_AREA_SN+"'>"+rccList[i].PM_AREA_SN+"</span>"
								...
								+"<td colno='PM_FINISH_COUNT' class='datagrid-cell' >"
			 					+"<span colno='PM_FINISH_COUNT' title='"+rccList[i].PM_FINISH_COUNT+"'>"+rccList[i].PM_FINISH_COUNT+"</span>"
			 					+"</td>"
								+"<td colno='PRODUCT_OPENTIME' class='datagrid-cell' >"
			 					+"<span colno='PRODUCT_OPENTIME' title='"+rccList[i].PRODUCT_OPENTIME+"'>"+rccList[i].PRODUCT_OPENTIME+"</span>"
			 					+"</td>"
								+"<td colno='PM_START_DATE' class='datagrid-cell' >"
			 					+"<span colno='PM_START_DATE' title='"+rccList[i].START_DATE+"'>"+rccList[i].START_DATE+"</span>"
			 					+"</td>";
								if(i==0){
...
var firstGroupSeq = $("#firstGroupSeq").val();
	util.showLoading("处理中...");
			$.ajax({
			    type: "POST",
			    dataType: "json",
			    url: getTracking+"&moNumber="+firstMoNumber+"&groupCode="+firstGroupCode+"&groupSeq="+firstGroupSeq,
			    data: "",
				success: function(data){
						util.closeLoading();
...
function createTableModel(ms,formId){
    ...
    if(formId=='ba0b1e72aae14ee38a2c1272e136dcd5'){<%--生产实时信息--%>
        $.each(uiTxt,function(i,v){
            var backgroudColor = (i+1)%2 == 0 ? "tr-hover" : "";
            arrHtml.push("<tr class='datagrid-row "+backgroudColor+" "+trClass+"' id='"+v.ID+"' onclick=\"showGroupStatistic('"+v['PM_MO_NUMBER']+"');\" abbr='"+v.ID+"' style='cursor: pointer;"+trStyle+"' "+trEvent+">");
            ...
            arrHtml.push("<span colno='PM_MO_NUMBER' class='"+ms.uiClass(formId, 'PM_MO_NUMBER')+"' style='"+ms.uiStyle(formId, 'PM_MO_NUMBER')+"' "+ms.uiEvent(formId, 'PM_MO_NUMBER')+" title='"+ms.titleAttr(formId,'PM_MO_NUMBER','IS_TITLE_ATTR',v)+"' >"+v['PM_MO_NUMBER']+"</span>")
            arrHtml.push("</td>");
            ...
            arrHtml.push("<span colno='PM_PROJECT_ID' class='"+ms.uiClass(formId, 'PM_PROJECT_ID')+"' style='"+ms.uiStyle(formId, 'PM_PROJECT_ID')+"' "+ms.uiEvent(formId, 'PM_PROJECT_ID')+" title='"+ms.titleAttr(formId,'PM_PROJECT_ID','IS_TITLE_ATTR',v)+"' >"+v['PM_PROJECT_ID']+"</span>")
            arrHtml.push("</td>");
            ...
            arrHtml.push("<span colno='PM_MODEL_CODE' class='"+ms.uiClass(formId, 'PM_MODEL_CODE')+"' style='"+ms.uiStyle(formId, 'PM_MODEL_CODE')+"' "+ms.uiEvent(formId, 'PM_MODEL_CODE')+" title='"+ms.titleAttr(formId,'PM_MODEL_CODE','IS_TITLE_ATTR',v)+"' >"+v['PM_MODEL_CODE']+"</span>")
            arrHtml.push("</td>");
            ...
            arrHtml.push("</td>");
            arrHtml.push("<td colno='PM_TARGET_QTY' class='datagrid-cell "+ms.tdClass(formId, 'PM_TARGET_QTY')+"' style='"+ms.tdStyle(formId, 'PM_TARGET_QTY')+"' "+ms.tdEvent(formId, 'PM_TARGET_QTY')+">")
            ...
            arrHtml.push("</td>");
            arrHtml.push("<td colno='PM_INPUT_COUNT' class='datagrid-cell "+ms.tdClass(formId, 'PM_INPUT_COUNT')+"' style='"+ms.tdStyle(formId, 'PM_INPUT_COUNT')+"' "+ms.tdEvent(formId, 'PM_INPUT_COUNT')+">")
            ...
            arrHtml.push("</td>");
            arrHtml.push("<td colno='PM_FINISH_COUNT' class='datagrid-cell "+ms.tdClass(formId, 'PM_FINISH_COUNT')+"' style='"+ms.tdStyle(formId, 'PM_FINISH_COUNT')+"' "+ms.tdEvent(formId, 'PM_FINISH_COUNT')+">")
