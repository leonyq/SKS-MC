...
<body>
	...
	<input type="hidden" name="paraMap1@__DATA_COUNT__" value="1" />
		...
		<tr >
		    <td class="name" style="width:63px;"/><!--<span class="dot">*</span>--><bu:uitn colNo="CT_STATION_QTY" formId="9ce89b8198274a25b7219ad226eda21e" /></td>
					...
					<div class="datagrid-div2 datagrid-div6 datagrid-view"  id="datagrid-view">
						<div class="datagrid-header" style="width:100%;">
							<div class="datagrid-header-inner" style="width:100%;" id="datagrid-header-inner1">
								<table  class="datagrid-htable fixedTable  " id="datagrid-htable" style="width:750px;">
									<tbody>
						...
						<div class="datagrid-body datagrid-div3"   style="height:390px;">
							<table class="dlist-table table-line" style="width:750px;table-layout:fixed;"  id="itemInfo" >
		...
		<div class="dialog dialog-s1" id="addItem" style="display:none;z-index:1030;margin-left:0px;left:16px;top:10px;width:600px;height:390px;">
			<div class="dialog-hd">
			<h3 class="tit">
			<dict:lang value="选择机种" />
				...
				<div class="dialog-bd-div" style="width:600px;height:430px;">
				...
				<table class="basic-table">
			    	...
			    	<table  class="dlist-table table-line " style="width:100%;">
						...
						<tr>
							...
							<th style="width: 120px;text-align: center;"><dict:lang value="机种" /></th>
							<th style="width: 120px;text-align: center;"><dict:lang value="机种名称" /></th>
							<th style="width: 120px;text-align: center;"><dict:lang value="机种规格" /></th>
						</tr>
			    	</thead>
			    	</table>
			    	</div>
			    	<div style="width:600px;height:230px;overflow-y: auto;overflow-x: hidden;" class="scroll">  
...
function addItemInfo(){
		...
		util.showLoading("处理中...");
		  $.ajax({
				type: "POST",
			    dataType: "json",
			    url: "buss/bussModel_exeFunc.ms?funcMId=5cc1123925d24003904c6761cf4571e3",
				...
				success: function(data){
						...
						if(null != data.ajaxPage.dataList){
							...
							for(var i=0;i<rccList.length;i++){
								...
								$("#itemTable tr:last").append("<td class='datagrid-cell datagrid-cell-1' style='width:42px;text-align: center;'>"+(i+1)+"</td>");
								$("#itemTable tr:last").append("<td class='datagrid-cell datagrid-cell-2' style='width:42px;text-align: center;'><input id='"+(trCount+1)+"' type='checkbox' name='isSelect1'/></td>");
								$("#itemTable tr:last").append("<td class='datagrid-cell datagrid-cell-3' style='width:172px;text-align: center;'>"+rccList[i].CI_ITEM_CODE+"<input type='hidden' id='CI_ITEM_CODE"+(trCount+1)+"' value='"+rccList[i].CI_ITEM_CODE+"'/></td>");
								$("#itemTable tr:last").append("<td class='datagrid-cell datagrid-cell-4' style='width:172px;text-align: center;'>"+rccList[i].CI_ITEM_NAME+"<input type='hidden' id='CI_ITEM_NAME"+(trCount+1)+"' value='"+rccList[i].CI_ITEM_NAME+"'/></td>");
								$("#itemTable tr:last").append("<td class='datagrid-cell datagrid-cell-5' style='width:172px;text-align: center;'>"+rccList[i].CI_ITEM_SPEC+"<input type='hidden' id='CI_ITEM_SPEC"+(trCount+1)+"' value='"+rccList[i].CI_ITEM_SPEC+"'/></td>");
								trCount++;
							}
	...
	$("#itemTbody :checkbox").each(function(){
			if($(this).attr("checked")){
			    if (!$("#trr"+$(this).attr("id")).length) {
					...
					$("#itemInfo tr:last").append("<td class='datagrid-cell datagrid-cell-3' style='width:150px;text-align: center;'>"+$("#CI_ITEM_CODE"+$(this).attr("id")).val()+"<input type='hidden' name='CI_ITEM_CODE' value='"+$("#CI_ITEM_CODE"+$(this).attr("id")).val()+"'/></td>");
					$("#itemInfo tr:last").append("<td class='datagrid-cell datagrid-cell-4' style='width:120px;text-align: center;'>"+$("#CI_ITEM_NAME"+$(this).attr("id")).val()+"</td>");
					$("#itemInfo tr:last").append("<td class='datagrid-cell datagrid-cell-5' style='width:120px;text-align: center;'>"+$("#CI_ITEM_SPEC"+$(this).attr("id")).val()+"</td>");
...
function clickTr(tableId){
    ...
    function addCal(){      
            ...
            document.forms.addForm.action = "${path}buss/bussModel_exeFunc.ms?funcMId=46dd40bfb5a845d4a1e2204985044e3e";
