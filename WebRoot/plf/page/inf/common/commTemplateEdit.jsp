...
<head>
    ...
    <title><dict:lang value="消息通用模板修改" /></title>
...
<body>
    <div class="layui-fluid all">
        <div class="headline"><dict:lang value="消息通用模板" />-<dict:lang value="修改" /></div>

        <form id="add_form" class="layui-form" name="addForm" action="${path}infComm/infCommAction_commTemplateEdit.ms" method="post"<%-- target="submitFrame"--%>>
            <%--            <input type="hidden" name="mcIframeName" value="${mcIframeName}">--%>
            <input type="hidden" name="templateDto.id" id="templateDto_id" value="${dataMap.id}">
            <input type="hidden" name="templateDto.createUser" id="templateDto_createUser" value="${dataMap.CREATE_USER}">
            <input type="hidden" name="templateDto.createTime" id="templateDto_createTime" value="${dataMap.CREATE_TIME}">
            <input type="hidden" name="templateDto.isSystem" id="templateDto_isSystem" value="${dataMap.IS_SYSTEM}">

            <div class="layui-form-item">
                <div class="list-left"><span class="Eng">*</span><dict:lang value="标题" /></div>
                <div class="list-right">
                    <div class="pane">
                        <input type="text" class="layui-input" lay-verify="required|checkV1" name="templateDto.title" id="templateDto_title" value="${dataMap.TITLE}">
                    </div>
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="list-right">
                    <div class="pane">
                        <input type="text" class="layui-input" lay-verify="required|checkV2" name="templateDto.code" id="templateDto_code" value="${dataMap.CODE}"  ${dataMap.IS_SYSTEM eq "Y"? "readonly='readonly'":""}>
                    </div>
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="list-right">
                    <div class="pane" >
                        <dict:selectDict dictCode="YOrN" dictValue="%{dataMap.IS_DEFAULT}" name="templateDto.isDefault" id="templateDto_isDefault" custom_option="<option value=''>%{getText('--请选择--')}</option>" />
                    </div>
                </div>
            </div>
            <div class="layui-form-item">
                    ...
                    <div class="pane" >
                        <select id="templateDto_wxTemplateId" name="templateDto.wxTemplateId" lay-verify="required" class="layui-select">
                            ...
                            <c:forEach items="${ajaxList}" var="wxTemplate">
                                <option <c:if test="${wxTemplate.ID eq dataMap.WX_TEMPLATE_ID}"> selected="selected"</c:if> value="<c:out value='${wxTemplate.ID }'/>"><c:out value='${wxTemplate.CODE }'/></option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="list-right">
                    <textarea id="templateDto_template" lay-verify="required|checkV3" class="layui-textarea" name="templateDto.template" maxlength="3000"><s:property value="dataMap.TEMPLATE"/></textarea>
                </div>
            </div>
            <div class="layui-form-item">
                <div class="list-left"><dict:lang value="模板例子" /></div>
                <div class="list-right">
                    <textarea id="templateDto_example" class="layui-textarea" name="templateDto.example" maxlength="3000"><s:property value="dataMap.example"/></textarea>
                </div>
            </div>
