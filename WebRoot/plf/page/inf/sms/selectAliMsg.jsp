...
<body>
<div class="all">
	<form id="search_form" name="searchForm" action="${path}sys/ALIMsgAction_querySelPage.ms"  method="post">
		...
		<tr>
				<td width="50%">
							<div class="search_div">
								...
								<s:textfield id="name" name="aliMsgDtoSh.name" cssClass="input" maxlength="100"/>
								</div>
							</div>
							<div>
						</div>
...
<div class="all">	
	<div>
	<form id="moduleForm" name="moduleForm" action="${path}sys/ALIMsgAction_querySelPage.ms" method="post" target="submitFrame">
   <input type="hidden" id="filepaths" name="paraMap.filepaths"></input>
    ...
    <thead>
		<tr class="tdcolor">
			...
			<th width="15%"><dict:lang value="短信模板名称" /></th>
			<th width="15%"><dict:lang value="短信类型" />
			</th>
		</tr>
		<thead>
		<tbody>
		...
		<tr>
			<td><input type = "radio" name="alimsgId" value = "<c:out value='${ls.id}' />"></td>
			<td>${index.count}</td>
	...
	function add(){
		var name=null;
		var msgId=$('input:radio[name="checked"]:checked').val();
		 $('input[name="alimsgId"]:checked').each(function () {
        var tr = $(this).closest('tr') 
        var tds = tr.find('td');
	        name=tds.eq(3).text();
    		});
		_setSelValue(name,msgId)
	}
	

	<%-- 回填选择的值 --%>
	function _setSelValue(name,msgId){
		_thisSelObj=parent.window._thisSelObj;
		_thisSelObj.value = name;
		parent.window.$("input[id='_aliMsg_id']").val(msgId);
		_thisSelObj.focus();
		parent.closePopWin();
	}
