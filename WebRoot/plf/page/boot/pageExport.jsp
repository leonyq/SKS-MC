				...
				<div id="iframe1" style="height: 90%;">
					...
					<div class="iframe1_left" id="module_tree"
						...
						<%--<select id="moduleExportType" name="moduleExportType"
								...
								--%>
								<s:hidden name="moduleExportType" value="3" id="moduleExportType"></s:hidden>
								<h3 class="moduleTitle">公用页面</h3>
...
<%--						</iframe>--%>
					...
					<div class="txac">
						...
						<input id="nextButton"type="button" value="<dict:lang value="上一步" />"  onclick="up(this);" class="layui-btn layui-btn-warm">
			...
			if(zTreesArr.length>0){
				...
				for (var i = 0; i < zTreesArr.length; i++) { 
					if(!zTreesArr[i].children){//判断不包含子结点
						...
						exportModule.pageType.push({
								"id": zTreesArr[i].id,
						});
					}
			    }
			}else{
				delete exportModule.pageType
			}
		};
		//ztree部分
		var setting = {
			...
			$.ajax({
				...
				success: function(data){
					if(data){
						...
						var arr = exportModule.pageType; //模块
