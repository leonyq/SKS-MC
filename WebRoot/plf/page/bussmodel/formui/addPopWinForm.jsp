...
<body>
		<div class="all">
				...
				<table class="add_table element_table">
					<tr>
						...
						<td >
							...
							name="paraMap.UI_DATA_TYPE"	onchange="setUiData(this);" filterVal="1,3,4" />
							-->
			...
			<i class="editCode" style="float: right;margin-right:20px;" id="editCode" onclick="editSqlValFun('comm_sql');"></i> 
							...
							<table class="table_list table-mc-form" id="fieldsTable" >
								<thead>
									<tr>
										...
										<th style="background: rgb(242,242,242);white-space: normal;padding: 0px 0px !important;"><dict:lang value="多语言" /></th>
					...
					<tr id="UI_DATA_13">
						...
						<td >
						<input type="text"  id="paraMap_IS_ORDER_BY" lay-verify="paraMap_IS_ORDER_BY_verify" name="paraMap.IS_ORDER_BY" value="<s:property value="dataMap.IS_ORDER_BY"/>"  />
						</td>
					</tr>		
					...
					--%><tr id="UI_DATA_12">
						...
						<td >
						<dict:selectDict dictCode="IS_DATA_AUTH"  custom_option="<option value=''>%{getText('--请选择--')}</option>" dictValue="%{dataMap.IS_DATA_AUTH}" name="paraMap.IS_DATA_AUTH"  />
						</td>
					</tr>		
					<tr id="UI_DATA_12">
						<td  class="tr1 title" >
					...
					<tr id="UI_DATA_12">
						...
						<td >
							<input type="text" id="paraMap_WP_HEIGHT" lay-verify="paraMap_WP_HEIGHT_verify" name="paraMap.WP_HEIGHT" value="<s:property value="dataMap.WP_HEIGHT"/>" />
						</td>
					</tr>	
					<tr id="UI_DATA_12">
						<td  class="tr1 title" >
			...
			<i class="editCode" style="float: right;margin-right:30px;" id="editCode" onclick="editJsValFun('paraMap_UI_DATA_4');"></i> 
						...
						<td >
							<textarea id="paraMap_UI_DATA_4" name="paraMap.UI_DATA_4" rows="4" 
								style="width: 100%;overflow:scroll;word-wrap:normal;"><s:property value="dataMap.UI_DATA_4"/></textarea>
						</td>
					</tr>
					<tr id="UI_DATA_5">
						<td  class="tr1 title" >
							<dict:lang value="回调函数名称" />
						</td>
						<td >
							<input type="text" id="paraMap_UI_DATA_5" lay-verify="paraMap_UI_DATA_5_verify" name="paraMap.UI_DATA_5" 
							value="<s:property value="dataMap.UI_DATA_5"/>" />
						</td>
					</tr>
					<tr id="UI_DATA_6">
						<td  class="tr1 title" >
							<dict:lang value="回调函数体" />
							<br>
			<i class="editCode" style="float: right;margin-right:30px;" id="editCode" onclick="editJsValFun('paraMap_UI_DATA_6');"></i> 
					...
					<tr id="UI_DATA_15">
						<td  class="tr1 title" > 
							<dict:lang value="清空函数名称" />
							
						</td>
						<td >
							<input type="text" id="paraMap_UI_DATA_15" lay-verify="paraMap_UI_DATA_15_verify" name="paraMap.UI_DATA_15" 
                              value='<s:property value="dataMap.UI_DATA_15"/>'/> 
						</td>
					</tr>
					
					<tr id="UI_DATA_14">
						<td  class="tr1 title" > 
							<dict:lang value="清空值函数" />
							<br>
			<i class="editCode" style="float: right;margin-right:30px;" id="editCode" onclick="editJsValFun('paraMap_UI_DATA_14');"></i> 
						</td>
						<td >
							<textarea id="paraMap_UI_DATA_14" name="paraMap.UI_DATA_14" rows="4" 
								style="width: 100%;overflow:scroll;word-wrap:normal;"><s:property value="dataMap.UI_DATA_14"/></textarea>
						</td>
					</tr>
	...
	function valData(){
		...
		var val2 = db_sql.value.toUpperCase();
	   	...
	   	var paraMap_IS_ORDER_BY = document.getElementById("paraMap_IS_ORDER_BY");
	   	if(!validateInput(paraMap_IS_ORDER_BY))
	   	{
	   		return false;
	   	}
	   	...
	   	var paraMap_UI_DATA_15 = document.getElementById("paraMap_UI_DATA_15");
	   	if(!validateInput(paraMap_UI_DATA_15))
	   	{
	   		return false;
	   	}
	   	*/
		...
		var backVal = true;
	   	...
	   	var fileStrs = "";//字段字符集，用来验证是否重复字段名
	   	var len = $("#fieldsTable tr:gt(0)").length;
	   	
	   	/*
	    if("<c:out value='${paraMap.UI_COMP__TYPE}' />" != "UI_COMP_VIEW" && len==0){
	    ...
	    */
		...
		$("#fieldsTable tr:gt(0)").each(function(i){
			...
			multiLang=$(this).find("input[type=checkbox]").is(':checked')?true:false;
			...
			if(showName.indexOf("\"") > 0){
				_alertValMsg($($(this).find("input").get(1)),"<dict:lang value="别名" /> -- <dict:lang value="不能包含敏感字符:如双引号,括号" />");
				backVal = false;
				return false;
			}
			...
			if(fileStrs.toUpperCase().indexOf(fieldName.toUpperCase()) != -1){
			...
			filedsVal += "{\"FIELD_NAME\":\""+fieldName+"\",\"SHOW_NAME\":\""+showName+"\",\"IS_SEARCH\":\""+selVal+"\",\"OBJ_STYLE\":\""+objStyle+"\",\"IS_SHOW\":\""+isShowVal+"\",\"MULTI_LANG\":\""+multiLang+"\",\"TYPE_NAME\":\""+typeName+"\"}";
			fileStrs += fieldName+",";
			if(i < len - 1){
				filedsVal += ",";
			}
	...
	function init(){
		...
		function addTr(){
			...
			var htmlTr = "<tr >"
						...
						+"<td style='white-space: normal;padding: 0px 0px !important;'><input type='text' class='mc-list-input-edit' id='objStyle_"+rowNo+"' name='objStyle_"+rowNo+"' style='width:100%;'/></td>"
						+"<td style='white-space: normal;padding: 0px 0px !important;'><input type='checkbox' lay-skin='primary' id='multi_lang_"+rowNo+"'  name='multi_lang_"+rowNo+"' checked value='true' style='width:20px;'/></td>"
		...
		$(function(){
			...
			if(data7 != null &&　data7　!= ""){
				...
				for(var i = 0;i < data7.length; i++){
					...
					var multi_lang=(function(){
						if(data7[i].MULTI_LANG==undefined)return true;
						return Boolean(eval(data7[i].MULTI_LANG));
					})();
					if(multi_lang==true||multi_lang==undefined){
					}else if(multi_lang==false){
						$(lastTr).find("input[type=checkbox]").prop('checked',false);
					}
				}
			}
...
$(function(){
	layui.use('form', function(){
		    ...
		    form.verify({
		        paraMap_IS_ORDER_BY_verify: function(value,item){
					   	var paraMap_IS_ORDER_BY = document.getElementById("paraMap_IS_ORDER_BY");
	   					if(!validateInput(paraMap_IS_ORDER_BY))
					   	{
					   		return "<dict:lang value="不能包含敏感字符:如引号,括号等特殊符号" />";
		    ...
		    form.verify({
		        paraMap_UI_DATA_15_verify: function(value,item){
					   	var paraMap_UI_DATA_15 = document.getElementById("paraMap_UI_DATA_15");
	   					if(!validateInput(paraMap_UI_DATA_15))
					   	{
					   		return "<dict:lang value="不能包含敏感字符:如引号,括号等特殊符号" />";
