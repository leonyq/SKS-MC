...
<head>
	...
	<title><dict:lang value="多选按钮" />UI <dict:lang value="配置" />
	</title>
	...
	<div class="all">
		<div class="add_table_div">
			<form id="add_form" name="addForm" class="layui-form" action="${path}sys/BussModelAction_cfgUIComp.ms" method="post" target="submitFrame">
				...
				<table class="add_table element_table">
					...
					<tr id="UI_DATA_1" style="display: none;">
						...
						<td >
							<input type="text" id="paraMap_UI_DATA_1" lay-verify="paraMap_UI_DATA_1_verify" name="paraMap.UI_DATA_1" value="<s:property value="dataMap.UI_DATA_1"/>" />
			...
			<i class="editCode" style="float: right;margin-right:20px;" id="editCode" onclick="editSqlValFun('comm_sql');"></i> 
			 ...
			 <textarea id="comm_sql" name="paraMap.UI_DATA_11" lay-verify="comm_verify" rows="6"
			 ...
			 <textarea id="oracle_sql" name="paraMap.UI_DATA_8" lay-verify="oracle_verify" rows="6"
              ...
              <textarea id="pg_sql" name="paraMap.UI_DATA_PG" lay-verify="pg_verify" rows="6" 
     	...
     	if(typeVal == "1")
     	{
	     	var paraMap_UI_DATA_1 = document.getElementById("paraMap_UI_DATA_1");
	       	if(val.isBlank(paraMap_UI_DATA_1)){
	   		  _alertValMsg(paraMap_UI_DATA_1,"<dict:lang value="手工输入常量" /> -- <dict:lang value="不能为空" />");
	   		  return false;
	     	}
     	 
     	var bool = $("#paraMap_UI_DATA_1").val().indexOf($("#paraMap_UI_DATA_1_0").val());  
	     	  if(bool==-1){//手工输入常量	要包含 分隔符 
	     	  _alertValMsg(paraMap_UI_DATA_1,"<dict:lang value="手工输入常量" /> -<dict:lang value="要包含" />- <dict:lang value="数据分隔符" />");
	   		  return false;
	     	}
     	}   	 
 
        //2018-06-28 10:29 --> cjdjk  end
...
var typeVal = $("#paraMap_UI_DATA_TYPE").val();
if(typeVal == "2"){
   var db_sql = document.getElementById($('input:radio[name="paraMap.codeType"]:checked').val()+"_sql");
   var val2 = db_sql.value.toUpperCase();
			...
			form.on('select(setUiData)', function(data){
		 	...
		 	form.verify({
		   		 paraMap_UI_DATA_1_0_verify: function(value,item){
				     	...
				     	var bool = $("#paraMap_UI_DATA_1").val().indexOf($("#paraMap_UI_DATA_1_0").val());  
					     	  if(bool==-1){//手工输入常量	要包含 分隔符 
					   		  return "<dict:lang value="手工输入常量" /> -<dict:lang value="要包含" />- <dict:lang value="数据分隔符" />";
					     	}
				     	} 
		     ...
		     form.verify({
		   		 paraMap_UI_DATA_1_verify: function(value,item){
		   		 		...
		   		 		if(typeVal == "1")
				     	{
					     	var paraMap_UI_DATA_1 = document.getElementById("paraMap_UI_DATA_1");
					       	if(val.isBlank(paraMap_UI_DATA_1)){
					   		  return "<dict:lang value="手工输入常量" /> -- <dict:lang value="不能为空" />";
					     	}
