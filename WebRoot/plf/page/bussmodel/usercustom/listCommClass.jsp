...
<div >	
	<div>
	<form id="searchForm" name="searchForm" action="${path}sys/UserCustomAction_listCommClass.ms" method="post">
    ...
    <table id="listTable" class="table_list table-left" >
		...
		<tr class="tdcolor">
			...
			<!-- <th width="160px;"><dict:lang value="操作" />
...
<script type="text/javascript">
	...
	function add(url){
		...
		url = "${path}sys/UserCustomAction_showAddCommClass.ms?paraMap.UP_ID=<s:property value='paraMap.UP_ID' />&mcIframeName="+mcIframeName;
		showPopWin(url, 450, 180,null,"<dict:lang value="公用类" /> <dict:lang value="新增" />");
	}
	var isModify=false;
	...
	function update(id){
		var mcIframeName = window.parent.name; //外层iframe的name
		var url = "${path}sys/UserCustomAction_showEditCommClass.ms?paraMap.ID="+id+"&mcIframeName="+mcIframeName;
		//window.top.showPopWin(url, "95%", "100%",closePopCbFun,"<dict:lang value="自定义类" /> <dict:lang value="修改" />");
		showPopWin(url, "400", "240",closePopCbFun,"<dict:lang value="自定义类" /> <dict:lang value="修改" />");
	}
    ...
    document.forms.submitForm.action="${path}sys/UserCustomAction_delCommClass.ms?paraMap.UP_ID=<s:property value='paraMap.UP_ID' />&mcIframeName="+mcIframeName;
		$("#del_id").val(delId);
