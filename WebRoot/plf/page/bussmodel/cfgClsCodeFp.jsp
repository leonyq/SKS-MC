...
<head>
	...
	<jsp:include page="/plf/common/fp/pub_head_fp.jsp">
		<jsp:param name="subStyle" value="1" />
	...
	<style>
		...
		._optn button .cancel {
			background: url(${path}plf/page/fp/img/ico-cancel.png?_mc_res_version=<%=PlfStaticRes.ICO_SAVE_H_PNG%>) no-repeat;
		}

		._optn button .debug {
			background: url(${path}plf/page/fp/img/ico-tb.png?_mc_res_version=<%=PlfStaticRes.ICO_SAVE_H_PNG%>) no-repeat;
		}
		._optn button .scrap {
			background: url(${path}plf/page/fp/img/ico-scrap.png?_mc_res_version=<%=PlfStaticRes.ICO_SAVE_H_PNG%>) no-repeat;
		}
	</style>
</head>
...
<div class="all">
	<div class="add_table_div">
		...
		<form id="editForm" class="layui-form" name="editForm"
			...
			<div class="ptop_10 txac _optn">
				<!--
					<button  style="margin:2px 2px  ;" class="layui-btn" onclick="closePage();">取消</button>
				...
				<button  type="button" id="submitBut" onclick="submitCode()"><i class="ico save"></i>提交</button>

				<button type="button" onclick="closePage();" ><i class="ico cancel"></i>取消</button>
				<s:if test="@com.more.fw.core.common.method.StringUtilsMc@isNotBlank(paraMap.CODE_ID)">
				<button type="button" onclick="validateIsCurrentsDebug()"><i class="ico debug"></i>调试</button>
					<button type="button" id="del_but" onclick="del(this);"><i class="ico scrap"></i>删除</button>
				</s:if>
...
<script type="text/javascript">
	...
	function reloadPg(msg) {
		isSubmit = false;
		top.utilsFp.alert(msg, reloadPgExe);
	}
	var piframeName = '<s:property value="paraMap.piframeName"/>';
	var piframeId = '<s:property value="paraMap.piframeId"/>';
	var pIframe = $("#"+piframeId, top.document)[0].contentWindow;
	// var piframeName = '<s:property value="paraMap.piframeName"/>';
	...
	function errorCallBackFun(msg){
		isSubmit = false;
		utilsFp.alert("error:"+msg.responseText);
	}
	...
	function closePage() {
		parent.closePopWinFp('${paraMap.popWinframeId}pop');
	}
