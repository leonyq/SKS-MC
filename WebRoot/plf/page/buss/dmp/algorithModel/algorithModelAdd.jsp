...
<head>
  ...
  <title><dict:lang value="算法模型信息新增"/></title>
...
<body>
    <div class="layui-fluid">
        ...
        <form id="add_form" class="layui-form" name="addForm" action="${path}dmp/DmpAction_algorithModelAdd.ms" method="post">
            <div class="layui-form-item">
                <div class="list-left"><span class="Eng">*</span><dict:lang value="算法名称"/></div>
                <div class="list-right">
                    <div class="pane">
                        <input type="text" placeholder="<dict:lang value="请输入算法名称"/>"  maxlength="20"  class="layui-input" name="procedureDto.wplCode" id="procedureDto_wplCode" onblur="validateCode(this.value)" lay-verify="required" >
                    </div>
                </div>
            </div>
            <div class="layui-form-item">
                <div class="list-left"><span class="Eng">*</span><dict:lang value="算法描述"/></div>
                <div class="list-right">
                    <div class="pane">
                        <input type="text"  placeholder="<dict:lang value="请输入算法描述"/>"    maxlength="33"  class="layui-input" name="procedureDto.wplName" id="procedureDto_wplName" lay-verify="required">
                    </div>
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="list-right">
                    ...
                    <div class="layui-form" lay-filter="pane1" style="position:absolute;float:left;" id="wpltype0" >
                     <!-- <dict:selectDict laysearch="" verify="required"   dictCode="ALGORITH_MODEL_TYPE" dictValue="%{TYPE}" name="procedureDto.wplType" id="procedureDto_wplType" custom_option="<option value=''>%{getText('--请选择--')}</option>" />  -->
                     <select lay-search verify="required" name="procedureDto.wplType" id="procedureDto_wplType">
            ...
            <div class="layui-form-item">
                ...
                <div class="list-right">
                	<script id="editor"  name="procedureDto.wplDesc"  type="text/plain" style="height:350px"></script>
                </div>
            </div>
          
        
        <div class="ptop_10" style="z-index:1000">
         	 <button type="button" class="layui-btn algsub" lay-filter="filterSubmit" onclick="addAlg()" ><dict:lang value="保存"/></button>
...
function bulidEvenForDmpIcon(){
 ...
 /*        $(".layui-select-title .layui-input").on("keyup",function(e){  	 
	...
	var typeStr2 = '            <div style="margin-top:30px;"><div class="layui-form-item">\
                <div class="list-left"><span class="Eng">*</span><dict:lang value="编码"/></div>\
                <div class="list-right">\
                    <div class="pane">\
                        ...
                        <input type="text" value="'+typeCode+'"  placeholder="<dict:lang value="请输入应用分类编码"/>"    maxlength="33"  class="layui-input" name="typeCode" id="typeCode" >\
                                   ...
                                   <div class="layui-form-item">\
                <div class="list-left"><span class="Eng">*</span><dict:lang value="名称"/></div>\
                <div class="list-right">\
                    <div class="pane">\
    ...
    */
    ...
    function toCodeView(){
     	$.post("${path}dmp/DmpAction_isHaveSourceCode.ms",{code:$("#procedureDto_wplCode").val()},function(res){
  		    ...
  		    }else{console.log($("#procedureDto_wplCode").val());
  		        var url = "${path}dmp/DmpAction_preAlgorithModelSourceCode.ms?paraMap.code="+$("#procedureDto_wplCode").val()+"&paraMap.id="+$("#procedureDto_id").val()+"&paraMap.type=add";
...
<script type="text/javascript">
   ...
   if(wplCode==""){
   layer.msg("<dict:lang value='算法名称不能为空'/>", {time: 1000, icon:5});
   return false;
   }

   var wplType = $("#procedureDto_wplType").val();
   if(wplType==""){
   layer.msg("<dict:lang value='应用分类不能为空'/>", {time: 1000, icon:5});
   return false;
   }  
   var wplName = $("#procedureDto_wplName").val();
   if(wplName==""){
   layer.msg("<dict:lang value='算法描述不能为空'/>", {time: 1000, icon:5});
   return false;
   }

       //  var content_old = ue.getPlainTxt();
		//if(content_old!=''){
		//	var content = ue.execCommand('insertHtml', content_old);
	//	}
              
   
    		var flag='${param.flag}';
    		var url = "${path}dmp/DmpAction_algorithModelAdd.ms";
	    jQuery.ajax({
		type: "POST",
	    dataType: "json",
	    data:{
		...
		success: function(data){	
				...
				if(flag=="orderSetting"){										
			  	  	window.parent.addAlgList(data.ajaxMap,"1");			  	 	 					  	 					
