...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
   <base href="${path}" />
  <title><dict:lang value="DMP服务新增"/></title>
    ...
    <div class="layui-fluid" style="padding-bottom:78px;overflow: auto;">
	        ...
	        <div class="layui-row">
	            ...
	            <div class="searchItem">
	                ...
	                <div class="search_r" title="<dict:lang value='DMP服务的唯一标识码，用户可自定义输入'/>">
	                    <input type="text" name="DMP_ID" id="DMP_ID" maxlength="50" class="layui-input" onblur="validateCode(this.value)"  value="">
	                </div>
	            </div>
	            <div class="searchItem">
	                <div class="search_l"><span class="Eng">*</span><dict:lang value="服务名称"/></div>
	                <div class="search_r" title="<dict:lang value='针对DMP服务的描述内容'/>">
	                   	<input type="text"  name="DMP_REMARK" id="DMP_REMARK" maxlength="50" class="layui-input">
	                </div>
	            </div>
	            <div class="searchItem">
        ...
        <form class="layui-form searchDiv searchDivSmall" action="">
	        <div class="layui-row">
	            <div class="searchItem">
	                <div class="search_l">
	                <span><dict:lang value="内部处理器数量"/></span>
	                </div>
	                <div class="search_r" title="<dict:lang value='DMP服务内部执行任务调度的对象，增大或缩小会影响DMP处理效率，建议值为100，可根据设备数量增多进行增加'/>">
	                    <input type="text"  placeholder="100"  name="maxprocessor" id="maxprocessor" maxlength="50" class="layui-input">
	                </div>
	            </div>
	            <div class="searchItem">
	            ...
	            <div class="searchItem">
	                ...
	                <div class="search_r" title="<dict:lang value='设置过小DMP服务会执行自动扩容，扩容会间断性的影响DMP服务内部任务的执行效率，建议值40，可随设备的增多进行增加'/>">
	                   	<input type="text" placeholder="40" name="minpoolsize" id="minpoolsize" maxlength="50" class="layui-input">
	                </div>
	            </div>
	            <div class="searchItem">
	                <div class="search_l"><dict:lang value="线程池最大容量"/></div>
	                <div class="search_r" title="<dict:lang value='设置过大会造成资源浪费，建议值150，可随设备的增多进行增加'/>">
	                   	<input type="text" placeholder="150" name="maxpoolsize" id="maxpoolsize" maxlength="50" class="layui-input">
	                </div>
	            </div>
	            <div class="searchItem">
	                ...
	                <div class="search_r" title="<dict:lang value='DMP服务内部任务缓存通道，当DMP执行效率降低时，设置过小会造成任务丢失，建议值500'/>">
	                   	<input type="text" placeholder="500" name="channelsize" id="channelsize" maxlength="50" class="layui-input">
	                </div>
	            </div>
          ...
          <div class="ptop_10">
          	  ...
          	  <!-- 编辑页面多一个新增   -->
	          <button class="layui-btn"  onclick="submitForm()" ><dict:lang value="保存"/></button>   
	          <button class="layui-btn layui-btn-danger"  onclick="window.history.back()" ><dict:lang value="取消"/></button>   
	      </div> 
    </div>
...
<script>
	...
	function submitForm(){
		...
		if(!/^[\w\u4e00-\u9fa5]+$/.test($("#DMP_ID").val())){
			$("#DMP_ID").val("");          
			layer.msg("<dict:lang value='DMP服务ID包含特殊字符'/>", {time: 1000, icon:5});
			return;
        }		
		if($("#DMP_REMARK").val()==""){
			$("#DMP_REMARK").val("");
    	...
    	var jsondata={"paraMap.DMP_ID":DMP_ID,"paraMap.DMP_REMARK":DMP_REMARK,"paraMap.paramFlagList":JSON.stringify(paramFlagList),
    			"paraMap.paramRemarkList":JSON.stringify(paramRemarkList),"paraMap.paramValueList":JSON.stringify(paramValueList),
    	    	"paraMap.beginIpList":JSON.stringify(beginIpList),"paraMap.endIpList":JSON.stringify(endIpList)};
    	
    	//console.log(jsondata);
    	var url = "${path}dmp/DmpServiceAction_dmpServiceAddInfo.ms";
    	jQuery.ajax({
    		type: "POST",
    	    dataType: "json",
    	    url: url,
