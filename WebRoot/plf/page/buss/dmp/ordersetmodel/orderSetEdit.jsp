...
<head>
  ...
  <title><dict:lang value="指令集信息修改"/></title>
        ...
        <form id="edit_form" class="layui-form" name="editForm" action="" method="post">
            ...
            <div class="layui-row">
                <div class="searchItem">
                    ...
                    <div class="search_r">
                        <input type="text"  maxlength="20"  placeholder="<dict:lang value="请输入指令集代码"/>"  readonly="readonly" class="layui-input" name="orderSetDto.tcsCode" id="orderSetDto_tcsCode" value="${dataMap.TCS_CODE}"   >
                    </div>
                </div>
                <div class="searchItem">
                    <div class="search_l"><span class="Eng" style="color:red;">*</span><dict:lang value="指令集名称" /></div>
                    <div class="search_r">
                        <input type="text"  maxlength="33" placeholder="<dict:lang value="请输入指令集名称"/>" class="layui-input" name="orderSetDto.tcsName" id="orderSetDto_tcsName"  value="${dataMap.TCS_NAME}">
                    </div>
                </div>
                <div class="searchItem" style="width:100%">
                    <div class="search_l"><dict:lang value="备注信息" /></div>
                    <div class="search_r">
                        <input style="height:38px;padding:8px;"  class="layui-input"  name="orderSetDto.tcsRemark" id="orderSetDto_tcsRemark"  maxlength="500" value="${dataMap.TCS_REMARK}"  />
                    </div>
                </div>
            </div>
        </form>
...
<%-- 异步加载指令列表数据 --%>
	...
	function submitForm(){
    	...
    	var url = "${path}dmp/OrderSetAction_ordersetModelEdit.ms";
    	var operHtml = "";
        var i=0;
        var detailList = {};
	    ...
	    dataType: "json",
	    url: url,
	    data: {"paraMap.TCS_CODE":tcsCode,"paraMap.TCS_NAME":tcsName,"paraMap.TCS_REMARK":tcsRemark,"paraMap.returnType":returnType,"paraMap.orderSetDetailList":JSON.stringify(detailList)},
		success: function(data){
            ...
            if(result=="FAIL2"){
            	...
            	layer.msg("<dict:lang value='修改失败'/>", {time: 1000, icon:5});
             	...
             	if(result=="SUCCESS"){
	            	...
	            	var url = "${path}dmp/OrderSetAction_preOrderSetModelView.ms?paraMap.tcsCode="+result;//?paraMap.UP_ID="+$("#up_id").val()+"&paraMap.typeFlag="+$("#typeFlag").val()+"&paraMap.mcIframeName="+window.parent.name;
