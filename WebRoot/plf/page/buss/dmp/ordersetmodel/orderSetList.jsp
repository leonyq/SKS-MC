...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
   <base href="${path}" />
  <title><dict:lang value="指令集信息列表"/></title>
		  ...
		  <s:hidden name="%{@com.more.fw.core.common.method.ConstantsMc@FUNC_CODE}" />
            <div class="layui-row">
                ...
                <div class="searchItem">
                    ...
                    <div class="search_r">
                        <input type="text"  id="paraMap_code" name="paraMap.TCS_CODE" maxlength="30" class="layui-input">
                    </div>
                </div>
                ...
                <div class="searchItem">
                    ...
                    <div class="search_r">
                        <input type="text"  id="paraMap_name" name="paraMap.TCS_NAME" maxlength="30"  class="layui-input">
                    </div>
                </div>
...
layui.use([ 'table', 'element', 'laypage'], function(){
  ...
  table.on('rowDouble', function(obj){
	  ...
	  //第一个实例
	    ...
	    id:'dmpList',
	    elem: '#dmpList',
	    url: '${path}dmp/OrderSetAction_fetchOrderSetModelList.ms', //数据接口
	    page: true, //开启分页
	    method: 'post',
	    limits:[10,20,50,100],
	    height: 'auto',
	    where:{"FUNC_CODE":"F_2704e59a41f64756bb0be83d557b4d5e"},
	    ...
	    cols: [[ //表头
	       ...
	       {field: 'TCS_CODE', title: '<dict:lang value="指令集代码"/>', sort: true},
	       {field: 'TCS_NAME', title: '<dict:lang value="指令集名称"/>', sort: true},
	       {field: 'TCS_REMARK', title: '<dict:lang value="指令集说明"/>', sort: true}
	    ]],
	  ...
	  function deleteData(rowIds,type){
       	var item = layer.open({
			  ...
			  $.post("${path}dmp/OrderSetAction_delAjaxformParam.ms",{rowIds:rowIds},function(res){
				  if(type=="list"){
				   reloadTable();
				  }else{
				  listCardAx();	
	 ...
	 var reloadTable = function(){
		  ...
		  formData["FUNC_CODE"] = "F_2704e59a41f64756bb0be83d557b4d5e";
		  tableIns.reload({
			  where: formData,
			  page: {
			    curr: currPage
    ...
    function edit(id){			
        var url = "${path}dmp/OrderSetAction_preOrderSetModelEdit.ms?paraMap.tcsCode="+id;//?paraMap.UP_ID="+$("#up_id").val()+"&paraMap.typeFlag="+$("#typeFlag").val()+"&paraMap.mcIframeName="+window.parent.name;
        window.location.href= url;
    }
    ...
    function view(id){			
        var url = "${path}dmp/OrderSetAction_preOrderSetModelView.ms?paraMap.tcsCode="+id;//?paraMap.UP_ID="+$("#up_id").val()+"&paraMap.typeFlag="+$("#typeFlag").val()+"&paraMap.mcIframeName="+window.parent.name;
        window.location.href= url;
    }
</script>
...
<script>
	...
	<%-- 异步刷新卡片列表数据 --%>
	function listCardAx(){
		var url = "${path}dmp/OrderSetAction_fetchOrderSetModelListCard.ms";//fetchOrderSetModelListCard
		var formData = form.serialize($("#searchForm"));
	    jQuery.ajax({
		type: "POST",
	    dataType: "json",
				...
				try{
					if(null !=data.ajaxMap){	
						...
						$.each(data.ajaxMap,function(key,values){   
                    		...
                    		if(values.length>20){
                    			operHtml =operHtml +"<div class='moreBtn moreloadlist' moreload_dataauth_id='"+arr[1]+"'><span class='text'>更多</span></div>";
                    		}
						    ...
						    $.each(values, function(i, paraDto) {
                				...
                				operHtml =operHtml +"<div class='item' rowId="+paraDto.TCS_CODE +" code="+paraDto.TCS_CODE+">";
                    			operHtml =operHtml +"<div class='p1'><span class='ico'></span>"+paraDto.TCS_CODE+"</div>";
                    			operHtml =operHtml +"<div class='p2' title="+paraDto.TCS_NAME+">"+paraDto.TCS_NAME+"</div>";
