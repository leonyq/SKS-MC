...
<div class="all">	
	<div>
	<form id="searchForm" name="searchForm" action="${path}webservice/ServerMgrAction_listModule.ms" method="post">
    ...
    <table id="listTable" class="table_list" style="table-layout: fixed;">
		...
		<tr class="tdcolor">
			...
			<th width="50px"><dict:lang value="操作" />
	...
	<s:iterator value="page.dataList" id="module" status="index">
		<tr id="<s:property value="#module.id"/>">
			...
			<td class="addTd" style="white-space: nowrap;overflow:hidden;text-overflow: ellipsis;" title=<s:property value="id"/>><s:property value="#module.id"/></td>
			<td class="addTd" style="white-space: nowrap;overflow:hidden;text-overflow: ellipsis;" title=<s:property value="name"/>>
				<span class="addSpan">
				<s:property value="#module.name"/></span>
				<input maxlength="16" class="addInput" style="display:none" value="<s:property value='#module.name'/>">
			</td>
			<td class="addTdDec" class="addTd" style="white-space: nowrap;overflow:hidden;text-overflow: ellipsis;" title=<s:property value="memo"/>>
				<span class="addSpanDec">
				<s:property value="#module.memo"/></span>
				<input maxlength="32" class="addInputDec" style="display:none" value="<s:property value='#module.memo'/>">
			</td>
			<td>
...
function upName(input){
	...
	$.ajax({
	    ...
	    url: "${path}webservice/ServerMgrAction_updateModuleName.ms",
...
function upDec(input) {
		$.ajax({
		    ...
		    url: '${path}webservice/ServerMgrAction_updateModuleMemo.ms',
			success: function(data){
				input.hide();
				input.prev().show();
				input.prev().text(input.val());
...
function add(){
	...
	$(".jsAddInput").die().live("blur",function(e) {
		...
		$.ajax({
		   ...
		   url: '${path}webservice/ServerMgrAction_addModule.ms',
			success: function(data){
...
function delConfirm() {
	$.ajax({
		type : "POST",
		dataType : "json",
		url : '${path}webservice/ServerMgrAction_deleteModule.ms',
