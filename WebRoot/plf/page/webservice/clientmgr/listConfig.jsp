...
<div class="all">	
	<div>
	<form id="searchForm" name="searchForm" action="${path}webservice/ClientMgrAction_listConfig.ms" method="post">
	...
	<s:iterator value="page.dataList" id="config" status="index">
		<tr id="<s:property value="#config.id"/>">
			...
			<td style="white-space: nowrap;overflow:hidden;text-overflow: ellipsis;" title=<s:property value="#config.name"/>><s:property value="#config.name"/></td>
			<td><dict:viewDict dictCode="WS_CLIENT_TYPE" dictValue="%{#config.type}" /></td>
			<td style="white-space: nowrap;overflow:hidden;text-overflow: ellipsis;" title=<s:property value="#config.memo"/>><s:property value="#config.memo"/></td>
			<td>
				...
				<bu:funOper funcNo="F627" type="a" href="javascript:void(0);" onclick="reload('%{#config.id}');" title="%{getText('重新加载')}" prefix="&nbsp;" >
...
function add(){
	var url = "${path}webservice/ClientMgrAction_toAddConfig.ms?config.moduleId="+"<s:property value="config.moduleId"/>";
	showPopWin(url, 800, 450,null,"<dict:lang value="服务配置" /> <dict:lang value="新增" />");
}

function edit(id){
	var url = "${path}webservice/ClientMgrAction_toEditConfig.ms?config.id="+id;
...
function delConfirm() {
	$.ajax({
		type : "POST",
		dataType : "json",
		url : '${path}webservice/ClientMgrAction_deleteConfig.ms',
