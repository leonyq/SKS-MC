...
<html>
    ...
    <div  style="width: 100%;">
        ...
        <div  style="width: 60%;float: left;" >
				...
				<div class="mod">
					<div class="mod-hd">						
						<div class="optn">
							<button type="button" onclick="add();"><i class="ico ico-save"></i><dict:lang value="提交" /></button>
							<button type="button" onclick="closeTopPopWinFp('queryUserPagepop');"><i class="ico ico-cancel"></i><dict:lang value="取消" /></button>
						</div>
					</div>
				</div>
	...
	<form id="submit_form" name="submitForm" action="" target="submit_frame" method="post">
		...
		<script>
         ...
         if(piframeId=='')
         {
           parentWindows = $(window.parent.document).contents().find("#"+ciframeId)[0].contentWindow;
         
         }
		 else{ parentWindows = $(window.parent.document).contents().find("#"+piframeId).contents().find("#"+ciframeId)[0].contentWindow;
         }			
			var setting = {
				data: {
					simpleData: {
	...
	function add(thisObj){
		...
		var domId="<c:out value='${param.domId}' />";
		var domName="<c:out value='${param.domName}' />";
		if(domId!='')
		{
		  parentWindows.$("#"+domId).val(userId);
		}
		if(domName!='')
		{
		  parentWindows.$("#"+domName).val(userName);
		}
		else{
		  parentWindows.$("#createUser").val(userName);
		} 
		closeTopPopWinFp('queryUserPagepop');
