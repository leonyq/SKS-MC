...
<body>
     <div class="all" style='margin-bottom: 15px;'>
        ...
        <form id="add_form" name="add_form"  class="layui-form" action="${path}comm/WorkFlowAction_saveProcessde.ms" method="post" target="submitFrame" enctype="multipart/form-data">
         <input type="hidden" id="flowId" name="paraMap.flowId" value='${paraMap.flowId }' >
              <div style='float:right;padding:5px 0;'>
                 <c:if test="${paraMap.disable==0 }">	
                    <input type="button" value="<dict:lang value="复制" />" onclick="copyFlow();" class="layui-btn">
                 </c:if>
                    <input type="button" value="<dict:lang value="导出" />" onclick="exportFlow();" class="layui-btn">
                    <c:if test="${paraMap.disable==0 }">
                    <input type="button" value="<dict:lang value="停用" />" onclick="disableFlow(0);" class="layui-btn layui-btn-danger">
                    </c:if>	
                    <c:if test="${paraMap.disable==1 }">
                    <input type="button" value="<dict:lang value="启用" />" onclick="disableFlow(1);" class="layui-btn">
                    </c:if>	
                  <c:if test="${paraMap.disable==0 }">			
        	...
        	<table class="add_table">
				...
				<c:if test="${paraMap.from_name!=null && paraMap.from_name!=''}">
				<tr>
						<td  class="tr1 title">
						...
						<td class="widget">
							<input type="text" id="fromflowName" name="paraMap.fromflowName" disabled="disabled" value='${paraMap.from_name }'  >
						</td>
				</tr>
				</c:if>
				<tr>
...
function copyFlow(){
     ...
     var mcIframeName="<c:out value='${mcIframeName}' />";
      ...
      showPopWin(url, 800, 580,null,"<dict:lang value='复制' />"+"-"+"<dict:lang value='新增' />");
    }
