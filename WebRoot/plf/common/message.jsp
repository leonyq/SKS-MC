...
src="${JS_PATH }/common/msg/jquery.messager.js?_mc_res_version=<%=PlfStaticRes.JQUERY_MESSAGER_JS %>"></script>
	...
	function getHMsg(){
		...
		if(false == _isMothMakrControl){ 
			...
			var url = "${path}sys/LoginAction_logout.ms?_mc_logOutType=timeOutBp&__random_timeOutSeq="+new Date();
			if(!_isMotParkBackMark)
			   url = "${path}<%=CommMethodMc.getCurrFpLogOutUrl()%>?_mc_logOutType=timeOutBp"+new Date();  
			   
			$.post(url );
			util.alert("<dict:lang value='由于未操作时间太长，已超时。' />",function(_url){
			   _canClose();
	    ...
	    $.ajax({
			...
			success: function(data){
				...
				if(data.ajaxMap.msgDto == "-1"){
					...
					util.confirm(closeSelf,"<dict:lang value='已在其它窗口中打开主界面，是否关闭本窗口' />?",null);
					return;
