 ...
 * @param: rootId - id of virtual root node (same as tree node id attribute in
 *         xml)
 * @type: public
 * @topic: 0
 */
...
/**
 * @desc: replace IMG tag with background images - solve problem with IE image
 *        caching , not works for IE6 SP1
...
dhtmlXTreeObject.prototype.onRowSelect = function(e, htmlObject, mode) {
	...
	if (!mode) {
		...
		if (obj.actionHandler)
			...
			that.callEvent("onClick", [ obj.id, lastId, obj ]);// modify by
																// liudg
																// 20120110
	}
};
...
dhtmlXTreeObject.prototype.onCheckBoxClick = function(e) {
	...
	return this.treeNod.callEvent("onCheck", [ this.parentObject.id,
			this.parentObject.checkstate, this.parentObject ]);// modify by
																// liudg
																// 20120110
};
/**
...
dhtmlXTreeObject.prototype._createItem = function(acheck, itemObject, mode) {
	...
	var td1 = document.createElement('td');
	td1.className = "standartTreeImage2";
	...
	// inp.style.width="12px"; inp.style.height="12px";
	...
	td12.className = "standartTreeImage1";
 ...
 * @param: newPath - path to images directory (related to the page with tree or
 *         absolute http url)
 * @type: public
 * @topic: 0
 */
 ...
 * @eventdesc: Event occurs after right mouse button was clicked. Assigning this
 *             handler can disable default context menu, and incompattible with
 *             dhtmlXMenu integration.
...
/**
 * @desc: set function called when tree node clicked, also can be forced to call
 *        from API
 ...
 * @eventdesc: Event raises immideatly after text part of item in tree was
 *             clicked, but after default onClick functionality was processed.
 *             Richt mouse button click can be catched by onRightClick event
 *             handler.
...
/**
 * @desc: set function called when tree node selected or unselected, include any
 *        select change caused by any functionality
 * @param: func - event handling function
 * @type: deprecated
 * @topic: 0,7
 * @event: onSelect
 ...
 * @type: public
 * @param: filePath - name of script returning XML; in case of virtual loading -
 ...
 * @eventdesc: Event raises right before item in tree was checked/unchecked. can
 *             be canceled (return false from event handler)
 * @eventparam: ID of item which will be checked/unchecked
 * @eventparam: Current checkbox state. 1 - item checked, 0 - item unchecked.
 * @eventreturn: true - confirm changing checked state; false - deny chaning
 *               checked state;
 */
 ...
 * @eventdesc: Event raises immideatly after item in tree got command to
 *             open/close , and before item was opened//closed. Event also
 *             raised for unclosable nodes and nodes without open/close
 *             functionality - in that case result of function will be ignored.
 *             Event does not occur if node was opened by dhtmlXtree API.
 * @eventparam: ID of node which will be opened/closed
 * @eventparam: Current open state of tree item. 0 - item has not children, -1 -
 *              item closed, 1 - item opened.
 ...
 * @eventdesc: Event raises immideatly after item in tree was opened//closed.
 *             Event also raised for unclosable nodes and nodes without
 *             open/close functionality - in that case result of function will
 *             be ignored. Event not raised if node opened by dhtmlXtree API.
 * @eventparam: ID of node which will be opened/closed
 * @eventparam: Current open state of tree item. 0 - item has not children, -1 -
 *              item closed, 1 - item opened.
 */
 ...
 * @eventdesc: Event raised immideatly after item in tree was doubleclicked,
 *             before default onDblClick functionality was processed. Beware
 *             using both onClick and onDblClick events, because component can
 *             generate onClick event before onDblClick event while
 *             doubleclicking item in tree. ( that behavior depend on used
 *             browser )
...
dhtmlXTreeObject.prototype._moveNodeTo = function(itemObject, targetObject,
	...
	this.callEvent("onDrop",
			[ itemObject.id, targetObject.id,
...
/**
 * @desc: enable mode with fixed tables (looks better, but has no horisontal
 *        scrollbar)
 * @beforeInit: 1
 * @param: mode - 1 - on, 0 - off;
 * @type: private
 ...
 * @param: hidden - if set to true, checkboxes not rendered but can be shown by
 *         showItemCheckbox
 * @topic: 0
 */
	...
	if ((state < 0) || (((this.XMLsource) && (!item.XMLload)))) {
		...
		if (this.checkEvent("onOpenEnd")) {
			...
			if (!this.xmlstate || !this.XMLsource)
				this.callEvent("onOpenEnd",
						[ item.id, this._getOpenState(item) ]);
			else {
 ...
 * @param: itemId - id of node
 * @type: public
 * @return: number of child items for loaded branches; true - for not loaded
 *          branches
 * @topic: 4
 */
...
/**
 * @desc: refresh tree branch from xml (XML with child nodes rerequested from
 *        server)
 ...
 * @param: itemId - id of node
 * @param: image1 - node without children icon or closed node icon (if image2
 *         specified)
...
/**
 * @desc: Returns the list of all subitems Ids from the next level of tree,
 *        separated by commas.
 * @param: itemId - id of node
 * @type: public
 * @return: list of all subitems from the next level of tree, separated by
 *          commas.
 * @topic: 6
 */
...
/**
 * @desc: Returns the list of all sub items from all next levels of tree,
 *        separated by commas.
 * @param: itemId - id of node
 * @edition: Professional
 * @type: private
...
/**
 * @desc: Returns the list of all children items from all next levels of tree,
 *        separated by commas.
 * @param: itemId - id of node
 * @type: private
 * @edition: Professional
 ...
 * @type: public
 * @return: text of selected node (or list of all selected nodes text if more
 *          than one selected)
 * @topic: 1
 */
...
dhtmlXTreeObject.prototype._setCheck = function(sNode, state) {
	...
	this._setSrc(z, this.imPath
			+ ((sNode.parentObject._r_logic || this._frbtr) ? this.radioArray
 ...
 * @type: public
 * @param: itemId - node id
 * @param: selectParent - If true parent of deleted item get selection, else no
 *         selected items leaving in tree.
 * @topic: 2
 */
 ...
 * @eventparam: if node droped as sibling then contain id of item before whitch
 *              source node will be inserted
 ...
 * @type: public
 * @param: itemId - item Id
 * @param: mode - moving mode
 *         (left,up,down,item_child,item_sibling,item_sibling_next,up_strict,down_strict)
...
dhtmlXTreeObject.prototype._loadDynXML = function(id, src) {
	...
	if (typeof (this.getUserData(id, "autoLoadAction")) != 'undefined'
			&& null != this.getUserData(id, "autoLoadAction")) {// add by liudg
		_method = this.getUserData(id, "autoLoadAction");
	}
	if (typeof (this.getUserData(id, "MID")) != 'undefined'
			&& null != this.getUserData(id, "MID")) {// add by liudg
		_MID = this.getUserData(id, "MID");
	}
	this.loadXML(src + _method + getUrlSymbol(src) + "uid=" + sn + "&id="
			+ this._escape(id) + "&paraMap.MID=" + _MID);
};
...
dhtmlXTreeObject.prototype.disableCheckbox = function(itemId, mode) {
	...
	if (!sNode)
		return;
	sNode.dscheck = convertStringToBoolean(mode) ? (((sNode.checkstate || 0) % 3) + 3)
			: ((sNode.checkstate > 2) ? (sNode.checkstate - 3)
					: sNode.checkstate);
...
/**
 * @desc: enable active images (clickable and dragable). By default only text
 *        part of the node is active
 * @beforeInit: 1
 * @param: mode - 1 - on, 0 - off;
 * @type: public
 * @topic: 0
...
/**
 * @desc: Returns the list of all children from all next levels of tree,
 *        separated by default delimiter.
 * @param: itemId - id of node
 * @type: public
 * @return: list of all children items from all next levels of tree, separated
...
/**
 * @desc: Returns the list of all items which have child nodes, separated by
 *        default delimiter.
 * @type: public
 * @return: list of all items which has child nodes, separated by default
 *          delimiter.
 * @topic: 6
 */
 ...
 *        separated by default delimiter
 * @type: public
 * @return: list of ID of items with checked checkboxes, separated by default
 *          delimiter
 * @topic: 5
 */
...
/**
 * @desc: return list of identificators of nodes with unchecked checkboxes,
 *        separated by default delimiter
 * @type: public
 * @return: list of ID of items with unchecked checkboxes, separated by default
 *          delimiter
 * @topic: 5
 */
...
/**
 * @desc: return list of identificators of nodes with third state checkboxes,
 *        separated by default delimiter
 * @type: public
 * @return: list of ID of items with third state checkboxes, separated by
 *          default delimiter
 * @topic: 5
 */
...
/**
 * @desc: return list of identificators of nodes with checked and third state
 *        checkboxes, separated by default delimiter
 * @type: public
 * @return: list of ID of items with checked and third state checkboxes,
 *          separated by default delimiter
 * @topic: 5
 */
 ...
 * @param: list - initial identificators list (optional, used by private
 *         methods)
 * @topic: 5
 */
 ...
 * @add by liudg 20120110
 */
dhtmlXTreeObject.prototype.getAllCheckedTextBranches = function() {
	var temp = this._getAllCheckedText("", "", 1);
	if (temp != "")
		temp += this.dlmtr;
	return temp + this._getAllCheckedText("", "", 2);
}
...
dhtmlXTreeObject.prototype._getAllCheckedText = function(htmlNode, list, mode) {
	...
	if (htmlNode.checkstate == mode)
		if (!htmlNode.nocheckbox) {
			if (list)
				list += this.dlmtr + htmlNode.label;
			else
				list = htmlNode.label;
		}
	var j = htmlNode.childsCount;
...
dhtmlXTreeObject.prototype.dhx_Event = function() {
	...
	this.attachEvent = function(original, catcher, CallObj) {
		...
		return (original + ':' + this[original].addEvent(catcher)); // return ID
																	// (event
																	// name &
																	// event ID)
	}
