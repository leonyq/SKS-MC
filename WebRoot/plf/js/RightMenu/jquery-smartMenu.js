	...
	$.fn.smartMenu = function(data, options) {
		...
		var htmlCreateMenu = function(datum) {
			...
			if (!jqueryMenu.size()) {
				...
				$(document).on('mouseleave','.smart_menu_box',function(){
				   $.smartMenu.hide();
				}).on('contextmenu','.smart_menu_box',function(e){
				   return false;
				})
		...
		$(this).each(function() {
			this.oncontextmenu = function(e) {
				...
				if (jqueryMenu) {
					jqueryMenu.css({
						display: "block",
						left: e.clientX + params.offsetX-3,
						top: e.clientY + st + params.offsetY-3
					});
