...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    ,closeThisTabs: function(){
      ...
      $(TABS_HEADER).siblings('.layui-this').find('.layui-tab-close').trigger('click');
    }
  ...
  var events = admin.events = {
    ...
    ,serach: function(othis){
      othis.off('keypress').on('keypress',function(e){
        ...
        if(e.keyCode === 13){
          ...
          href = href + admin.escape(this.value);
    ...
    //输入框搜索(图标)
    ,iconSerach: function(objThis){
      var othis = $("#wordkey");
      var value = admin.escape(othis.val());
      if(!value.replace(/\s/g, '')) return;
      var href = othis.attr('lay-action')
      ,text = othis.attr('lay-text') || '搜索';

      href = href + value;
      text = text + ' <span style="color: #FF5722;">'+ value +'</span>';
      ...
      //如果搜索关键词已经打开，则刷新页面即可
      events.serach.keys || (events.serach.keys = {});
      events.serach.keys[admin.tabsPage.index] = value;
      if(value === events.serach.keys[admin.tabsPage.index]){
        events.refresh(othis);
      }
  ...
  element.on('nav(layadmin-system-side-menu)', function(elem){
	...
	//opentype 判断是否新增浏览器标签页 
	var opentype = elem.context.dataset.opentype;
	var outerHTML = elem.context.outerHTML;
	
	/*
	if(opentype == '1'){
		var arrSplit =outerHTML.split("lay-href");
		var openUrl = arrSplit[1].split("\"")[1].replace(/&amp;/,'&');
		var host = window.location.host;
		//var projectName = util.getContextPath();
		if(openUrl){
			window.open(openUrl);
			return false;
		}
	...
	*/
    ...
    var logo_height = 49; //logo的高度
    if(elem.siblings('.layui-nav-child')[0] && elem.parent().get(0).tagName.toLowerCase() == 'li'){
    	//当前菜单含有子菜单并且是第一级菜单
    	$(".layui-nav-tree > li.layui-nav-item").each(function(index,element){
        	if( $(this).is(".layui-nav-itemed") ){
        		//var _top = 56*index; //获取当前li距离第一个li的距离
        		//var top_height = $(this).position().top - logo_height;
        		var top_height = $(this).offset().top - logo_height;
        		//console.log(top_height);
        		
        		if(top_height<=0){
        			//var _top = 56*index;
        			//console.log( typeof(top_height) );
        			//console.log(top_height);
        			$(".layui-side-scroll").scrollTop($(this).position().top);
        			//console.log( $(".layui-side-scroll").scrollTop() );
        		}
        		
        		//如果是最后一个菜单 Li
        		var count_li = $(".layui-nav-tree > li.layui-nav-item").length;
        		if(count_li == (index+1)){
  ...
  $body.on('click', '*[lay-href]', function(){
    ...
    //监控页面tab新增 by tangws
    var openTypeFlag = $(this).attr('data-opentype'); //左侧菜单
    var shortcutMenu = $(this).find(".iconfont").attr('opentype')//首页 快捷方式
