...
<%
    String prefix = CommMethodMc.escapeJs(StringUtilsMc.toString(request.getAttribute("frontendHostUrl"))) + (String)request.getSession().getAttribute(Constants.SERVICE_PATH) + "/";
%>
<%
...
%><font color="red"><%=CommMethodMc.escapeJs(StringUtilsMc.toString(errornessservices.get(servicName))) %></font>
<%
    ...
    if (axisService != null) {
        ...
        String serviceName = CommMethodMc.escapeJs(axisService.getName());
%><h2><font color="blue"><a href="<%=prefix + serviceName%>?wsdl"><%=serviceName%></a></font></h2>
<font color="blue">Service EPR : </font><font color="black"><%=prefix + serviceName%></font><br>
<%
...
%>
<h4>Service Description : <font color="black"><%=CommMethodMc.escapeJs(serviceDescription)%></font></h4>
...
%><li><%=axisOperation.getName().getLocalPart()%></li>
    <%--                <br>Operation EPR : <%=prifix + serviceName.getLocalPart() + "/"+ axisOperation.getName().getLocalPart()%>--%>
    <%
        }
    %></ul>
<%
