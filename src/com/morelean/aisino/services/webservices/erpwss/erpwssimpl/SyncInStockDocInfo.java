...
import com.morelean.aisino.common.Method;
	 ...
	 * @Description: 入库单接口实现类
	 *  接收xml格式数据（入库物料信息清单），进行信息处理。当前处理策略是:  
	 *  1. 首先判断所有传入数据的入库单是否存在于已有表单中，同时查询该入库单状态。如果发现 WDI_STATUS（入库单状态） 不为1 （开立）
	 *     则进行返回 false：入库单号XXX正在处理中，不可进行操作。
	 *      ———— 操作方法：将入库单号统一存入 ，入库单 snList(java.util.List) 统一判断
	 *  2. 然后处理data_status 为 -1 的数据  进行物料删除操作，若删除后 单据，入库单信息下无物料信息则同时进行两者的删除工作
	 *   	  ———— 操作方法：存入delList 进行逐条判断删除
	 *  3. 进行数据添加，修改操作。分别对 （入库单表 T_WMS_RECEIVE_DOC   仓库单据信息表 T_WMS_DOC_INFO  料号详情表 T_WMS_RECEIVE_ITEM ）进行存入/修改 操作
	 *      ———— 操作方法：存入docList 遍历 List 对每条数据进行单独的插入，修改（效率低）
	 * @author:mmz
	 * @version:1.0
	 * @since 2018年4月10日上午1:19:30
	 * @Copyright: Copyright (c) 2018
	 * @Company: morelean
	 */
	public class SyncInStockDocInfo implements IWebService {
		...
		public Map<String, Object> excute(Object... objs) {
			...
			List<String> snList = new ArrayList<String>();// 存储每个物料节点的入库单号，用于入库单状态的判断（详见类说明）
			...
			for (int i = 0 ;i < dataList.size(); i++) {// 分别存入snList，delList，docList
				...
				for (Entry<String, Object> m : dataList.get(i).entrySet()) {
					...
					if ("WRD_DOC_NUM".equals(m.getKey())) {// 单号
						vMap.put("WRD_DOC_NUM", m.getValue());
					} else if ("WRD_DOC_TYPE".equals(m.getKey())) {// 入库类型
						vMap.put("WRD_DOC_TYPE", m.getValue());
					} else if ("WRD_SUP_CODE".equals(m.getKey())) {// 供应商代码
						vMap.put("WRD_SUP_CODE", m.getValue());
					} else if ("WRD_CUST_CODE".equals(m.getKey())) {// 客户代码
						vMap.put("WRD_CUST_CODE", m.getValue());
					} else if ("WRI_ITEM_SEQ".equals(m.getKey())) {// 项次
						vMap.put("WRI_ITEM_SEQ", m.getValue());
					} else if ("WRI_ITEM_CODE".equals(m.getKey())) {// 物料料号
						vMap.put("WRI_ITEM_CODE", m.getValue());
					} else if ("WRI_PLAN_NUM".equals(m.getKey())) {// 计划数量
						vMap.put("WRI_PLAN_NUM", m.getValue());
					} else if ("WRI_CONNECT_DOC".equals(m.getKey())) {// 关联单号
						vMap.put("WRI_CONNECT_DOC", m.getValue());
					} else if ("WRI_WH_CODE".equals(m.getKey())) {//仓库代码 
						vMap.put("WRI_WH_CODE", m.getValue());
					} else if ("WRI_MEMO".equals(m.getKey())) {// 物料备注
						vMap.put("WRI_MEMO", m.getValue());
					} else if ("WRD_DATE".equals(m.getKey())) {// 开单时间
						Date date = null;
						try {
						...
						vMap.put("WRD_CREATE_TIME", date);
					} else if ("WRD_DISPATCH_SN".equals(m.getKey())) {// 收发类型代码
						vMap.put("WRD_DISPATCH_SN", m.getValue());
					} else if ("DEPART_NO".equals(m.getKey())) {// 收发类型代码
					...
					// 必填项非空判断
					if (!vMap.containsKey("WRD_DOC_NUM") || "".equals(vMap.get("WRD_DOC_NUM"))) {
						resmap.put("MSG", "FALSE:入库单号为空");
						return resmap;
					}
					if (!vMap.containsKey("WRD_DOC_TYPE") || "".equals(vMap.get("WRD_DOC_TYPE"))) {
						resmap.put("MSG", "FALSE:入库单类型为空");
						return resmap;
					}
					if (!vMap.containsKey("WRI_ITEM_SEQ") || "".equals(vMap.get("WRI_ITEM_SEQ"))) {
						resmap.put("MSG", "FALSE:物料项次为空");
						return resmap;
					}
					if (!vMap.containsKey("WRI_ITEM_CODE") || "".equals(vMap.get("WRI_ITEM_CODE"))) {
						resmap.put("MSG", "FALSE:物料料号为空");
						return resmap;
					}
					if (!vMap.containsKey("WRI_PLAN_NUM") || "".equals(vMap.get("WRI_PLAN_NUM"))) {
						resmap.put("MSG", "FALSE:计划数量为空");
						return resmap;
					}
					if (!vMap.containsKey("WRI_CONNECT_DOC") || "".equals(vMap.get("WRI_CONNECT_DOC"))) {
						resmap.put("MSG", "FALSE:关联单号为空");
						return resmap;
					}
					if (!vMap.containsKey("WRI_WH_CODE") || "".equals(vMap.get("WRI_WH_CODE"))) {
						resmap.put("MSG", "FALSE:仓库代码为空");
						return resmap;
					}
			
			/*开始业务信息录入|需进行数据库操作项*/
			snList.add(vMap.get("WRD_DOC_NUM").toString());//加入入库单状态判断清单
			if (index == -1) {// 需删除的物料
				    delList.add(vMap);
				}else{//增|改物料
					  docList.add(vMap);
	 ...
	 try {
			...
			for(int j = 0 ; j< snList.size(); j++){
			 ...
			 String snListSql = " select WRD_DOC_NUM from T_WMS_RECEIVE_DOC where WRD_DOC_NUM  in (?) and WRD_STATUS !='1' ";
			 List<Map<String,Object>> statuslist194 = modelService.listDataSql(snListSql, new Object[]{arraySn});
			 	if(null!=statuslist194&&!"".equals(statuslist194)&&statuslist194.size()>0){
			 		...
			 		for(Map<String,Object> map196:statuslist194){
			 			 stb.append(map196.get("WRD_DOC_NUM")+" , ");
			 		}
			 		
			 		resmap.put("MSG", "FALSE:入库单（"+stb.toString()+")正在操作，或者已经关结");
			 		return resmap;
			 	}//入库单状态判断结束
		...
		private String delItem(ModelService modelService , TableDataMapExt tdMap , List<Map<String,Object>> list ,Map<String,Object> resmap) {
			...
			/*判断该物料是否存在于 物料总明细单(T_WMS_RECEIVE_DETAIL)中。如果有，@return  FALSE:不能删除有入库总明细的物料*/
			//获得物料id 数组
			String searchSql = " SELECT ID FROM T_WMS_RECEIVE_ITEM WHERE wri_item_code  = ? AND wri_item_seq   = ? AND wri_doc_num  = ? ";
			...
			for(int i = 0 ; i<list.size() ; i++){
				interimList = modelService.listDataSql(searchSql
						, new Object[]{list.get(i).get("WRI_ITEM_CODE"),list.get(i).get("WRI_ITEM_SEQ"),list.get(i).get("WRD_DOC_NUM")});
				if (null != interimList && interimList.size() > 0) {// 存在
					arl.add(interimList.get(0).get("ID").toString());
				}else{
					return "FALSE:入库单为："+list.get(i).get("WRD_DOC_NUM")+",项次为:"+list.get(i).get("WRI_ITEM_SEQ")+",物料料号为："+list.get(i).get("WRI_ITEM_CODE")
							+"的物料不存在物料信息表中，无法删除。";
				}
			}//物料ID获得完毕
			
			//判断删除的id有没有明细
			String delsql = "SELECT t.wrd_item_code FROM T_WMS_RECEIVE_DETAIL t WHERE t.WRD_ITEM_ID = ?";
			...
			//删除物料信息
			tdMap.setTableName("T_WMS_RECEIVE_ITEM");
			for(int i=0;i<arl.size();i++){
		...
		private String changeItem(ModelService modelService , TableDataMapExt tdMap , List<Map<String,Object>> list ,Map<String,Object> resmap) {
			...
			for (Map<String, Object> vmap : list) {
					...
					String searchdocsql = "select id from T_WMS_RECEIVE_DOC where WRD_DOC_NUM = ? ";
					List<Map<String,Object>> searchdocList = modelService.listDataSql(searchdocsql, new Object[]{vmap.get("WRD_DOC_NUM").toString()});
					...
					if (inx) {// 添加
						...
						tdMap.setTableName("T_WMS_RECEIVE_DOC");
						validmap.put("ID", StringUtils.getUUID());// 
						validmap.put("WRD_DOC_NUM", vmap.get("WRD_DOC_NUM"));//入库单号
						validmap.put("WRD_DOC_TYPE", vmap.get("WRD_DOC_TYPE"));//单号类型
						if(vmap.containsKey("WRD_SUP_CODE"))//供应商
						 validmap.put("WRD_SUP_CODE", vmap.get("WRD_SUP_CODE"));
						if(vmap.containsKey("WRD_CUST_CODE"))//客户
						 validmap.put("WRD_CUST_CODE", vmap.get("WRD_CUST_CODE"));
						validmap.put("WRD_STATUS", "1");//状态 1 开立  2，操作中 3 关结
						if(vmap.containsKey("WPD_DELIVERY_DATE"))//入库日期  注意是WPD 可能是数据库建立时错误
							validmap.put("WPD_DELIVERY_DATE", vmap.get("WPD_DELIVERY_DATE"));
						if(vmap.containsKey("WRD_CREATE_TIME"))//开单时间
							validmap.put("WRD_CREATE_TIME", vmap.get("WRD_CREATE_TIME"));
						if(vmap.containsKey("DATA_AUTH")){//组织机构代码
							validmap.put("DATA_AUTH", vmap.get("DATA_AUTH"));
							validmap.put("DEPT_ID", vmap.get("DATA_AUTH"));
						}
						if(vmap.containsKey("WRD_DISPATCH_SN"))//收发标志
							validmap.put("WRD_DISPATCH_SN", vmap.get("WRD_DISPATCH_SN"));
						if(vmap.containsKey("WRD_URGENT_FLAG"))//加急标志
							validmap.put("WRD_URGENT_FLAG", vmap.get("WRD_URGENT_FLAG"));
						...
						validmap.put("ID", StringUtils.getUUID());
					 	validmap.put("WDI_DOC_NUM", vmap.get("WRD_DOC_NUM"));//单据号
					 	validmap.put("WDI_STATUS", "1");/*1*/
						if(vmap.containsKey("WRD_URGENT_FLAG"))//加急标志
							validmap.put("WDI_URGENT_FLAG", vmap.get("WRD_URGENT_FLAG"));//加紧标识   
						validmap.put("T_WMS_DOC_INFO", "N");/*需加T_WMS_DOC_INFO---ERP同步标识 N*/
						if(vmap.containsKey("WRD_CUST_CODE"))//客户
							validmap.put("WDI_CUST_CODE", vmap.get("WRD_CUST_CODE"));
					 	validmap.put("WDI_DOC_TYPE", vmap.get("WRD_DOC_TYPE"));//单号类型
						if(vmap.containsKey("WRD_DISPATCH_SN"))//收发类型
							validmap.put("WDI_DISPATCH_TYPE", vmap.get("WRD_DISPATCH_SN"));
						if(vmap.containsKey("WDI_CREATE_TIME"))//开单时间
							validmap.put("WDI_CREATE_TIME", vmap.get("WRD_CREATE_TIME"));
						if(vmap.containsKey("DATA_AUTH"))//组织机构代码
						{
							validmap.put("DATA_AUTH", vmap.get("DATA_AUTH"));
							validmap.put("DEPT_ID", vmap.get("DATA_AUTH"));
						...
						tdMap.setTableName("T_WMS_RECEIVE_ITEM");
					 	validmap.put("ID", StringUtils.getUUID());
					 	validmap.put("WRI_DOC_NUM",vmap.get("WRD_DOC_NUM"));//单据号
					 	validmap.put("WRI_STATUS", "1");/**/
				 		validmap.put("WRI_ITEM_CODE", vmap.get("WRI_ITEM_CODE"));
				 		validmap.put("WRI_ITEM_SEQ", vmap.get("WRI_ITEM_SEQ"));/**/
				 		validmap.put("WRI_PLAN_NUM", vmap.get("WRI_PLAN_NUM"));/**/
						if(vmap.containsKey("DATA_AUTH"))//组织机构代码
						{
							validmap.put("DATA_AUTH", vmap.get("DATA_AUTH"));
							validmap.put("DEPT_ID", vmap.get("DATA_AUTH"));
						}
				 		//validmap.put("WRI_PACK_NUM", packNums[i]);/**/
				 		//validmap.put("WRI_GIFT_NUM", giftNums[i]);/**/
				 		validmap.put("WRI_CONNECT_DOC", vmap.get("WRI_CONNECT_DOC"));/**/
				 		validmap.put("WRI_WH_CODE", vmap.get("WRI_WH_CODE"));/**/
				 		if(vmap.containsKey("WRI_MEMO"))
				 			validmap.put("WRI_MEMO",vmap.get("WRI_MEMO"));
						tdMap.setColMap(validmap);
						tdMap.getColMap().put("EDIT_TIME", DateUtil.getCurDate());
						tdMap.getColMap().put("CREATE_TIME", DateUtil.getCurDate());
						modelService.save(tdMap);
					...
					//更新入库单
						...
						if(vmap.containsKey("DATA_AUTH"))//组织机构代码
						{
					 	...
					 	tdMap.setColMap(validmap);
						tdMap.getColMap().put("EDIT_TIME", DateUtil.getCurDate());
						tdMap.setSqlWhere("  AND WDI_DOC_NUM = '"+vmap.get("WRD_DOC_NUM")+"' ");//单据号
					 	modelService.edit(tdMap);
					...
					//判断物料信息是否存在
						...
						searchlist = modelService.listDataSql(searchSql
								, new Object[]{vmap.get("WRI_ITEM_CODE"),vmap.get("WRI_ITEM_SEQ"),vmap.get("WRD_DOC_NUM")});
						...
						}else{//修改物料  //当前未接收 入库数量 未进行入库数量和计划数量比较  
		...
		/** 
		 * @Description:  判断计划数量是否大于入库数量
		 * @param modelService
		 * @param itemIds  修改的物料ID 数组
		 * @param planNums 物料ID对应的 计划数量数组
		 * @return Boolean  true 可更改   false  不可更改  
		 * @author: mmz
		 * @Company:morelean     
		 */ 
		public  boolean invalidPlanCtn( ModelService modelService , String[] itemIds ,String[] planNums){
			...
			for(int i=0;i<itemIds.length;i++){
				if(!itemIds[i].equals("")){
					...
					if(pList.get(0).get("WRI_RECEIVE_NUM")!=null){
						if(Integer.parseInt(planNums[i])<Integer.parseInt(pList.get(0).get("WRI_RECEIVE_NUM").toString())){
							return false;
						}
					}
				}
