...
/**
 * @Title 账户别名清单获取
 * @description 获取账户别名清单
 * @ClassName GetOhterInfo
 * @author lyc
...
public class GetOtherInfo implements IWebService{
...
public Map<String, Object> excute(Object... objs) {
      ...
      String errorMsg = "";
  try {
    //获取T_WMS_OTHER_INOUT_NAME
    String sql="select * from T_WMS_OTHER_INOUT_NAME";
    List List = modelService.listDataSql(sql);
    if (List.size()==0)
    {
      errorMsg=CommMethod.getText("未获取到账户别名信息");
      map.put("isSuccess", "N");
      map.put("errorMsg", errorMsg);
      return map;
    }
    map.put("isSuccess", "Y");
    map.put("T_WMS_OTHER_INOUT_NAME", List);
    map.put("errorMsg", errorMsg);
  } catch (Exception e) {
    log.error(e);
    errorMsg=CommMethod.getText("获取账户别名信息异常");
    map.put("isSuccess", "N");
