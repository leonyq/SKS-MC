 ...
 * @author development
 */
public class SaveConfigureInfo2 implements FuncService
{
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    try
    {
      ...
      String sql = "SELECT T.ID FROM T_CO_SN_CONFIGURE T WHERE T.CCC_CONFIGURE_CODE=?";
      List<String> list = modelService.listDataSql(sql, new Object[] {conCode});
      if (list != null && !list.isEmpty())
      {
        return modelAction.alertParentInfo(modelAction.getText("配置编码已存在，请更换"));
      }
      sql = "SELECT T.ID FROM T_CO_SN_CONFIGURE T WHERE T.CCC_CONFIGURE_NAME=?";
      list = modelService.listDataSql(sql, new Object[] {conName});
      if (list != null && !list.isEmpty())
      {
      ...
      {
        sql = "SELECT T.ID FROM T_CO_SN_CONFIGURE T WHERE T.CCC_CONFIGURE_TYPE=? AND T.CCC_DEFAULT_FLAG='Y'";
        list = modelService.listDataSql(sql, new Object[] {conType});
        if (list != null && !list.isEmpty())
        {
