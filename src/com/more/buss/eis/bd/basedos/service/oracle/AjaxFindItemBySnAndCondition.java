...
public class AjaxFindItemBySnAndCondition implements FuncService {
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
        ...
        String querySql = "SELECT rownum,T1.ID,T1.CIG_SN,T1.CIG_NAME,T1.CIG_TYPE,T1.DATA_AUTH,SD.NAME AS DATA_AUTH2 " +
        		...
        		if(StringUtils.isNotBlank(dataAuth)){
        			querySql += " AND T1.DATA_AUTH = :DATA_AUTH AND T2.DATA_AUTH = :DATA_AUTH AND T3.DATA_AUTH = :DATA_AUTH  ";	
        			map.put("DATA_AUTH", dataAuth);
        		}else {
        			querySql+=" and T2.DATA_AUTH=T3.DATA_AUTH AND T3.DATA_AUTH in (select DEPT_ID from sy_data_auth where user_id = :user_id)  ";
