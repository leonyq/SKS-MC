package com.more.buss.eis.bd.route.service.pg;
...
public class SaveRouteLsSereice implements FuncService {
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
		...
		String data_auth = String.valueOf(modelAction.getSession().getAttribute("mcDataAuth"));
		...
		try {
			...
			int count = 0;
			String dataId = null;
			String sql = "SELECT CR_ROUTE_ID AS COUNT FROM T_CO_ROUTE T WHERE T.CR_ROUTE_ID= ? and data_auth= ? ";
			if (flag.equals("edit")) {
				dataId = hsr.getParameter("dataId");
				crRouteId = hsr.getParameter("routeCode");
				sql = sql + " and t.id != ?";
				count = modelService.countSql(sql, new Object[] { crRouteId, dataId });
			} else {
				count = modelService.countSql(sql, new Object[] { crRouteId, data_auth });
			}
			...
			if (StringUtils.isNotBlank(dataInfo)) {
				...
				if (flag.equals("edit")) {
					...
					routeCountExt.setSqlWhere(" and CR_ROUTE_ID = ? ");
					...
					delrouteControlExt.setSqlWhere(" and CR_ROUTE_ID = ? ");
