...
public class EditListCalendarSereiceSee implements FuncService {
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService) {
    ...
    String dataAuth = map.get("DATA_AUTH").toString();
    ...
    String sqlWork = "select t.ID,to_char(t.CALW_START,'HH24:mi') CALW_START,"
        ...
        + "from T_CO_CALENDAR_WORKTIME t where CAL_ID = ? AND DATA_AUTH =? order by t.CALW_START";
    ...
    String sqlRest = "select tr.ID,to_char(tr.CALR_START,'HH24:mi') CALR_START,to_char(tr.CALR_END,'HH24:mi') CALR_END, "
        ...
        + "from (select tw.ID ID,tw.CALW_GROUP CALW_GROUP,tw.CALW_TIME CALW_TIME,tw.DATA_AUTH "
        + "from T_CO_CALENDAR_WORKTIME tw where tw.CAL_ID = ? and tw.DATA_AUTH = ? order by tw.CALW_START)  w  "
        + "where rownum = 1) tws where tr.CALW_ID = tws.ID and tr.data_auth = ?  and tws.data_auth = ?   order by tr.CALR_START";
    restList = modelService.listDataSql(sqlRest, new Object[] { dataId,dataAuth,dataAuth,dataAuth });
    modelAction.setDataList0(restList);

    String sqlArea = "select t.CA_ID as areaid,t.CA_NAME as areaname from t_co_area t where CA_ID in (select area_id from t_co_calendar_line where cal_id=? and data_auth=? ) and data_auth=?  ";
