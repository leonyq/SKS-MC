 ...
 * @Company: morelean
 */
public class EditListCalendarSereice implements FuncService {
  /**
   * @param modelAction
   *          modelAction
   * @param modelService
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService) {
    ...
    String sqlWork = "select t.ID,to_char(t.CALW_START,'HH24:mi') CALW_START,"
        ...
        + "from T_CO_CALENDAR_WORKTIME t where CAL_ID = ? order by t.CALW_START";
    workList = modelService.listDataSql(sqlWork, new Object[] { dataId });
    ...
    String sqlRest = "select tr.ID,to_char(tr.CALR_START,'HH24:mi') CALR_START,to_char(tr.CALR_END,'HH24:mi') CALR_END, "
        ...
        + "from T_CO_CALENDAR_WORKTIME tw where tw.CAL_ID = ? order by tw.CALW_START) "
        + "where rownum = 1) tws where tr.CALW_ID = tws.ID order by tr.CALR_START";
    restList = modelService.listDataSql(sqlRest, new Object[] { dataId });
    modelAction.setDataList0(restList);
    
    String sqlArea = "select t.CA_ID as areaid,t.CA_NAME as areaname from t_co_area t where CA_ID in (select area_id from t_co_calendar_line where cal_id=?)";
