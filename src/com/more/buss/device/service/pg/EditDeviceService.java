 ...
 *
 */
public class EditDeviceService implements FuncService {
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
	    ...
	    String sql = "select row_number() over() as ROWNUM,A.cdl_table,A.cdl_loadpoint,A.cdl_memo,A.cdl_track, T.value AS dict from T_CO_DEVICE_LOADPOINT A "+
	                  "LEFT JOIN SY_DICT_VAL T  ON T.CODE= A.CDL_TRACK  AND  T.DICT_CODE=? "+
	                  " where A.cd_device_id = ? AND A.DATA_AUTH = ? order by A.cdl_table, A.CDL_LOADPOINT ";
	    String countSql = "SELECT COUNT(1) NUM from T_CO_DEVICE_LOADPOINT t WHERE cd_device_id = ? AND DATA_AUTH = ? ";
	    loadPointList = modelService.listDataSql(sql, new Object[] {"TRACK_TYPE", dataId,dataAuth });
	    map = modelService.queryForMap(countSql, new Object[] { dataId,dataAuth });
