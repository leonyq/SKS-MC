 ...
 * @Company: morelean
 */
public class EditPlanDetailSave1 implements FuncService{
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
	    ...
	    try {
			//获取设备保养计划信息
	    	String planType = request.getParameter("paraMapObj.T3#CDP_TYPE");//计划类型
	    	String planSn = request.getParameter("paraMapObj.CDP_PLAN_SN");//计划代码
	    	String planName = request.getParameter("paraMapObj.T3#CDP_PLAN_NAME");//计划名称
	    	String planCycle = request.getParameter("paraMapObj.T3#CDP_CYCLE");//保养周期
	    	String planSpace = request.getParameter("CDP_SPACE");//保养间隔
	    	String productTimes = request.getParameter("CDP_PRODUCT_TIMES");//生产次数
	    	String memo = request.getParameter("paraMapObj.T3#CDP_MEMO");//备注
	    	String id = request.getParameter("dataId");
	    	String sql = "select t.CDP_PLAN_SN from T_CO_DEVICE_PLAN_INFO t where t.CDP_PLAN_SN=?  and t.id != ?";
	        int count = modelService.execSql(sql, new Object[] {planSn,id});
	        if (count != 0)
	        {
