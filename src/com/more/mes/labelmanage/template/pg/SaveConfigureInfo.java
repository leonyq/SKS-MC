  ...
  {//
    ...
    try
    {
          ...
          if (!"".equals(displayWays[i]))
          {
            ...
            if ("1".equals(displayWays[i]))
            {
              ...
              for (int j = 0; j < itemInfoStrs.length; j++ )
              {
                ...
                cldItemMap.getColMap().put("CLD_COLUMN_WIDTH", Integer.parseInt(colWidth));// 列宽(PX)
            ...
            else if ("2".equals(displayWays[i]))
            {
              ...
              cldItemMap.getColMap().put("CLD_DISPLAY_WAY", displayWays[i]);// 展示方式（1-列表显示，2-数据排列）
              cldItemMap.getColMap().put("CLD_COLUMN_NAME", colName);// 数据项列名
              cldItemMap.getColMap().put("CLD_ROW", Integer.parseInt(colRow));// 行数
              cldItemMap.getColMap().put("CLD_COLUMN", Integer.parseInt(colNum));// 列数
              modelService.save(cldItemMap);
            }
          }
        }
      ...
      else
      {
        ...
        // cldMap2.setSqlWhere(" and CLD_LABEL_ID = ? ");
        // paraList.add(infoId);
        // cldMap2.setSqlWhereArgs(paraList);
        //
      ...
      cldMap3.setSqlWhere(" and ID=? ");
      paraList2.add(infoId);
      cldMap3.setSqlWhereArgs(paraList2);
      if (dataAuth != null)
      {
