...
package com.more.mes.aps.change.pg;
...
public class ChangeRate implements FuncService {
	private static final Log log = LogFactory.getLog("com.more.mes.aps.change.pg.ChangeRate");
	...
	private List getPlanChangeRate(HttpServletRequest httpServletRequest, ModelService modelService,
		...
		try {
			...
			String sqlStr = "SELECT \r\n" + "D.NAME DATA_AUTH, \r\n" + "A.PROJECT_ID, \r\n"
					+ "COALESCE(A.PRODUCT_MATERIAL,' ') AS PRODUCT_MATERIAL, \r\n"
					...
					+ "A.PROJECT_STATUS, \r\n"
					+ "COALESCE(TO_CHAR(A.PROLEPSIS_START_DATE,'YYYY-MM-DD HH24:MI:SS'),' ') AS PROLEPSIS_START_DATE, \r\n"
					+ "COALESCE(TO_CHAR(A.PROLEPSIS_END_DATE,'YYYY-MM-DD HH24:MI:SS'),' ') AS PROLEPSIS_END_DATE, \r\n"
					+ "COALESCE(TO_CHAR(A.TPPB_PLAN_DELIVERY_DATE,'YYYY-MM-DD HH24:MI:SS'),' ') AS TPPB_PLAN_DELIVERY_DATE, \r\n"
					+ "COALESCE(A.PM_CHANGE_TIMES,0) AS PM_CHANGE_TIMES, \r\n" + "C.VALUE AS PROJECT_STATUS_TEXT \r\n"
					+ "FROM T_PM_PROJECT_BASE A \r\n"
					+ "LEFT JOIN T_CO_ITEM B ON A.PRODUCT_MATERIAL=B.CI_ITEM_CODE \r\n"
					+ "LEFT JOIN SY_DICT_VAL C ON C.DICT_CODE='PROJECT_STATUS' AND C.CODE=A.PROJECT_STATUS \r\n"
					+ "LEFT JOIN SY_DEPT D ON A.DATA_AUTH=D.ID\r\n"
					+ "WHERE 1=1 AND COALESCE(A.PM_CHANGE_TIMES,0)>0  %s";
