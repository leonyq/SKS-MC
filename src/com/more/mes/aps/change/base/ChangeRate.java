...
package com.more.mes.aps.change.base;
...
{
	private static final Log log = LogFactory.getLog("com.more.mes.aps.change.base.ChangeRate");
	 ...
	 * @return
	 */
	private List getPlanChangeRate(HttpServletRequest httpServletRequest,ModelService modelService,PaginationImpl impl){
		try{
			//获取查询条件
        ...
        paramTypeVar.add(Types.VARCHAR);
			...
			String sqlStr="SELECT\r\n" + 
		  	  "A.DATA_AUTH, \r\n"+
					...
					"WHERE 1=1  "
					+ "AND NVL(A.PM_CHANGE_TIMES,0)>0  %s";
			...
			return modelService.listSql(
					String.format(sqlStr, strWhere), impl,null, paramObject.toArray(), paramType, "ORDER BY PROLEPSIS_START_DATE DESC", null);
		}
		catch(Exception ex){
