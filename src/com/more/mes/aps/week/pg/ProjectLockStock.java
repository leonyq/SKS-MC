...
package com.more.mes.aps.week.pg;
	...
	private List query(HttpServletRequest httpServletRequest,PaginationImpl impl,ModelService modelService){
		try{
			...
			String sqlStr="SELECT \r\n" + 
					...
					"A.PROJECT_ID,  \r\n" + 
					"B.CBD_ITEM_CODE,  \r\n" + 
					"COALESCE(C.CI_ITEM_NAME,' ') AS CI_ITEM_NAME,  \r\n" + 
					"COALESCE(C.CI_ITEM_SPEC,' ') AS CI_ITEM_SPEC,  \r\n" + 
					"COALESCE(B.CBD_ITEM_NUM,0) AS CBD_ITEM_NUM,  \r\n" + 
					"(COALESCE(A.PRODUCT_COUNT,0)-COALESCE(A.FINISH_COUNT,0))*COALESCE(B.CBD_ITEM_NUM,0) AS TOTAL_NEED_NUM,  \r\n" + 
					"COALESCE(STOCK.WSI_ITEM_NUM,0) AS STOCK_NUM,  \r\n" + 
					"COALESCE(STOCK.WSI_ITEM_NUM,0)-COALESCE(LOCK_INFO.WLD_LOCK_NUM,0) AS CAN_LOCK, \r\n" + 
					"CASE WHEN COALESCE(B.CBD_ITEM_NUM,0)=0  \r\n" + 
					"	THEN 0  \r\n" + 
					"		ELSE  \r\n" + 
					"		 CASE WHEN ROUND((COALESCE(STOCK.WSI_ITEM_NUM,0)-COALESCE(LOCK_INFO.WLD_LOCK_NUM,0))/COALESCE(B.CBD_ITEM_NUM,0),0)> (COALESCE(A.PRODUCT_COUNT,0)-COALESCE(A.FINISH_COUNT,0)) THEN (COALESCE(A.PRODUCT_COUNT,0)-COALESCE(A.FINISH_COUNT,0)) \r\n" + 
					"			 ELSE ROUND((COALESCE(STOCK.WSI_ITEM_NUM,0)-COALESCE(LOCK_INFO.WLD_LOCK_NUM,0))/COALESCE(B.CBD_ITEM_NUM,0),0) END \r\n" + 
					"		 END KITTING, \r\n" + 
					"CASE WHEN ((COALESCE(A.PRODUCT_COUNT,0)-COALESCE(A.FINISH_COUNT,0))*COALESCE(B.CBD_ITEM_NUM,0))>(COALESCE(STOCK.WSI_ITEM_NUM,0)-COALESCE(LOCK_INFO.WLD_LOCK_NUM,0)) \r\n" + 
					"	THEN '2' \r\n" + 
					...
					"SELECT  \r\n" + 
					"A.WSI_ITEM_CODE,  \r\n" + 
					"COALESCE(SUM(A.WSI_ITEM_NUM),0) AS WSI_ITEM_NUM  \r\n" + 
					...
					"SELECT  \r\n" + 
					"A.WLD_ITEM_CODE,  \r\n" + 
					"COALESCE(SUM(A.WLD_LOCK_NUM),0) AS WLD_LOCK_NUM  \r\n" + 
