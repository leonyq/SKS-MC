 ...
 *
 */
public class WbOtherInstoreServiceJob implements IJobx {
	...
	public String result(String docnum,String LOGIN_NAME){
		String result = "";
	...
	private String deal(List<Map<String, String>> list,Map<String,Map<String,String>> item
		...
		for(int j=0;j<docsize;j++){
			...
			jsonMap.put("crdcode", DocTemp.get("CRDCODE"));//入库类别
			...
			String MethodName="U8OtherEntry";
			
			MsHTranMan tran=  BussService.getHbTran();
			try{
				Map<String, String> map=WsServiceUtil.postServicelOGIN_NAME(MethodName, json, "其他入库单回写", 38,DOCNUM,LOGIN_NAME);
				...
				if(map.get("errcode").equals(WsServiceUtil.RESULTCODE_SUCCESS)){
					...
					result.append("其他入库单：" + DOCNUM + "、仓库:" + DocTemp.get("CWHCODE") + "回写失败,原因："+map.get("errmsg")+";");
				}
				tran.commit();
			}catch(Exception e){
				tran.rollback();
	...
	public List<Map<String, String>> getList(String WRD_DOC_NUM){
		String sql="SELECT T2.WRD_DOC_NUM CODE,TO_CHAR(NVL(T.WRD_HANDOVER_TIME, T.WRD_RECEIVE_TIME),'YYYY-MM-DD') ddate,T8.WA_ERP_CODE cwhcode,T2.WRD_ERP_OTHER_SN crdcode, "+
               "t2.wrd_memo cmemo,T3.ERP_CODE cacc,t2.wrd_memo cmemo,t2.wrd_sup_code cvencode, "+
               "T5.WRI_ITEM_CODE cinvcode,T5.WRI_ITEM_SEQ, T2.WRD_ERP_DEPT CDEPCODE,"+
               "TO_CHAR(T.WRD_SPLIT_NUM) iquantity,T.ID,T5.ID ITEM_ID,T2.WRD_DOC_NUM  "+
               ...
               "WHERE T2.WRD_DOC_TYPE='DJ06' AND T.WRD_HANDOVER_EMP IS NOT NULL AND T.WRD_UPLOAD_FLAG='N' "+
               "AND T7.WDO_UPLOAD_FLAG='Y' AND T2.WRD_ERP_OTHER_SN IS NOT NULL "+
               "AND T2.WRD_ERP_DEPT IS NOT NULL ";
