package com.more.mes.terminal.service.pg;
...
public class TerminalService {
   ...
   static public List GetUserInfo(String workStation, String userCode)
   {
	   try
	   {
		   String strSql="SELECT\r\n" + 
		   		...
		   		"      )\r\n" + 
		   		"      )T ON T.USER_ID=A.ID\r\n" + 
		   		"      WHERE LOWER(A.LOGIN_NAME)=LOWER(?) LIMIT 1";
   ...
   static public List GetUserJob(String workStation,String userCode)
   {
	   ...
	   {
		   String strSql="SELECT\r\n" + 
		   		"      STRING_AGG(T.JOB_NAME,',') AS JOB_NAME\r\n" + 
		   		"      FROM SY_USER A\r\n" + 
		   		"      LEFT JOIN (\r\n" + 
		   		"      SELECT\r\n" + 
		   		"      A.USER_ID,\r\n" + 
   ...
   static public List GetMoInfo(String moNumber,String dataAuth)
   {
       ...
       {
    	   String strSql="SELECT \r\n" + 
    	   		"COALESCE(B.CI_ITEM_CODE,'') CI_ITEM_CODE, \r\n" + 
    	   		"COALESCE(B.CI_ITEM_NAME,'') CI_ITEM_NAME, \r\n" + 
    	   		"COALESCE(B.CI_ITEM_SPEC,'') CI_ITEM_SPEC, \r\n" + 
    	   		"COALESCE(A.PM_TARGET_QTY,0) PM_TARGET_QTY, \r\n" + 
    	   		"COALESCE(A.PM_FINISH_COUNT,0) PM_FINISH_COUNT \r\n" + 
    	   		"FROM T_PM_MO_BASE A  \r\n" + 
	   ...
	   try
	   {
		 //获取工序
		   String strSql="SELECT  \r\n" + 
	    	...
	    	if(null==moNumber){
	    		strSql="SELECT  \r\n" + 
		    			"COALESCE(SUM(A.MGS_OK_PCS),0)+COALESCE(SUM(A.MGS_T_OK_PCS),0) OK_PCS,  \r\n" + 
		    			"COALESCE(SUM(A.MGS_NG_PCS),0) NG_PCS  \r\n" + 
		    			"FROM T_MID_GROUP_STATISTIC A  \r\n" + 
		    			"WHERE A.MGS_WORK_DATE=TO_CHAR(NOW(),'YYYY-MM-DD')  \r\n" + 
   ...
   static public List GetLanguageType()
   {
	   ...
	   {
		   String strSql="SELECT\r\n" + 
		   		"COLUMN_NAME AS LANGUAGE_TYPE\r\n" + 
		   		"FROM INFORMATION_SCHEMA.COLUMNS\r\n" + 
		   		"WHERE UPPER(TABLE_NAME)='T_SYS_LANGUAGE'\r\n" + 
		   		"AND UPPER(COLUMN_NAME) NOT IN('MSGNAME','CREATE_USER','CREATE_DATE','UPDATE_USER','UPDATE_DATE')";
		   return modelService.listDataSql(strSql);
	   }
	   catch(Exception ex)
	   {
   ...
   static public List GetTop5Error(String workStation,String dataAuth)
   {
	   try
	   {
		   ...
		   strSql="SELECT  \r\n" + 
		   		...
		   		"A.WE_ERROR_CODE,   \r\n" + 
		   		"B.CEC_NAME,  \r\n" + 
		   		"COALESCE(SUM(A.WE_ERROR_NUM),0) WE_ERROR_NUM  \r\n" + 
		   		"FROM T_WIP_ERROR A  \r\n" + 
		   		...
		   		"AND TO_CHAR(A.WE_TEST_TIME,'YYYY-MM-DD')=TO_CHAR(NOW(),'YYYY-MM-DD')  \r\n" + 
		   		"AND A.DATA_AUTH=?\r\n" + 
		   		"GROUP BY A.WE_ERROR_CODE,B.CEC_NAME  \r\n" + 
		   		"ORDER BY COALESCE(SUM(A.WE_ERROR_NUM),0) DESC  \r\n" + 
		   		")T LIMIT 6";
		    return modelService.listDataSql(strSql,new Object[]{_groupCode,_line,dataAuth});
	   }
	   catch(Exception ex)
   ...
   static public List GetTimeOutPut(String workStation,String dataAuth)
   {
       try
       {
    	   ...
    	   strSql="SELECT \r\n" + 
    	   		...
    	   		"COALESCE(SUM(COALESCE(A.MGS_OK_PCS,0)+COALESCE(A.MGS_T_OK_PCS,0)),0) PVALUE   \r\n" + 
    	   		"FROM T_MID_GROUP_STATISTIC A   \r\n" + 
    	   		"WHERE A.MGS_GROUP_CODE=?  \r\n" + 
    	   		"AND A.MGS_AREA_SN=?   \r\n" + 
    	   		"AND A.MGS_WORK_DATE=TO_CHAR(NOW(),'YYYY-MM-DD')   \r\n" + 
    	   		"AND A.DATA_AUTH=?\r\n" + 
   ...
   static public List GetMoAndEmpByWorkstation(String workStation,String dataAuth)
   {
       try
       {
    	   ...
    	   strSql="SELECT  \r\n" + 
    	   		...
    	   		"INNER JOIN T_CO_CALENDAR_WORKTIME B ON A.CAL_ID=B.CAL_ID  \r\n" + 
    	   		"WHERE A.AREA_ID=?\r\n" + 
    	   		"AND A.DATA_AUTH=?\r\n" + 
    	   		"AND   \r\n" + 
    	   		"(  \r\n" + 
    	   		"--非跨天  \r\n" + 
    	   		"(B.CALW_INTER_DAY='N'   \r\n" + 
    	   		"AND TO_CHAR(NOW(),'HH24:MI')>=TO_CHAR(B.CALW_START,'HH24:MI')   \r\n" + 
    	   		"AND TO_CHAR(NOW(),'HH24:MI')<=TO_CHAR(B.CALW_END,'HH24:MI'))  \r\n" + 
    	   		"OR   \r\n" + 
    	   		"--跨天  \r\n" + 
    	   		"(B.CALW_INTER_DAY='Y'   \r\n" + 
    	   		"AND (TO_CHAR(NOW(),'HH24:MI')>=TO_CHAR(B.CALW_START,'HH24:MI') AND TO_CHAR(NOW(),'HH24:MI')<='23:59:59')  \r\n" + 
    	   		"AND (TO_CHAR(NOW(),'HH24:MI')<=TO_CHAR(B.CALW_END,'HH24:MI') AND TO_CHAR(NOW(),'HH24:MI')>='00:00:00')  \r\n" + 
    	   		")  \r\n" + 
    	   		")  \r\n" + 
