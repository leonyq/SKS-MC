 ...
 * 
 */
package com.more.mes.smt.linemonitor.service.pg;
...
public class LineMonitorService
{
  private static final Log log = LogFactory
			.getLog("com.more.mes.smt.linemonitor.service.pg.LineMonitorService");
  ...
  public Map getSetting(String caId) {
    ...
    String sql = "select a.id,"
                 ...
                 + "       a.server_ip,"
                 + "       coalesce(a.server_port,-1) server_port,"
                 + "       a.server_state"
    ...
    String stationSql = "select a.id,\r\n" + 
    		...
    		"a.ca_id,\r\n" + 
    		"a.type,\r\n" + 
    		"case a.type when '1' then '投入站' when '2' then '炉前站' else '' end as type_name,\r\n" + 
    		"a.work_center_id,\r\n" + 
    		"a.sensor_state,\r\n" + 
    		"a.sensor_controller,\r\n" + 
    		"a.input_signal,\r\n" + 
    		...
    		"a.work_center_state,\r\n" + 
    		"b.ca_name\r\n" + 
    		"from t_cs_station  a left join t_co_area b on a.ca_id = b.ca_id LEFT JOIN T_CO_AREA C ON A.WORK_CENTER_ID=C.CA_ID \r\n" + 
    		"where a.type=:type and b.ca_id=:caId";
    // 设备信息
    /*
    ...
    String deviceSql = "SELECT\r\n" + 
    		...
    		"COALESCE(D.TRACK,'') AS TRACK,\r\n" + 
    		"COALESCE(D.LINE_CONTROLLER,'') AS LINE_CONTROLLER,\r\n" + 
    		"COALESCE(D.CONTROLLER_SIGNAL,'') AS CONTROLLER_SIGNAL\r\n" + 
    		"FROM T_CO_AREA_DEVICE A\r\n" + 
    		"LEFT JOIN T_CO_AREA B ON A.CA_ID=B.CA_ID\r\n" + 
    		"LEFT JOIN T_CO_DEVICE C ON A.CAD_DEVICE_ID=C.ID\r\n" + 
    		"LEFT JOIN T_CS_CONTROL_SIGNAL D ON B.CA_PARENTAREAID=D.CA_ID \r\n" + 
    		"                               AND C.CD_DEVICE_SN=D.DEVICE_SN \r\n" + 
    		"								AND A.CAD_DEVICE_SEQUENCE=CAST(D.DEVICE_SEQ AS VARCHAR)\r\n" + 
    		"WHERE B.CA_PARENTAREAID=:caId \r\n" + 
    		"AND A.CAD_DEVICE_ID IN (\r\n" + 
    		"SELECT E.CD_DEVICE_ID FROM T_CO_DEVICE_LOADPOINT E\r\n" + 
    		") \r\n" + 
    		"ORDER BY CAST(A.CAD_DEVICE_SEQUENCE AS INTEGER) ASC";
  ...
  public List getDeviceOnLineList(String caId, String deviceSn)
  {
    ...
    moSql = moSql + " ) T  order by CAST(T.cad_device_sequence AS INTEGER)";
  ...
  public List getMaterialList(String areaSN/* , String moNum, String deviceSN */)
  {
    ...
    /*String sql = "SELECT\r\n" + 
    		...
    		"B.PDO_DEVICE_SN,\r\n" + 
    		"C.CD_DEVICE_NAME,\r\n" + 
    		"CASE B.PDO_TYPE WHEN '1' THEN '上线' WHEN '2' THEN '转产' WHEN '3' THEN '就绪' ELSE '' END PDO_TYPE,\r\n" + 
    		"A.CMO_MO_NUMBER,\r\n" + 
    		"A.CMO_MODEL_CODE,\r\n" + 
    		"CASE A.CMO_PROCESS_FACE WHEN '0' THEN '单面' WHEN '1' THEN '正面' WHEN '2' THEN '反面' WHEN '3' THEN '阴阳面' ELSE '' END WSI_PROCESS_FACE,\r\n" + 
    		"COALESCE(A.CMO_TABLE_NO,'')||'-'||COALESCE(A.CMO_LOADPOINT,'') CMO_LOADPOINT, \r\n" + 
    		"CASE A.CMO_CHANEL_SN WHEN 'A' THEN '单通道' WHEN 'L' THEN '左通道' WHEN 'M' THEN '中通道' WHEN 'R' THEN '右通道' ELSE '' END WSI_CHANEL_NO,\r\n" + 
    		"E.WSI_ITEM_SN,\r\n" + 
    		"A.CMO_ITEM_CODE,\r\n" + 
    		"F.CI_ITEM_NAME,\r\n" + 
    		"F.CI_ITEM_SPEC,\r\n" + 
    		...
    		"WHERE A.CMO_AREA_SN=:areaSN \r\n" + 
    		"ORDER BY COALESCE(A.CMO_SHKIP_FLAG,'A') ASC, COALESCE(E.WSI_ONLINE_FLAG,'A') ASC,B.PDO_DEVICE_SEQ ASC,A.CMO_TABLE_NO ASC,A.CMO_LOADPOINT ASC,E.WSI_CHANEL_NO ASC";*/
    String sql="SELECT\r\n" + 
    		"B.PDO_DEVICE_SEQ,\r\n" + 
    		"B.PDO_DEVICE_SN,\r\n" + 
    		"C.CD_DEVICE_NAME,\r\n" + 
    		...
    		"CASE A.WSI_PROCESS_FACE WHEN '0' THEN '单面' WHEN '1' THEN '正面' WHEN '2' THEN '反面' WHEN '3' THEN '阴阳面' ELSE '' END WSI_PROCESS_FACE,\r\n" + 
    		"COALESCE(A.WSI_TABLE_NO,'')||'-'||COALESCE(A.WSI_TABLE_NO,'') CMO_LOADPOINT, \r\n" + 
    		"CASE A.WSI_CHANEL_NO WHEN 'A' THEN '单通道' WHEN 'L' THEN '左通道' WHEN 'M' THEN '中通道' WHEN 'R' THEN '右通道' ELSE '' END WSI_CHANEL_NO,\r\n" + 
    		"A.WSI_ITEM_SN,\r\n" + 
    		"A.WSI_ITEM_CODE AS CMO_ITEM_CODE,\r\n" + 
    		"F.CI_ITEM_NAME,\r\n" + 
    		"F.CI_ITEM_SPEC,\r\n" + 
    		"A.WSI_ONLINE_FLAG,\r\n" + 
    		"COALESCE(E.CMO_SHKIP_FLAG,'N') AS CMO_SHKIP_FLAG,\r\n" + 
    		"A.WSI_LOADTIME\r\n" + 
    		...
    		"WHERE A.WSI_AREA_ID=:areaSN AND A.WSI_ONLINE_FLAG='Y' AND COALESCE(A.WSI_DEVICE_SEQ,'&')<>'&' AND COALESCE(B.PDO_DEVICE_SEQ,'&')<>'&'\r\n" + 
    		"ORDER BY COALESCE(E.CMO_SHKIP_FLAG,'A') ASC, \r\n" + 
    		"COALESCE(A.WSI_ONLINE_FLAG,'A') ASC,\r\n" + 
  ...
  public List getAlarmInfo(String areaSN)
  {
    String sql = "select DISTINCT t.id, \r\n" + 
    		"t.qwl_time, \r\n" + 
    		"t.qwl_type, \r\n" + 
    		"'类型:' || (CASE t.qwl_event WHEN '1' THEN '换班巡检' WHEN '2' THEN 'QC巡检' WHEN '3' THEN '品质报警' ELSE '' END) || '/制令单:' || t.qwl_mo_number || \r\n" + 
  ...
  {
    String sql = "select T2.*  from (select T1.device_sn, \r\n" + 
    		"                 min(T1.DEVICE_STATUS) as DEVICE_STATUS, \r\n" + 
    		"                 min(T1.FRIST_TRACK) as FRIST_TRACK, \r\n" + 
    		"                 max(T1.ALARM_TYPE) as ALARM_TYPE \r\n" + 
    		"                 from (SELECT DISTINCT B.CD_DEVICE_SN as device_sn,  C.PDO_TYPE, \r\n" + 
    		"                 D.CDS_FRIST_TRACK,  D.CDS_ALARM_TYPE, \r\n" + 
    		"								 CASE C.PDO_TYPE WHEN '' THEN '空闲' WHEN '1' THEN '上线' WHEN '2' THEN '转产' WHEN '3' THEN '就绪' ELSE '' END DEVICE_STATUS,\r\n" + 
    		"                 CASE    WHEN D.CDS_FRIST_TRACK IS NULL THEN       CASE \r\n" + 
    		"                       WHEN C.PDO_TYPE = '3' THEN '连通' \r\n" + 
    		"                       WHEN C.PDO_TYPE IS NULL THEN '连通'        ELSE '断开' END \r\n" + 
    		"                   ELSE       CASE       WHEN D.CDS_ALARM_TYPE='2' THEN\r\n" + 
    		"                         CASE           WHEN D.CDS_FRIST_TRACK = '1' THEN '断开' \r\n" + 
    		"                           WHEN D.CDS_FRIST_TRACK = '2' THEN '连通'\r\n" + 
    		"                           ELSE '不操作' END        ELSE '不操作' END\r\n" + 
    		"                   END FRIST_TRACK, \r\n" + 
    		"									 CASE D.CDS_ALARM_TYPE WHEN '1' THEN 'N' WHEN '2' THEN 'Y' ELSE '' END ALARM_TYPE\r\n" + 
    		"                 FROM T_CO_AREA_DEVICE T LEFT JOIN T_CO_AREA A \r\n" + 
    		"                 ON T.CA_ID = A.CA_ID  LEFT JOIN T_CO_DEVICE B \r\n" + 
    		"                 ON T.CAD_DEVICE_ID = B.ID  LEFT JOIN T_PM_DEVICE_ONLINE C \r\n" + 
    		"                 ON B.CD_DEVICE_SN = C.PDO_DEVICE_SN \r\n" + 
    		"                 AND C.PDO_AREA_SN =:areaSN  LEFT JOIN T_CO_DEVICE_SINGEL D \r\n" + 
    		"                 ON B.CD_DEVICE_SN = D.CDS_DEVICE_SN    WHERE 1 = 1 \r\n" + 
	  ...
	  try{
		  ...
		  if(dataList.size()<=0){
			  ...
			  if("1".equals(signFlag)) sqlInner="?,'N','N','N','N','N','N'";
				  ...
				  sqlInner="'N',?,'N','N','N',(CASE ? WHEN 'Y' THEN 'Y' ELSE 'N' END),(CASE ? WHEN 'Y' THEN 'N' ELSE 'Y' END )"; 
			  ...
			  if("1".equals(signFlag)) sqlInner="TCLA_SENSOR_SIGN=?";
				  ...
				  sqlInner="TCLA_SMEMA_SIGN=?,TCLA_CALL_SIGN=(CASE ? WHEN 'Y' THEN 'Y' ELSE 'N' END ),TCLA_PASS_SIGN=(CASE ? WHEN 'Y' THEN 'N' ELSE 'Y' END )"; 
