...
public class SaveEditCheckTypeInfo implements FuncService
{
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    try
    {
      ...
      String qmtSampleType = request.getParameter("mySampleType");// 检测类型
      ...
      if (!"".equals(modelCode))
      {
        modTable1.setSqlWhere(" and QMT_MODEL_CODE=? and DATA_AUTH = ? ");
        paraList.add(modelCode);
        paraList.add(dataAuth);
      }
      else
      {
        modTable1.setSqlWhere(" and QMT_MODEL_CODE is null and QMT_ITEM_GENRE = ? and DATA_AUTH = ? ");
