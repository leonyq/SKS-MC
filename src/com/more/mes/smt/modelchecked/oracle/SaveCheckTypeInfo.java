...
public class SaveCheckTypeInfo implements FuncService
{
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    try
    {
      String modelCode = request.getParameter("paraMap1@0#QMT_MODEL_CODE");// 机种
      String step = request.getParameter("paraMap1@0#QMT_TEST_STEP");// 检测阶别
      String QMT_SAMPLE_TYPE = request.getParameter("paraMap1@0#QMT_SAMPLE_TYPE");// 检测类型
      ...
      String sql = "select id from T_QM_MODEL_TYPE where QMT_MODEL_CODE=? and QMT_TEST_STEP=? and "
                   + "QMT_ITEM_TYPE=? and QMT_ERROR_GRADE=? ";
      List tempList = null;
      for (int i = 0; i < len; i++ )
      {
        tempList = modelService.listDataSql(
          sql,
          new Object[] {modelCode, step, itemTypes[i],
            errorGrades[i]});
      ...
      for (int i = 0; i < len; i++ )
      {
        ...
        modTable.getColMap().put("QMT_ALQ_VALUE", AQLValues[i]);
        ...
        modTable.getColMap().put("QMT_THRESHOLD", thresholds[i]);
        if (dataAuth != null)
        {
