...
public class SaveProjectItem implements FuncService
{
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    String modelCode = request.getParameter("paraMap1@0#QMI_MODEL_CODE");// 机种
    ...
    for (int i = 0; i < len; i++ )
    {
      ...
      {
        maxValNum = Float.parseFloat(maxValue[i]);
      }
      if (!StringUtils.isBlank(minValue[i]))
      {
        minValNum = Float.parseFloat(minValue[i]);
      }
