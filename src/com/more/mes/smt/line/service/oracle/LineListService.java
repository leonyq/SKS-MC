 ...
 * 
 */
package com.more.mes.smt.line.service.oracle;
...
public class LineListService
{
  ...
  /**
   * @Description: 获取前5项不良信息
   * @return List
   * @throws
   */
  public List getErrorList()
  {
    String sql = "SELECT A.WE_ERROR_CODE, B.CEC_NAME, A.WE_ERROR_NUM "
                 + " FROM (select c.* from (select  ROW_NUMBER() OVER() ROWNUM, T.WE_ERROR_CODE, SUM(T.WE_ERROR_NUM) WE_ERROR_NUM "
                 + " FROM T_WIP_ERROR T "
                 + " WHERE TO_CHAR(T.WE_TEST_TIME, 'yyyy-MM-dd') = "
                 + " TO_CHAR(clock_timestamp(), 'yyyy-MM-dd') GROUP BY T.WE_ERROR_CODE) c where c.ROWNUM <= 5) A "
                 + " LEFT JOIN T_CO_ERROR_CODE B " + " ON A.WE_ERROR_CODE = B.CEC_CODE "
                 + " ORDER BY coalesce(A.WE_ERROR_NUM, 0) DESC ";
  ...
  {
    String sql = "select substr(aa.cec_name, 1, 8) cec_name, aa.num"+
			 " from (select nvl(sum(t.we_error_num), 0) num,"+
			 "              b.ca_parentareaid,"+
			 "              a.cec_name"+
			 "         from t_wip_error t"+
			 "         left join t_co_error_code a"+
			 "           on t.we_error_code = a.cec_code"+
			 "         left join t_co_area b"+
			 "           on b.ca_id = t.WE_AREA_SN"+
			 "        WHERE substr(to_char(t.we_test_time, 'yyyy-mm-dd hh24:mi:ss'),"+
			 "                     1,"+
			 "                     10) ="+
			 "              substr(to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), 1, 10)"+
			 "        group by b.ca_parentareaid, a.cec_name"+
			 "        order by num desc) aa"+
			 "	where aa.ca_parentareaid = ?"+
			 " AND rownum <= 5";
    Map<String, String> sqlParaMap = new HashMap<String, String>();
    sqlParaMap.put("areaSn", areaSn);
    List list = modelService.listDataSql(sql, new Object[] {areaSn});
  ...
  public List getLineRateInfo(String areaSn)
  {
	  String sql = "SELECT CASE"+
		      "   WHEN SUM(NVL(A.MPS_MO_INPUT, 0)) = 0 THEN"+
		      "    0"+
		      "   ELSE"+
		      "    ROUND(SUM(NVL(A.MPS_OK_PCS, 0)) / SUM(NVL(A.MPS_MO_INPUT, 0)) * 100,"+
		      "          2)"+
		      "	 	END AS PERCNT"+
	    	  "		FROM T_MID_PRODUCT_STATISTIC A"+
	    	  "		LEFT JOIN T_CO_AREA B"+
	    	  "		ON B.CA_ID = A.MPS_AREA_SN"+
	    	  "		LEFT JOIN T_CO_AREA C"+
	    	  "		ON C.CA_ID = B.CA_PARENTAREAID"+
	    	  "		WHERE A.MPS_WORK_DATE = to_char(sysdate, 'yyyy-MM-dd')"+
		      "		AND C.CA_ID = ?";
	    Map<String, String> sqlParaMap = new HashMap<String, String>();
	    sqlParaMap.put("areaSn", areaSn);
	    List list = modelService.listDataSql(sql, new Object[] {areaSn});
  ...
  {
    String sql = " SELECT ROUND(SUM(NVL(T1.CST_STANDARD_TIME, 0) * T.MPS_MO_OUTPUT) /"+
             "	(CASE"+
             "   WHEN T2.CAL_ID IS NULL THEN"+
             "    (SELECT SUM(aa.sumtime)"+
             "       FROM (SELECT (C.CALW_END - C.CALW_START) * 24 * 60 * 60 -"+
             "                    (C.CALW_REST_TOTAL * 60) sumtime"+
             "               FROM T_CO_CALENDAR B"+
             "               LEFT JOIN T_CO_CALENDAR_WORKTIME C"+
             "                 ON C.CAL_ID = B.ID"+
             "              WHERE B.CAL_DEFAULT_FLAG = 'Y'"+
             "                AND TO_CHAR(SYSDATE, 'HH24') >"+
             "                    TO_CHAR(C.CALW_START, 'HH24')"+
             "             UNION ALL"+
             "             SELECT (TO_DATE(TO_CHAR(SYSDATE, 'HH24:MI:SS'),"+
             "                             'HH24:MI:SS') -"+
             "                    TO_DATE(TO_CHAR(C.CALW_START, 'HH24:MI:SS'),"+
             "                             'HH24:MI:SS')) * 24 * 60 * 60 -"+
             "                    (C.CALW_REST_TOTAL * 60) sumtime"+
             "               FROM T_CO_CALENDAR B"+
             "               LEFT JOIN T_CO_CALENDAR_WORKTIME C"+
             ...
             "                AND TO_CHAR(SYSDATE, 'HH24:MI:SS') <"+
             "                    TO_CHAR(C.CALW_END, 'HH24:MI:SS')) aa)"+
             "   ELSE"+
             "    (SELECT SUM(BB.sumtime)"+
             "       FROM (SELECT (F.CALW_END - F.CALW_START) * 24 * 60 * 60 -"+
             "                    (F.CALW_REST_TOTAL * 60) sumtime"+
             "               FROM T_CO_CALENDAR_WORKTIME F"+
             "             WHERE F.CAL_ID = T2.CAL_ID"+
             "                AND TO_CHAR(SYSDATE, 'HH24') >"+
             "                    TO_CHAR(F.CALW_START, 'HH24')"+
             "             UNION ALL"+
             "             SELECT (TO_DATE(TO_CHAR(SYSDATE, 'HH24:MI:SS'),"+
             "                             'HH24:MI:SS') -"+
             "                    TO_DATE(TO_CHAR(F.CALW_START, 'HH24:MI:SS'),"+
             "                             'HH24:MI:SS')) * 24 * 60 * 60 -"+
             "                    (F.CALW_REST_TOTAL * 60) sumtime"+
             "               FROM T_CO_CALENDAR_WORKTIME F"+
             "              WHERE F.CAL_ID = T2.CAL_ID"+
             ...
             "                AND TO_CHAR(SYSDATE, 'HH24:MI:SS') <"+
             "                    TO_CHAR(F.CALW_END, 'HH24:MI:SS')) BB)"+
             " END),"+
             "	2) EFF,"+
	       "	a.ca_id"+
	       "	FROM T_MID_PRODUCT_STATISTIC T"+
	       "	left join t_co_area a"+
	       "  	on t.mps_area_sn = a.ca_id"+
	       "		LEFT JOIN t_co_standard_time t1"+
	       "		ON t1.cst_model_code = t.mps_model_code"+
	       " AND T.MPS_AREA_SN = T1.CST_AREA_SN"+
	       " AND T.MPS_PROCESS_FACE = T1.CST_PROCESS_FACE"+
	       " LEFT JOIN T_CO_CALENDAR_LINE T2"+
	       "  ON T2.AREA_ID = T.MPS_AREA_SN"+
	       "	where a.ca_parentareaid = ?"+
	       " 	AND t.mps_work_date ="+
		   "    substr(to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), 1, 10)"+
		 "	GROUP BY a.ca_id, T2.CAL_ID"+
		 "	ORDER BY ca_id ";
    Map<String, String> sqlParaMap = new HashMap<String, String>();
    sqlParaMap.put("areaSn", areaSn);
    List list = modelService.listDataSql(sql, new Object[] {areaSn});
  ...
  {
    String sql = "SELECT BB.CA_ID, EXP(SUM(DECODE(BB.DIRECRT, 0, 0, LN(BB.DIRECRT)))) DIRECRT"+
    		   "	FROM (SELECT AA.CA_ID,"+
               "	ROUND(1 - (SUM(AA.MGS_NG_PCS) /"+
               "     (SUM(AA.MGS_OK_PCS) + SUM(AA.MGS_NG_PCS))),"+
               "     4) DIRECRT,"+
               "	AA.MGS_GROUP_CODE"+
	           "	FROM (SELECT A.CA_ID, T.MGS_NG_PCS, T.MGS_OK_PCS, T.MGS_GROUP_CODE"+
	           "       FROM T_CO_AREA A"+
	           "       LEFT JOIN T_MID_GROUP_STATISTIC T"+
	           "         ON T.MGS_AREA_SN = A.CA_ID"+
	           "      WHERE A.CA_PARENTAREAID =?"+
	           "        AND T.MGS_WORK_DATE ="+
	           "            SUBSTR(TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS'),"+
	           "                   1,"+
	           "                   10)) AA"+
	           "			GROUP BY AA.CA_ID, AA.MGS_GROUP_CODE"+
	           "			ORDER BY AA.CA_ID) BB"+
	           "		GROUP BY BB.CA_ID";
    Map<String, String> sqlParaMap = new HashMap<String, String>();
    sqlParaMap.put("areaSn", areaSn);
    List list = modelService.listDataSql(sql, new Object[] {areaSn});
 ...
 * @throws ParseException 
  ...
  public List TimeInterval(String areaSn) throws ParseException{
	  ...
	  String sql2 = "SELECT SUM(AA.MPS_PO_OUTPUT) MPS_PO_OUTPUT,AA.BEGIN_TIME,AA.END_TIME,AA.MPS_WORK_DATE FROM(" +
      		...
      		"             		WHERE A.CKC_AREA_SN = ?)"+
      		"   AND T.MPS_WORK_DATE =\n" + 
      		"       SUBSTR(TO_CHAR(SYSDATE - 1, 'YYYY-MM-DD HH24:MI:SS'), 1, 10)" +
      		")AA\n" + 
 ...
 * @throws ParseException 
  ...
  public  List periodThroughRate(String areaSn) throws ParseException{
	  ...
	  //获取直通率
      String sql2 = "SELECT bb.BEGIN_TIME,bb.END_TIME,bb.MGS_WORK_DATE,exp(sum(decode(bb.DIRECRT,0,0,ln(bb.DIRECRT)))) DIRECRT FROM(" +
      		...
      		"             	WHERE A.CKC_AREA_SN = ?)"+
      		"           AND T.MGS_WORK_DATE =\r\n" + 
      		"               SUBSTR(TO_CHAR(SYSDATE - 1, 'yyyy-mm-dd hh24:mi:ss'), 1, 10)) AA\r\n" + 
	  ...
	  valList = modelService.listDataSql(sql2, new Object[] {areaSn,areaSn});
	//获取不良pcs数
      String pcsSql = "SELECT TO_CHAR(SYSDATE,'YYYY-MM-DD') WORK_DATE,\r\n" + 
      		"TO_CHAR(C.CALW_START,'HH24:MI') BEGIN_TIME,TO_CHAR(C.CALW_END,'HH24:MI') END_TIME,\r\n" + 
      		"(SELECT NVL(SUM(T1.WE_ERROR_NUM),0) FROM T_WIP_ERROR T1 WHERE trunc(T1.WE_TEST_TIME) = TRUNC(SYSDATE)\r\n" + 
      		"AND TO_CHAR(T1.WE_TEST_TIME,'HH24:MI:SS')>=TO_CHAR(C.CALW_START,'HH24:MI:SS')\r\n" + 
      		"AND TO_CHAR(T1.WE_TEST_TIME,'HH24:MI:SS')<TO_CHAR(C.CALW_END,'HH24:MI:SS')" +
      		"AND T1.WE_AREA_SN IN(SELECT KA.CA_ID FROM T_CO_KANBAN_CONFIG F\r\n" + 
      		"LEFT JOIN T_CO_AREA KA ON KA.CA_PARENTAREAID=F.CKC_AREA_SN\r\n" + 
      		"  WHERE F.CKC_AREA_SN = ?)) ERROR_NUM\r\n" + 
      		"  FROM T_CO_CALENDAR B\r\n" + 
      		"  LEFT JOIN T_CO_CALENDAR_WORKTIME C\r\n" + 
      		"    ON C.CAL_ID = B.ID\r\n" + 
      		" WHERE B.CAL_DEFAULT_FLAG = 'Y'\r\n" + 
      		" AND C.CALW_INTER_DAY = 'N'\r\n" + 
      		" GROUP BY C.CALW_START,C.CALW_END\r\n" + 
      		"UNION ALL\r\n" + 
      		"SELECT TO_CHAR(SYSDATE-1,'YYYY-MM-DD') WORK_DATE,\r\n" + 
      		"TO_CHAR(C.CALW_START,'HH24:MI') BEGIN_TIME,TO_CHAR(C.CALW_END,'HH24:MI') END_TIME,\r\n" + 
      		"(SELECT NVL(SUM(T1.WE_ERROR_NUM),0) FROM T_WIP_ERROR T1 WHERE trunc(T1.WE_TEST_TIME) = TRUNC(SYSDATE-1)\r\n" + 
      		"AND TO_CHAR(T1.WE_TEST_TIME,'HH24:MI:SS')>=TO_CHAR(C.CALW_START,'HH24:MI:SS')\r\n" + 
      		"AND TO_CHAR(T1.WE_TEST_TIME,'HH24:MI:SS')<TO_CHAR(C.CALW_END,'HH24:MI:SS')" +
      		"AND T1.WE_AREA_SN IN(SELECT KA.CA_ID FROM T_CO_KANBAN_CONFIG F\r\n" + 
      		"LEFT JOIN T_CO_AREA KA ON KA.CA_PARENTAREAID=F.CKC_AREA_SN\r\n" + 
