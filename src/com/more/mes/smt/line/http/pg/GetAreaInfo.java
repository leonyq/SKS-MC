...
public class GetAreaInfo extends InterfaceLog implements IhttpServiceJosn
{
  ...
  public String exeFunc(HttpCoreService service)
  {
    ...
    List list = lls.getAreaInfo(areaSn);
    if (list != null && !list.isEmpty())
    {
      json = JSONArray.fromObject(list);
    }
    List deviceList = lls.getWorkstationDevice(areaSn);
    JSONArray deviceJson = new JSONArray();
    if(deviceList!=null&&!deviceList.isEmpty()){
    	deviceJson=JSONArray.fromObject(deviceList);
    }
    Map map = new HashMap();
    map.put("json", json);
    map.put("deviceJson", deviceJson);
