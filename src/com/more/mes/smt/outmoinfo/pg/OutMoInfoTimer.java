package com.more.mes.smt.outmoinfo.pg;
...
public class OutMoInfoTimer implements IJobx
{
    ...
    try
    {
      ...
      if (map.get("VR_VALUE") != null)
      {
        ...
        String itemSql = "select distinct t.*,t1.wom_standard_hours,t1.WOM_NEXT_NUM "
                         ...
                         + "AND t1.WOM_PROCESS_FACE=t.PM_PROCESS_FACE AND t1.wom_next_time<clock_timestamp() "
        ...
        if (itemList != null && itemList.size() > 0)
        {
          ...
          String idSql = "select t.* from T_PM_PROJECT_DETAIL t " + "where t.PROJECT_ID=? ";
          for (int i = 0; i < itemNum; i++ )
          {
            ...
            // 获取对应的物料bom信息
            List<Map> idList = modelService.listDataSql(idSql,
              new Object[] {itemList.get(i).get("PM_PROJECT_ID")});
            if (idList != null && idList.size() > 0)
            {
              ...
              for (int j = 0; j < idNum; j++ )
              {
                if (idList.get(j).get("CBD_ITEM_CODE") != null)
                {
                  ...
                  outStockItem.getColMap().put("WOI_CONNECT_DOC",
                    itemList.get(i).get("PM_MO_NUMBER"));// 关联单号
        ...
        if (itemSmtList != null && itemSmtList.size() > 0)
        {
          ...
          for (int i = 0; i < smtNum; i++ )
          {
            ...
            // 获取对应的料站明细信息
            List<Map> idList = modelService.listDataSql(idSql, new Object[] {
              itemSmtList.get(i).get("PM_MODEL_CODE"), itemSmtList.get(i).get("PM_AREA_SN")});
            if (idList != null && idList.size() > 0)
            {
              int idNum = idList.size();
              TableDataMapExt outStockItem = new TableDataMapExt();
              ...
              for (int j = 0; j < idNum; j++ )
              {
                if (idList.get(j).get("CMD_ITEM_CODE") != null)
                {
                  ...
                  outStockItem.getColMap().put("WOI_PLAN_NUM",
                    itemSmtList.get(i).get("WOM_NEXT_NUM"));// 计划数量 等于下次备料数
                  outStockItem.getColMap().put("WOI_ITEM_SEQ", 0);// 项次
                  outStockItem.getColMap().put("WOI_CONNECT_DOC",
                    itemSmtList.get(i).get("PM_MO_NUMBER"));// 关联单号
                  modelService.save(outStockItem);
                }
              }
            }
        ...
        if (uList != null && uList.size() > 0)
        {
          ...
          for (int i = 0; i < num; i++ )
          {
            ...
            wom.setSqlWhere(" and id =? ");
            paraList.add(uList.get(i).get("ID"));
            wom.setSqlWhereArgs(paraList);

            wom.put("WOM_OUT_NUM",
              (int)uList.get(i).get("WOM_OUT_NUM") + (int)uList.get(i).get("WOM_NEXT_NUM"));
            ...
            int planNum = (int)uList.get(i).get("WOM_MO_NUM")
                          - (int)uList.get(i).get("WOM_OUT_NUM");
            int cNum = 2 * ((int)map.get("VR_VALUE") * 3600 / (int)uList.get(i).get(
              "WOM_STANDARD_HOURS"));
            int nextNum = (int)map.get("VR_VALUE") * 3600
                          / (int)uList.get(i).get("WOM_STANDARD_HOURS");
            if (planNum <= cNum)
            {
        ...
        String addSmtSql = "select a.* from (select ROW_NUMBER() OVER() ROWNUM,t.*, t1.cst_standard_time "
                           + " from T_PM_MO_BASE t "
                           + " LEFT JOIN t_co_standard_time t1 "
                           + " ON t1.cst_model_code = t.pm_model_code "
                           + " AND t1.cst_step_code = t.pm_product_step "
                           + " AND t1.cst_process_face = t.pm_process_face "
                           + " where NOT EXISTS (SELECT 1 "
                           + " FROM T_WMS_OUT_MO_INFO a "
                           + " WHERE a.WOM_MO_NUMBER = t.PM_MO_NUMBER "
                           + " and a.WOM_PROCESS_FACE = t.PM_PROCESS_FACE) "
                           + " and t.pm_product_step = '1' "
                           + " and t.PM_STATUS not in ('4', '5') "
                           + " and t.PM_SCHEDULE_DATE < clock_timestamp() + ((? ::int/ 24)::text ||' days')::interval "
                           + " ) a where rownum <= 1 ";
        List<Map> aList = modelService.listDataSql(addSmtSql, new Object[] {advanceTime});
        if (aList != null && aList.size() > 0)
        {
          ...
          {// 取到了标准工时
            ...
            {
              int cNum = 2 * ((int)map.get("VR_VALUE") * 3600 / (int)aList.get(i).get(
                "CST_STANDARD_TIME"));
              int outNum = (int)map.get("VR_VALUE") * 3600
                           / (int)aList.get(i).get("CST_STANDARD_TIME");
              ...
              // 获取对应的料站明细信息
              List<Map> idList = modelService.listDataSql(idSql,
                new Object[] {aList.get(i).get("PM_AREA_SN"), aList.get(i).get("PM_MODEL_CODE")});
              if (idList != null && idList.size() > 0)
              {
                int idNum = idList.size();
                TableDataMapExt outStockItem = new TableDataMapExt();
                ...
                for (int j = 0; j < idNum; j++ )
                {
                  if (idList.get(j).get("CMD_ITEM_CODE") != null)
                  {
                    ...
                    outStockItem.getColMap().put("WOI_DOC_NUM", WDI_DOC_NUM);// 出库单号
                    outStockItem.getColMap().put("WOI_STATUS", "1");// 物料状态
                    outStockItem.getColMap().put("WOI_ITEM_CODE",
                      idList.get(j).get("CMD_ITEM_CODE"));// 物料料号
                    outStockItem.getColMap().put("WOI_PLAN_NUM",
                      (int)idList.get(j).get("CMD_POINT_NUM") * outNum);// 计划数量
                    outStockItem.getColMap().put("WOI_ITEM_SEQ", 0);// 项次
                    outStockItem.getColMap().put("WOI_CONNECT_DOC",
                      aList.get(i).get("PM_MO_NUMBER"));// 关联单号
                    modelService.save(outStockItem);
                  }
                }
              }
        ...
        String addSql = "select a.* from (select ROW_NUMBER() OVER() ROWNUM,t.*, t1.cst_standard_time "
                        + " from T_PM_MO_BASE t "
                        + " LEFT JOIN t_co_standard_time t1 "
                        + " ON t1.cst_model_code = t.pm_model_code "
                        + " AND t1.cst_step_code = t.pm_product_step "
                        ...
                        + " and t.PM_SCHEDULE_DATE < clock_timestamp() + ((? ::int/ 24)::text ||' days')::interval "
                        + " ) a where rownum <= 1 ";
        List<Map> addList = modelService.listDataSql(addSmtSql, new Object[] {advanceTime});
         ...
         * "and a.WOM_PROCESS_FACE=t.PM_PROCESS_FACE) and t.pm_product_step='1' " +
         * "and t.PM_STATUS not in('4','5') and t.PM_SCHEDULE_DATE<SYSDATE+" + advanceTime +
         * "/24 and rownum<=1 "; List<Map> addList = modelService.listDataSql(addSql);
         */
        if (addList != null && addList.size() > 0)
        {
          ...
          {// 取到了标准工时
            ...
            {
              int cNum = 2 * ((int)map.get("VR_VALUE") * 3600 / (int)addList.get(i).get(
                "CST_STANDARD_TIME"));
              int outNum = (int)map.get("VR_VALUE") * 3600
                           / (int)addList.get(i).get("CST_STANDARD_TIME");
              ...
              // 获取对应的料站明细信息
              List<Map> idList = modelService.listDataSql(idSql,
                new Object[] {addList.get(i).get("PM_PROJECT_ID")});
              if (idList != null && idList.size() > 0)
              {
                int idNum = idList.size();
                TableDataMapExt outStockItem = new TableDataMapExt();
                ...
                for (int j = 0; j < idNum; j++ )
                {
                  if (idList.get(j).get("CBD_ITEM_CODE") != null)
                  {
                    ...
                    outStockItem.getColMap().put("WOI_STATUS", "1");// 物料状态
                    outStockItem.getColMap().put("WOI_ITEM_CODE",
                      idList.get(j).get("CBD_ITEM_CODE"));// 物料料号
                    outStockItem.getColMap().put("WOI_PLAN_NUM",
                      (int)idList.get(j).get("CBD_ITEM_NUM") * outNum);// 计划数量
                    outStockItem.getColMap().put("WOI_ITEM_SEQ", 0);// 项次
                    outStockItem.getColMap().put("WOI_CONNECT_DOC",
                      addList.get(i).get("PM_MO_NUMBER"));// 关联单号
                    modelService.save(outStockItem);
                  }
                }
              }
  ...
  @SuppressWarnings("unchecked")
  public String getOutNo(String dataAuth, String prefix, String dispatchSn,
