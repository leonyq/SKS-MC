package com.more.mes.smt.wmsoutstock.etstock.addfp.pg;
...
public class SearchItem implements FuncService {
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
		...
		try {
			...
			String existCode = modelAction.getParaMap().get("existCode");// bom子件ID
			String codes = modelAction.getParaMap().get("codes");// bom子件code
			String projectId = modelAction.getParaMap().get("projectId");
			String sql = " SELECT DISTINCT C.*, B.WSI_ITEM_NUM, coalesce(D.wos_item_rn, 0) WOS_ITEM_RN  , coalesce(D.wos_item_n, 0) WOS_ITEM_N    "
					...
					+ "   left join T_PM_PROJECT_DETAIL T2     ON T2.PROJECT_ID = T1.PROJECT_ID      "
					+ " 	 LEFT JOIN T_CO_ITEM T4         ON T2.CBD_ITEM_CODE = T4.CI_ITEM_CODE) C  "
					+ " 	 LEFT JOIN (SELECT T6.WSI_ITEM_CODE, coalesce(T6.WSI_ITEM_NUM) WSI_ITEM_NUM     "
					+ "    FROM T_WMS_STOCK_INFO T6      GROUP BY T6.WSI_ITEM_CODE,WSI_ITEM_NUM) B  ON C.CBD_ITEM_CODE = B.WSI_ITEM_CODE "
			...
			// 计算当前投入量
			String cntsql = "  SELECT coalesce(sum( coalesce(ET_FQC_COUNT,0)),0)  ET_FQC_COUNTS  FROM  T_WMS_OUTSTOCK_DOC A WHERE ET_CONECT_PROJECT_ID = ? AND wod_doc_type ='DJ27'  ";
			...
			// 获得已投入数量
			// itemList = modelService.listSql(sql, modelAction.getPage(), sqlWhere, null,
			// null, null);
