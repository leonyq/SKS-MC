	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
		...
		if (type.equals("errorMethod")) {
			...
			String sql = "SELECT * from(select t.WE_ERROR_CODE,"
					+ "(SELECT A.CEC_NAME FROM T_CO_ERROR_CODE A WHERE A.CEC_CODE = T.WE_ERROR_CODE) ERROR_NAME,"
					+ "COUNT(t.we_error_code) ERRORNUM "
					+ "from T_WIP_ERROR t WHERE t.we_model_code=? and t.DATA_AUTH=? "
					+ "GROUP BY t.we_error_code  ORDER BY ERRORNUM DESC)TT  LIMIT 10 ";
			List<Map<String, Object>> mList = modelService.listDataSql(sql, new Object[] { modelCode, auth });
			CommMethod.listToEscapeJs(mList);
			modelAction.setAjaxList(mList);
		}
