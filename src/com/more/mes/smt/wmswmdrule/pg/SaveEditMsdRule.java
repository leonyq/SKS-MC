package com.more.mes.smt.wmswmdrule.pg;
...
public class SaveEditMsdRule implements FuncService{
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
		 	...
		 	if (StringUtils.isNotBlank(WSR_BAKE_LIMIT))
      {
        ...
        {
		 		  msd1.getColMap().put("WSRD_BAKE_TEMP", Double.parseDouble(backTemps[i]));
        }
		 		if (StringUtils.isNotBlank(backTimes[i]))
        {
		 		 msd1.getColMap().put("WSRD_BAKE_TIME", Double.parseDouble(backTimes[i]));
        }
		 		if (StringUtils.isNotBlank(backMax[i]))
        {
		 		 msd1.getColMap().put("WSRD_BAKE_MAX", Double.parseDouble(backMax[i]));
        }
		 		if (StringUtils.isNotBlank(tempDeps[i]))
        {
		 		  msd1.getColMap().put("WSRD_TEMP_DEPARTURE", Double.parseDouble(tempDeps[i]));
        }
