...
public class getHomeInformation extends InterfaceLog implements IhttpServiceJosn
{
  ...
  public String exeFunc(HttpCoreService service)
  {
    ...
    try
    {
       ...
       */
      /*
       * String Sql = " SELECT * FROM  sy_data_auth A " + " LEFT JOIN sy_USER C ON C.ID=A.USER_ID "
       * + " LEFT JOIN T_APP_MOUDLE B ON B.DEPT_ID=A.DEPT_ID " +
       * " WHERE C.LOGIN_NAME=? and B.TAM_MENU_ID=? "; res = modelService.countSql(Sql, new
       * Object[] {n_emp_no, tam_menu_id});
       */
      res = 1;
      ...
      if (res != 0)
      {
        ...
        String SqlType = "SELECT distinct TAS_SHOW_TYPE,TAM_SEQ FROM T_APP_SHOWMANAGE A "
                         + "LEFT JOIN T_APP_MOUDLE B ON A.TAS_MOUDLE_ID  = B.ID "
                         + " WHERE A.TAS_MENU_ID=? order by b.TAM_SEQ";
        
        // 判断该菜单下的功能模块
        listType1 = modelService.listDataSql(SqlType, new Object[] {tam_menu_id});
        ...
        int w = 0;
        for (int f = 0; f < listType1.size(); f++ )
        {
          if (listType1.get(f).get("TAS_SHOW_TYPE").equals("1"))
          {   if(checkDeviceCollectCode == false){
              ...
              {
                if (listTitle.get(i).get("TAS_SHOW_TYPE").toString().equals("1"))
                {
                  HashMap<String, Object> resultHashMap = new HashMap<String, Object>();
                  Map<String, String> map = (Map<String, String>)listTitle.get(i);
                  ...
                  if (null != date && date.size() > 0)
                  {
                    ...
                    {
                      if (map.get("TAM_MOUDLE_NAME").equals(
                        date.get(j).get("title").toString()))
                      {
                        a = j;
                        break;
                      }
                  ...
                  if (a != -1)
                  {
                    listContent1 = (List<Map>)date.get(a).get("data");
                  }
                  ...
                  if (map.get("ID") != null)
                  {
                    ...
                    String sqlContent = " select t1.TAS_FUN_NAME,t5.PIC_NAME,t1.TAS_PIC_ID, "
                                        ...
                                        + " where t1.TAS_SHOW_TYPE =? " + " and TAM_MENU_ID = ? "
                                        + " and t1.id = ? " + " order by t1.TAS_SEQ ";

                    listContent = modelService.listDataSql(sqlContent, new Object[] {"1",
                      tam_menu_id, listTitle.get(i).get("ID")});//
                    for (int i1 = 0; i1 < listContent.size(); i1++ )
                    {
                      String competitive = listContent.get(i1).get("TAS_DATA_CONTENT").toString();
                      String src;

                      String a3 = CommMethod.toJsonObj(competitive).get("oracle").toString();
                      String selSql = null;
                      String str = a3;
                      ...
                      List selSqlMap = this.getSelSql(a3, service, nType);

                      map4 = (Map<String, Object>)selSqlMap.get(0);

                      selSql = (String)selSqlMap.get(1);

                      if (StringUtils.isNotBlank(listContent.get(i1).get("PIC_NAME")))
                      {
                        src = listContent.get(i1).get("PIC_NAME").toString();
                        listContent.get(i1).put("PIC_NAME", "/N2/plf/userPhoto/" + src);
                      }
                      ...
                      for (Object key : list5.get(0).keySet())
                      {

                        a3 = String.valueOf(list5.get(0).get(key));
                      }

                      listContent.get(i1).put("TAS_DATA_CONTENT", a3);
                    ...
                    if (a != -1)
                    {
                      ...
                      for (int j = 0; j < listContent1.size(); j++ )
                      {
                      ...
                      listContent=mapList;
                      listContent = this.delNull(listContent);
                      resultHashMap.put("data", listContent);
                    }
                    else
                    {
                      listContent = this.delNull(listContent);
             ...
             checkDeviceCollectCode = true;
            
          }
          else if (listType1.get(f).get("TAS_SHOW_TYPE").equals("2"))
          {   if(checkDeviceCollectCode2 == false){

              List<Map> listContent = new ArrayList<>();
              // List date = new ArrayList<>();
              for (int i = 0; i < listTitle.size(); i++ )
              {
                if (listTitle.get(i).get("TAS_SHOW_TYPE").toString().equals("2"))
                {
                  HashMap<String, Object> resultHashMap = new HashMap<String, Object>();
                  Map<String, String> map = (Map<String, String>)listTitle.get(i);
                  resultHashMap.put("title",
                    map.get("TAM_MOUDLE_NAME") + "-" + map.get("TAS_FUN_NAME"));
                  if (map.get("ID") != null)
                  {
                    String dateId = map.get("ID");
                    // 获取功能细项、图片ID、数据内容
                    ...
                    listContent = modelService.listDataSql(sqlContent, new Object[] {"2",
                      tam_menu_id, listTitle.get(i).get("ID")});
                    for (int i1 = 0; i1 < listContent.size(); i1++ )
                    {
                      ...
                      for (int j = 0; j < list5.size(); j++ )
                      {
                        ...
                        for (Object key : map2.keySet())
                        {
                          ...
                          if (a == 0)
                          {
                            map31.put("X_NAME", String.valueOf(map2.get(key)));
                          }
                          else if (a == 1)
                          {
                            map31.put("Y_NUMBER", String.valueOf(map2.get(key)));
                          }
                          else if (a == 2)
                          {
                            map31.put("Y_UNIT", String.valueOf(map2.get(key)));
                          }
         ...
         checkDeviceCollectCode2 =true;
          }
          else if (listType1.get(f).get("TAS_SHOW_TYPE").equals("3"))
          {   if(checkDeviceCollectCode3 == false){
              ...
              for (int i = 0; i < listTitle.size(); i++ )
              {
                if (listTitle.get(i).get("TAS_SHOW_TYPE").toString().equals("3"))
                {
                  HashMap<String, Object> resultHashMap = new HashMap<String, Object>();
                  Map<String, String> map = (Map<String, String>)listTitle.get(i);
                  resultHashMap.put("title",
                  ...
                  if (map.get("ID") != null)
                  {
                    ...
                    listContent = modelService.listDataSql(sqlContent, new Object[] {"3",
                      tam_menu_id, listTitle.get(i).get("ID")});
                    for (int i1 = 0; i1 < listContent.size(); i1++ )
                    {
          ...
          checkDeviceCollectCode3 = true;
          }
          else if (listType1.get(f).get("TAS_SHOW_TYPE").equals("4"))
          {   if(checkDeviceCollectCode4 == false){

              List<Map> listContent = new ArrayList<>();
              // List date = new ArrayList<>();
              for (int i = 0; i < listTitle.size(); i++ )
              {
                if (listTitle.get(i).get("TAS_SHOW_TYPE").toString().equals("4"))
                {
                  HashMap<String, Object> resultHashMap = new HashMap<String, Object>();
                  Map<String, String> map = (Map<String, String>)listTitle.get(i);
                  resultHashMap.put("title",
                  ...
                  if (map.get("ID") != null)
                  {
                    ...
                    List list = new ArrayList<>();
                    listContent = modelService.listDataSql(sqlContent, new Object[] {"4",
                      tam_menu_id, listTitle.get(i).get("ID")});
                    for (int i1 = 0; i1 < listContent.size(); i1++ )
                    {
          ...
          checkDeviceCollectCode4 =true; 
          }
          else if (listType1.get(f).get("TAS_SHOW_TYPE").equals("5"))
          {   if(checkDeviceCollectCode5 == false){

              List<Map> listContent = new ArrayList<>();
              // List date = new ArrayList<>();
              for (int i = 0; i < listTitle.size(); i++ )
              {
                if (listTitle.get(i).get("TAS_SHOW_TYPE").toString().equals("5"))
                { 
                  HashMap<String, Object> resultHashMap = new HashMap<String, Object>();
                  Map<String, String> map = (Map<String, String>)listTitle.get(i);
                  resultHashMap.put("title",
                  ...
                  if (map.get("ID") != null)
                  {
                    ...
                    List list = new ArrayList<>();
                    listContent = modelService.listDataSql(sqlContent, new Object[] {"5",
                      tam_menu_id, listTitle.get(i).get("ID")});
                    for (int i1 = 0; i1 < listContent.size(); i1++ )
                    {
              ...
              checkDeviceCollectCode5 = true;
          }
          else if (listType1.get(f).get("TAS_SHOW_TYPE").equals("6"))
          {   if(checkDeviceCollectCode6 == false){

              List<Map> listContent = new ArrayList<>();
              // List date = new ArrayList<>();
              for (int i = 0; i < listTitle.size(); i++ )
              {  
                if (listTitle.get(i).get("TAS_SHOW_TYPE").toString().equals("6"))
                { 
                  HashMap<String, Object> resultHashMap = new HashMap<String, Object>();
                  Map<String, String> map = (Map<String, String>)listTitle.get(i);
                  resultHashMap.put("title",
                  ...
                  if (map.get("ID") != null)
                  {
                    ...
                    List list = new ArrayList<>();
                    listContent = modelService.listDataSql(sqlContent, new Object[] {"6",
                      tam_menu_id, listTitle.get(i).get("ID")});// ,listTitle.get(i).get("ID")
                    ...
                    for (int i1 = 0; i1 < listContent.size(); i1++ )
                    {
                      ...
                      list51 = modelService.listDataSql(a3);
                      ...
                      for (int j = 0; j < list51.size(); j++ )
                      {
                        ...
                        for (Object key : map2.keySet())
                        {
                          if (count1 == 0)
                          {
                            ...
                            String rNoc = map2.get(NOC).toString();// 具体国家名

                            map6.put("BAR_TITLE_NAME", rNoc);
                          ...
                          else
                          {
                            ...
                            DOLL = key.toString();
                            int dollCount = Integer.valueOf(String.valueOf(map2.get(DOLL)));// 具体奖牌数
                            whatMap.put("BAR_VALUE", dollCount);

                            whatMap.put("BAR_NAME", DOLL);
                        ...
                        dCount++ ;
                        map6.put("BAR_DATA_CONTENT", Datalist2);
                        list2.add(map6);
                      }
          ...
          checkDeviceCollectCode6 = true;
          }
          else if (listType1.get(f).get("TAS_SHOW_TYPE").equals("7"))
          {   if(checkDeviceCollectCode7 == false){

              List<Map> listContent = new ArrayList<>();
              for (int i = 0; i < listTitle.size(); i++ )
              {
                if (listTitle.get(i).get("TAS_SHOW_TYPE").toString().equals("7"))
                { 
                  HashMap<String, Object> resultHashMap = new HashMap<String, Object>();
                  Map<String, String> map = (Map<String, String>)listTitle.get(i);
                  resultHashMap.put("title",
                  ...
                  if (map.get("ID") != null)
                  {
                    ...
                    List list = new ArrayList<>();
                    listContent = modelService.listDataSql(sqlContent, new Object[] {"7",
                      tam_menu_id, listTitle.get(i).get("ID")});
                    for (int i1 = 0; i1 < listContent.size(); i1++ )
                    {
                      ...
                      for (int j = 0; j < list5.size(); j++ )
                      {
                        ...
                        for (Object key : map2.keySet())
                        {
                          ...
                          if (a == 0)
                          {
                            map31.put("T_X_NAME", String.valueOf(map2.get(key)));
                          }
                          else if (a == 1)
                          {
                            map31.put("T_Y1_BAR", String.valueOf(map2.get(key)));
                          }

                          else if (a == 2)
                          {
                            map31.put("T_Y1_UNIT", String.valueOf(map2.get(key)));
                          }
                          else if (a == 3)
                          {
                            map31.put("T_Y2_LINE", String.valueOf(map2.get(key)));
                          }
                          else if (a == 4)
                          {
                            map31.put("T_Y2_UNIT", String.valueOf(map2.get(key)));
                          }
        ...
        else
        {
          ...
          {}
          if (checkDeviceCollectCode2 == true)
          {}
          if (checkDeviceCollectCode3 == true)
          {}
          if (checkDeviceCollectCode4 == true)
          {}
          if (checkDeviceCollectCode5 == true)
          {}
          if (checkDeviceCollectCode6 == true)
          {}
          if (checkDeviceCollectCode7 == true)
          {}
  ...
  private List<Map> selectLustTile(String n_emp_no, String tam_menu_id)
  {
    ...
    list = modelService.listDataSql(SqlTitle, new Object[] {n_emp_no, tam_menu_id});
    ...
    else
    {
      ...
      SqlTitle = "select * from( select distinct t3.TAM_MOUDLE_NAME,t1.tas_fun_name,t1.ID, "
                 ...
                 + " left join T_APP_SHOWMANAGE t1 " + " on t.TAS_FUN_ID = t1.ID "
                 + " inner join T_APP_MOUDLE t3 " + " on t3.id = t1.TAs_MOUDLE_ID "
                 + " where t3.TAM_MENU_ID = ? " + " and t.TAR_ROLE_ID in " + " (select ROLE_ID "
                 + "  from sy_user_role "
                 + " where USER_ID IN (select id from sy_user where login_name = ?))) "
                 + " order by TAM_SEQ,TAS_SEQ ";
      list = modelService.listDataSql(SqlTitle, new Object[] {tam_menu_id, n_emp_no});
    }
