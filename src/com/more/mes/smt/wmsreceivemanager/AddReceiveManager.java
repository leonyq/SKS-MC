 ...
 * @author development
 */
public class AddReceiveManager implements FuncService
{
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    try
    {
      ...
      String wrdDispatchSn = request.getParameter("paraMap1@0#WRD_DISPATCH_SN");// 收发类型
      String wpdDeliveryDate = request.getParameter("paraMap1@0#WPD_DELIVERY_DATE");// 入库日期
      ...
      for (int i = 0; i < itemCodes.length; i++ )
      {
        for (int j = i + 1; j < itemCodes.length; j++ )
        {
          ...
          { // 如果物料料号相同则判断项次
            ...
            {
              // 如果采购单号不为空则判断料号采购单号项次
              if (!wpiPos[i].equals(""))
              {
                if (wpiPos[i].equals(wpiPos[j]))
                { // 如果采购单号相同则
                  return modelAction.alertParentInfo(modelAction.getText("相同物料料号,采购单号,项次需不同"));
                }
              }
              else
              {
      ...
      // String WRD_DOC_NUM = getReceiveNo(dataAuth, "DJ02", WRD_DISPATCH_SN, modelService);// 入库单号
      GetSnService gss = new GetSnService();
      String userId = StringUtils.toString(CommMethod.getSessionUser().getId());
      String wrdDocNum = gss.getSnForDocNum("DJ02", wrdDispatchSn, userId, dataAuth);
      if (wrdDocNum.indexOf("NG") != -1)
      {
        return modelAction.alertParentInfo(modelAction.getText(wrdDocNum));
      }
      ...
      for (int i = 0; i < itemCodes.length; i++ )
      {
        ...
        receive1.getColMap().put("WRI_ITEM_SEQ", Integer.parseInt(itemSeqs[i]));
        receive1.getColMap().put("WRI_PLAN_NUM", Integer.parseInt(planNums[i]));
        receive1.getColMap().put("WRI_PACK_NUM", Integer.parseInt(packNums[i]));
        receive1.getColMap().put("WRI_GIFT_NUM", Integer.parseInt(giftNums[i]));
