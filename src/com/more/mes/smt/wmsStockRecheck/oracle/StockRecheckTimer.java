...
public class StockRecheckTimer implements IJobx{
	...
	public void exec(Args arg0) throws Exception {
		try {
			...
			String checkSql = "SELECT t.wsi_item_code,SUM(t.wsi_item_num) item_num " +
					...
					"WHERE (t.wsi_test_time + c.ci_recheck_days)<SYSDATE " +
					"and t.wsi_stock_flag='2' " +
					"GROUP BY t.wsi_item_code ";
			//根据物料获取物料信息
			String sql = "SELECT DISTINCT t.* FROM t_wms_stock_info t " +
					"LEFT JOIN T_QM_INSEPCT_PARENT_INFO a ON a.qip_sn=t.wsi_item_sn " +
					"AND (a.QIP_TEST_FLAG ='1' OR a.QIP_TEST_FLAG ='3' OR a.QIP_TEST_FLAG IS NULL) " +
					"LEFT JOIN t_co_item c ON c.ci_item_code=t.wsi_item_code " +
					"WHERE (t.wsi_test_time + c.ci_recheck_days)<SYSDATE " +
			...
			if(timeList!=null&&timeList.size()>0){
				...
				for(int i=0;i<num;i++){
					...
					CommMethod.addSysDefCol(recheck.getColMap(), user.get(0));
				 	...
				 	//新增复检单明细信息
					pList = modelService.listDataSql(sql,new Object[] {timeList.get(i).get("WSI_ITEM_CODE")});
					int pNum = pList.size();
					for(int j=0;j<pNum;j++){
						recheckDet.getColMap().clear();
					 	...
					 	stock.setSqlWhere(" and WSI_ITEM_SN='"+pList.get(j).get("WSI_ITEM_SN")+"'");
					 	stock.getColMap().put("WSI_STOCK_FLAG", "6");
					 	modelService.edit(stock);
					}
					List<Object> reList = getBatchNo("0","",user.get(0).getDeptId(),WRD_DOC_NUM,modelService);
					
					TableDataMapExt recheck1 = new TableDataMapExt();
				 	recheck1.setTableName("T_WMS_RECHECK_DOC");
				 	recheck1.setSqlWhere(" and WRD_DOC_NUM='"+WRD_DOC_NUM+"'");
	...
	@SuppressWarnings("unchecked")
	public List<Object> getBatchNo(String testStep,String sampleType,String dataAuth,String RecheckNo,ModelService modelService){
		ProFuncDto pf = new ProFuncDto();
