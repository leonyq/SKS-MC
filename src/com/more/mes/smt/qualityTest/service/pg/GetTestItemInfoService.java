...
public class GetTestItemInfoService implements FuncService
{
  ...
  {//
     ...
     * " ON C.QII_ERROR_CODE=E.CEC_CODE WHERE C.QII_SN=?) A " + " ORDER BY A.QII_ITEM_TYPE) D";
     */
    ...
    List resultList = modelService.listDataSql(sql, new Object[] {infoId, qipSn,dataAuth});
    if (resultList == null || resultList.isEmpty())
    {
      ...
      sql = "SELECT ROW_NUMBER() OVER() ROWNUM, D.* " + " FROM (SELECT A.* "
            ...
            + " C.QII_ERROR_GRADE, " + " C.QII_ERROR_NUM, "
            + " E.CEC_NAME         QII_ERROR_DEC, "
            + " F.VALUE            AS QII_ERROR_GRADE_NAME, " + " C.QII_ACTUAL_VALUE, "
            + " C.QII_MEMO " + " FROM (SELECT T.QII_INSPECT_SN, "
            + " T1.QTI_ITEM_TYPE     QII_ITEM_TYPE, "
      ...
      resultList = modelService.listDataSql(sql, new Object[] {infoId, qipSn,dataAuth});
      if (resultList == null || resultList.isEmpty())
      {
        /*
    ...
    String sql2 = "SELECT T.QIP_ITEM_NUM, " + " T.QIP_TEST_NUM, " + " T.QIP_SCARP_NUM, "
                  ...
                  + " AND T.QIP_INSPECT_SN = ? AND T.DATA_AUTH=?";
    Map resultMap = modelService.queryForMap(sql2, new Object[] {qipSn, inspectSn,dataAuth});
