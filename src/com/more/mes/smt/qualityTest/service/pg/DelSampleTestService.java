  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    try
    {
      ...
      for (int i = 0; i < qipSnStrsLength; i++ )
      {
        ...
        // + " WHERE T.QIP_INSPECT_SN = ? " + " AND T.QIP_SN = ?";
        String editParSampleSql = "UPDATE T_QM_INSEPCT_PARENT_INFO  "
                                  + " SET QIP_TEST_FLAG = '2', QIP_INSPECT_FLAG='N',"
                                  + " QIP_SCARP_NUM  = 0, QIP_TEST_NUM  = 0,QIP_SAMPLE_NUM=0, "
                                  + " QIP_ERROR_NUM = 0, " + " QIP_TEST_EMP  = null, "
                                  + " QIP_TEST_TIME = null ,QIP_TEST_RESULT=null "
                                  + " WHERE QIP_INSPECT_SN = ? " + " AND QIP_SN = ? AND T.DATA_AUTH=?";
      ...
      if (tempList == null || tempList.isEmpty())
      {
        ...
        String editSampleInfoSql = "UPDATE T_QM_INSEPCT_PARENT_INFO "
                                   + " SET QIP_TEST_FLAG = '2' " + " WHERE QIP_INSPECT_SN = ? AND DATA_AUTH=?";
    ...
    try
    {
      ...
      if (emptyFlag == true)
      {
        ...
        saveInspectInfoSql = "update T_QM_INSPECT_INFO "
                             + " set qii_actual_num=0, qii_error_num=0, "
                             + " qii_list_flag = '1',qii_insepct_result = null "
                             + " where qii_inspect_sn = ? AND DATA_AUTH=?";
        modelService.execSql(saveInspectInfoSql, new Object[] {inspectSn,dataAuth});
      }
      else
      {
        // saveInspectInfoSql = "update T_QM_INSPECT_INFO t " + " set t.qii_error_num     = "
        // + " (select coalesce(max(t2.qim_error_num), 0) "
        // + " from T_QM_INSPECT_MID_INFO t2 "
        // + " where t2.qim_inspect_sn = ?), " + " t.qii_list_flag      = ?, "
        // + " t.qii_insepct_result = ? " + " where t.qii_inspect_sn = ? ";
        ...
        // + " where t.qii_inspect_sn = ? ";
        String saveInspectInfoSql2 = "update T_QM_INSPECT_INFO t " + " set qii_actual_num     = "
                                     + " (select coalesce(sum(t2.qip_test_num), 0) "
                                     + " from T_QM_INSEPCT_PARENT_INFO t2 "
