  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    try
    {
      for (int i = 0; i < lengt; i++ )
      {
         ...
         */
        String editPoSql = "UPDATE T_WMS_PO_DETAIL "
                           + " SET WPD_RECEIVE_NUM = WPD_RECEIVE_NUM - ? "
                           + " WHERE (WPD_PO, WPD_ITEM_CODE) IN "
                           ...
                           + " WHERE B.QII_INSPECT_SN = ? " + " AND B.QII_ITEM_CODE = ? and b.DATA_AUTH=?) and DATA_AUTH=? ";
