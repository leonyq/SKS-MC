...
public class SaveSampleTestService implements FuncService
{
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    try
    {
      ...
      for (int i = 0; i < idLengt; i++ )
      {
        ...
        if (!"".equals(idStrs[i]))
        {
          // 修改
          int errNum = 0;
          if (!StringUtils.isBlank(errorNumStrs[i]))
          {
            errNum = Integer.parseInt(errorNumStrs[i]);
          }
          float actualNum = 0;
          if (!StringUtils.isBlank(actualValStrs[i]))
          {
            actualNum = Float.parseFloat(actualValStrs[i]);
          }
          ...
          insItemMap.getColMap().put("QII_ACTUAL_VALUE", actualNum);// 实际值
          insItemMap.getColMap().put("QII_ERROR_NUM", errNum);// 不良数量
        ...
        else
        {
          // 保存
          float minNum = 0;
          if (!StringUtils.isBlank(minValStrs[i]))
          {
            minNum = Float.parseFloat(minValStrs[i]);
          }
          float maxNum = 0;
          if (!StringUtils.isBlank(maxValStrs[i]))
          {
            maxNum = Float.parseFloat(maxValStrs[i]);
          }
          float actualNum = 0;
          if (!StringUtils.isBlank(actualValStrs[i]))
          {
          ...
          insItemMap.getColMap().put("QII_ITEM_MAXVALUE", maxNum);// 上限值
          insItemMap.getColMap().put("QII_ITEM_MINVALUE", minNum);// 下限值
          insItemMap.getColMap().put("QII_ACTUAL_VALUE", actualNum);// 实际值
          insItemMap.getColMap().put("QII_TEST_RESULT", testResStrs[i]);// 检测结果(1-OK，2-NG)
          insItemMap.getColMap().put("QII_ERROR_CODE", errorCodeStrs[i]);// 不良代码
          insItemMap.getColMap().put("QII_ERROR_GRADE", errorGradeStrs[i]);// 缺陷等级（1-轻缺陷，2-重缺陷，3-致命缺陷）
    ...
    String tempSql = "SELECT  T.QII_SN,T.QII_ITEM_TYPE, " + " T.QII_ERROR_GRADE, "
                     ...
                     + " WHERE T1.QIP_INSPECT_SN = ? and  T.QII_SN = ? AND T.DATA_AUTH=?"
