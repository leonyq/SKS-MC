...
public class GetTestItemInfoService implements FuncService
{
  ...
  {//
    ...
    String dataAuth = request.getParameter("dataAuth");//CommMethodMc.getDataAuthBySession(request);
    ...
    String sql = "SELECT ROWNUM, D.* " + " FROM (SELECT A.* " + " FROM (SELECT B.*, " + " C.ID, "
                 ...
                 + " WHERE C.QII_SN = ? AND C.DATA_AUTH=? ) A " + " ORDER BY A.QII_ITEM_TYPE) D ";
    List resultList = modelService.listDataSql(sql, new Object[] {dataAuth,infoId, dataAuth,qipSn,dataAuth});
    if (resultList == null || resultList.isEmpty())
    {
      sql = "SELECT ROWNUM, D.* " + " FROM (SELECT A.* " + " FROM (SELECT B.*, " + " C.ID, "
            ...
            + " AND F.DICT_CODE = 'QMS_ERROR_GRADE' " + " WHERE C.QII_SN = ? AND C.DATA_AUTH=?) A "
            + " ORDER BY A.QII_ITEM_TYPE) D ";
      resultList = modelService.listDataSql(sql, new Object[] {dataAuth,dataAuth,infoId , dataAuth, qipSn,dataAuth});
      if (resultList == null || resultList.isEmpty())
      {
        sql = "SELECT ROWNUM, D.* " + " FROM (SELECT A.* " + " FROM (SELECT B.*, " + " C.ID, "
              + " C.QII_TEST_RESULT, " + " C.QII_ERROR_CODE, " + " C.QII_ERROR_GRADE, "
              ...
              + " LEFT JOIN T_QM_INSPECT_INFO T " + " ON T.QII_TEST_STEP = T1.QTI_TEST_STEP AND T1.DATA_AUTH = ? "
              ...
              + " WHERE T1.QTI_DEFAULT_FLAG = 'Y' " + " AND T.ID = ?) B "
              + " LEFT JOIN T_QM_INSPECT_ITEM_INFO C "
              + " ON B.QII_INSPECT_SN = C.QII_INSPECT_SN "
              + " AND B.QII_ITEM_TYPE = C.QII_ITEM_TYPE " + " AND B.QII_ITEM_SN = C.QII_ITEM_SN "
              + " LEFT JOIN T_CO_ERROR_CODE E " + " ON C.QII_ERROR_CODE = E.CEC_CODE  AND E.DATA_AUTH=? "
              ...
              + " AND F.DICT_CODE = 'QMS_ERROR_GRADE' " + " WHERE C.QII_SN = ? AND C.DATA_AUTH=?) A "
              + " ORDER BY A.QII_ITEM_TYPE) D ";
        resultList = modelService.listDataSql(sql, new Object[] {dataAuth,infoId,dataAuth, qipSn,dataAuth});
        if (resultList == null || resultList.isEmpty())
        {
          ...
          if (resultList == null || resultList.isEmpty())
          {
            ...
            if (resultList == null || resultList.isEmpty())
            {
              ...
              resultList = modelService.listDataSql(sql, new Object[] {infoId,dataAuth});
            }
          }
        }
      }
    ...
    String sql2 = "SELECT T.QIP_ITEM_NUM, " + " T.QIP_TEST_NUM, " + " T.QIP_SCARP_NUM, "
                  + " T.QIP_SAMPLE_NUM, " + " T.QIP_SAMPLE_FLAG, " + " T.QIP_ERROR_NUM, "
                  + " T.QIP_MEMO, " + " T.QIP_TEST_EMP, " + " T2.VR_VALUE "
                  + " FROM T_QM_INSEPCT_PARENT_INFO T " + " LEFT JOIN T_SYS_PARAMETER_INI T2 "
                  + " ON 1 = 1 " + " AND T2.VR_CLASS = 'QM0001' " + " WHERE T.QIP_SN = ? "
                  + " AND T.QIP_INSPECT_SN = ? AND T.DATA_AUTH=? AND T2.DATA_AUTH=?";
    Map resultMap = modelService.queryForMap(sql2, new Object[] {qipSn, inspectSn,dataAuth,dataAuth});
