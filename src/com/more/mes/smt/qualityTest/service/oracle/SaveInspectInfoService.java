...
public class SaveInspectInfoService implements FuncService {
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
		...
		String docStr = null;
		String insnStr = null;
		String atmoPressure = request
				.getParameter("paraMapObj.QII_ATMOSPHERIC_PRESSURE");// 大气压
		...
		String inspectBasis = request
				.getParameter("paraMapObj.QII_INSEPCT_BASIS");// 检验依据
		String memo = request.getParameter("paraMapObj.QII_MEMO");// 备注
		String inspectRes = request
				.getParameter("paraMapObj.QII_INSEPCT_RESULT");// 判定结果
		...
		tdMap.getColMap().put("QII_ATMOSPHERIC_PRESSURE", atmoPressure);
		tdMap.getColMap().put("QII_TEMPERATURE", temerature);
		tdMap.getColMap().put("QII_HUMIDITY", humidity);
		tdMap.getColMap().put("QII_INSEPCT_BASIS", inspectBasis);
		tdMap.getColMap().put("QII_MEMO", memo);
		tdMap.getColMap().put("QII_ERROR_NUM", errorNum);
		...
		try {
			...
			if (modelAction.getUpFile() != null) {
				...
				String realPath = modelAction.getRequest().getSession()
						.getServletContext()
						.getRealPath("up_load/comm/QIIFile");
				...
				String realPath1 = modelAction.getRequest().getSession()
						.getServletContext()
						.getRealPath("up_load/comm/QIIFile/" + fileName);
			...
			if (dataAuth != null)
            {
              tdMap.getColMap().put("DATA_AUTH", dataAuth);
            }
		...
		return modelAction.reloadIframeByIds(iframeId,
				modelAction.getText("编辑成功"), "0");
	}

	public String changeStatus(String doc, String inSn,String dataAuth,
