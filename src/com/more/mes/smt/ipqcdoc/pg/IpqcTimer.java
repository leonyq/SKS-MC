package com.more.mes.smt.ipqcdoc.pg;
    ...
    throws Exception
  {
    try
    {
      ...
      String insSql = "SELECT to_char(t1.qit_begin_time,'hh24:mi:ss') begin_time,"
                      ...
                      + "AND to_char(clock_timestamp(),'hh24:mi:ss') >= to_char(t1.qit_begin_time,'hh24:mi:ss') "
                      + "AND to_char(clock_timestamp(),'hh24:mi:ss') <= to_char(t1.qit_end_time,'hh24:mi:ss') ";
      ...
      if (insList != null && insList.size() > 0)
      {
        ...
        int count = modelService.execSql(checkSql, new Object[] {insList.get(0).get("BEGIN_TIME"),
          insList.get(0).get("END_TIME")});
        if (count == 0)
        { // 时间段里面没有记录则新增数据
          // 取出有效的IPQC检测项目
          ...
          if (ipqcList != null && ipqcList.size() > 0)
          {
            ...
            List<UserDto> user = modelService.listSql(userSql, null, null, null, null,
              new UserDto());
