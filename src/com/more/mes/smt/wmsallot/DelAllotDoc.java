 ...
 *
 */
public class DelAllotDoc implements FuncService {
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
		...
		try {
			...
			// 判断删除的id有没有明细
			String sql2 = "SELECT t.WAD_ITEM_CODE FROM T_WMS_ALLOT_DETAIL t WHERE t.WAD_ITEM_ID = ? AND t.DATA_AUTH=?";
			int num = strArray.length;
			for (int i = 0; i < num; i++) {
				int count = modelService.countSql(sql2, new Object[] { strArray[i], auth });
				if (count != 0) {
			...
			String sql = "DELETE FROM T_WMS_DOC_INFO WHERE WDI_DOC_NUM IN("
					...
					+ "(SELECT 1 FROM T_WMS_ALLOT_ITEM a WHERE a.WAI_DOC_NUM=t.WAD_DOC_NUM and a.DATA_AUTH=? )"
					+ " and t.DATA_AUTH=? )";
			modelService.execSql(sql, new Object[] { auth, auth });
			// 删除没有关联的调拨单信息 即当该单号所关联的物料全部被删除时删除调拨信息
			String sql3 = "DELETE FROM T_WMS_ALLOT_DOC WHERE WAD_DOC_NUM IN("
					+ "select t.WAD_DOC_NUM from T_WMS_ALLOT_DOC t WHERE NOT exists"
					+ "(SELECT 1 FROM T_WMS_ALLOT_ITEM a WHERE a.WAI_DOC_NUM=t.WAD_DOC_NUM and a.DATA_AUTH=? )"
					+ " and t.DATA_AUTH=? )";
			modelService.execSql(sql3, new Object[] { auth, auth });
		} catch (Exception e) {
			log.error(e);
