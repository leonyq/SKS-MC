package com.more.mcmes.moorder.pg;
...
String isCloseWin = req.getParameter("isCloseWin");// 是否关闭窗口
    ...
    if (vrValue.equals("1"))
    {
      ...
      if ("true".equals(indictorFlag))
      {
        ...
        sql = "SELECT * FROM T_CO_INDICATORR T WHERE T.CO_MODEL_CODE=? AND  COALESCE(T.CO_MO_NUMBER,'')='' AND T.DATA_AUTH=? ";
        ...
        if (!(result == null) && !result.isEmpty())
        {
          ...
          sql = "SELECT T2.* " + " FROM T_CO_INDICATORR T "
                ...
                + " WHERE T.CO_MODEL_CODE = ?  AND T.CO_MO_NUMBER IS NULL "
                + "AND T.DATA_AUTH=?  ";
          result = modelService.listDataSql(sql, new Object[] {modelCode,dataAuth});
          if (!(result == null) && !result.isEmpty())
          {
    ...
    String sql = "select t.PMO_NUMBER from T_PM_MO_ONLINE t where t.PMO_NUMBER = ? and t.PMO_AREA_SN = ? and t.PMO_TRACK = ? AND T.DATA_AUTH=? ";
    ...
    for (int i = 0; i < size; i++ )
    {
      ...
      if(quList!=null&&quList.size()>0){
    	  ...
    	  String insSql = "INSERT INTO T_WIP_STATION_ITEM(WSI_MO_NUMBER,WSI_ACTIONTYPE,WSI_MODEL_CODE," +
    	  		...
    	  		"now(),?,?,DEPT_ID,CREATE_USER,now(),EDIT_USER,EDIT_TIME,WSI_ITEM_TYPE,DATA_AUTH,WSI_PRODUCT_NUM," +
    	  		"? FROM T_WIP_STATION_ITEM T WHERE T.WSI_DEVICE_SEQ = ? AND T.WSI_TABLE_NO = ? " +
    	  		" AND T.WSI_LOADPOINT_SN = ? AND T.WSI_CHANEL_NO = ? AND T.WSI_AREA_ID = ? AND T.WSI_ONLINE_FLAG = 'Y' "+
    	  	  "AND T.DATA_AUTH=? ";
    	  try {
    ...
    if(arList!=null&&arList.size()>0){
        String checkSql = "select t.CMO_DEVICE_SEQ from T_CO_MATERIAL_ONLINE t WHERE T.CMO_AREA_SN = ? " +
        		"AND T.CMO_MO_NUMBER = ? AND T.CMO_PROCESS_FACE = ? " +
        		"AND T.CMO_DEVICE_SEQ = ? " +
        		"AND T.CMO_LOAD_FLAG = 'N' AND T.CMO_SHKIP_FLAG = 'N' AND T.CMO_TRACK = ? AND T.DATA_AUTH=? ";
    ...
    String m_res = SaveProductSn(modelService, projectId, pmMoNumber, "", "", "",
  ...
  //生成工单号段
  @SuppressWarnings("unchecked")
  public String SaveProductSn(ModelService modelService, String project_id, String mo_number, String sn_start,
