 ...
 **/
public class WorkOrderCheck implements FuncService {
...
//            throw new BussException(modelAction.getText("备料失败,工单分隔符格式错误,请使用'|' ','与空格"));
            modelAction.setAjaxString("提交检测失败");
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            if (!ok.equals(res)) {
                modelAction.setAjaxString(curWorkOrder+"提交检测失败," + res);
                return BaseActionSupport.AJAX;
//                throw new BussException(modelAction.getText(curWorkOrder+"生产备料失败," + res), res);
        ...
        modelAction.setAjaxString("提交检测成功");
        return BaseActionSupport.AJAX;
