 ...
 * 
 */
public class PackBomEditInfoInit implements FuncService {
	...
	public String exeFunc(ModelAction modelAction, ModelService modelService) {
      ...
      String sql = "SELECT distinct T1.*,T3.CI_ITEM_NAME,T3.CI_ITEM_SPEC ,T4.PACK_NUM "
		  + "FROM T_PM_PROJECT_DETAIL T1 "
		  + " LEFT JOIN T_PM_PROJECT_BASE T2 ON T1.PROJECT_ID = T2.PROJECT_ID  "
		  + " LEFT JOIN T_CO_ITEM T3 ON T3.CI_ITEM_CODE = T1.CBD_ITEM_CODE  "
          + " LEFT JOIN T_PROJECT_PACK_FEED T4 ON T4.PROJECT_ID =T1.PROJECT_ID and T4.ITEM_CODE =T1.CBD_ITEM_CODE"
		  + " WHERE 1=1 AND T2.PROJECT_ID = ?  ";
      
		List<Map> dataList = modelService.listDataSql(sql, new Object[]{projectId});
       /*String packSql = "SELECT T.PACK_NUM FROM T_PROJECT_PACK_FEED T WHERE 1=1 AND T.PROJECT_ID = ? and ";
        Map map = modelService.queryForMap(packSql, new Object[]{projectId});*/
