package com.more.mcmes.StopProjectBase;
 ...
 **/
public class StopProjectBaseStatusImpL implements FuncService {
...
//            throw new BussException(modelAction.getText("备料失败,工单分隔符格式错误,请使用'|' ','与空格"));
            modelAction.setAjaxString("禁用失败");
            return BaseActionSupport.AJAX;
        }
        ...
        String userId = modelAction.getCurrUser().getId();
          ...
          for (int i = 0; i < split.length; i++) {
            ...
            if (!StringUtils.isEmpty(curWorkOrder)) {
              String sql = "UPDATE T_PM_PROJECT_BASE T SET T.PROJECT_STATUS='6' WHERE 1=1 AND T.PROJECT_ID = '"+curWorkOrder+"'";
			...
			e.printStackTrace();
            throw new BussException(modelAction.getText("禁用失败," + e.getMessage()), e.getMessage());
        }
        ...
        modelAction.setAjaxString("禁用成功！");
        return BaseActionSupport.AJAX;
