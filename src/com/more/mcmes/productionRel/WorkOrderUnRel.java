 ...
 **/
public class WorkOrderUnRel implements FuncService {
        ...
        try {
            ...
            if(topSnList.size() > 0){
                for(String curSn:topSnList){
                    res = unRelTop(modelService, dataAuth, curSn, userId);
                }
            }else{
                for(String curSn:subSnList){
                    res = unRelSub(modelService, dataAuth, curSn, userId);
                }
            }
     ...
     * @return
     */
    public String unRelTop(ModelService modelService, String dataAuth, String relWorkerOrder, String userId){
        ...
        list.add(userId);
        outType.add("String");
        pf.setClassName("P_C_VOID_UNREL_PROJECT_TOP");
     ...
     * @return
     */
    public String unRelSub(ModelService modelService, String dataAuth, String relWorkerOrder, String userId){
        ProFuncDto pf = new ProFuncDto();
        int[] inPlace = {1, 2, 3};
        int[] outplace = {4};
        List<Object> list = new ArrayList<Object>();
        ...
        list.add(userId);
        outType.add("String");
        pf.setClassName("P_C_VOID_UNREL_PROJECT_SUB");
        pf.setInPlace(inPlace);
        pf.setOutPlace(outplace);
        pf.setTotalLen(list.size() + outType.size());
        pf.setValArgs(list);
