...
public class productCheckPro implements ValidateFuncService {
    ...
    public OperResult valJavaFun(String formId, Map paraMap,
        ...
        String proNo = (String)paraMap.get("PROJECT_NO"); 
        String dataid = modelAction.getDataId();
        String sql = "select * from T_PM_PROJECT where PROJECT_NO=?";
        List subList = modelService.listDataSql(sql, new Object[] { proNo });
        if (subList!=null&&subList.size() > 0) {
            if(!StringUtils.isEmpty(dataid)&&subList.size()==1){
               String id1=(String) ((Map)subList.get(0)).get("ID");
               if(!dataid.equals(id1)){
                   return new OperResult(false, "项目编号："+proNo+"，已存在！");
               }
            }
            else{
                return new OperResult(false, "项目编号："+proNo+"，已存在！");
