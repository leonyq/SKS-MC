...
public class SaveChildTask implements FuncService
{
  ...
  public String exeFunc(ModelAction modelAction, ModelService modelService)
  {
    ...
    String pId = request.getParameter("paraMap1@0#PID");// 父id
    ...
    try
    {
      ...
      if (day <= 7)
      {
        if ((planWeekDay < planBeginDay || (planWeekDay == planBeginDay && planHoursTes < planBeginHoursTes))
            && planWeekDay != 0 && planBeginDay != 6)
        {
          day = day - 2;
        }
    ...
    // String sql1
    // ="select max(TASK_NUMBER) TASK_NUMBER from T_TK_TASKCENTER where TASK_NUMBER like '%"+date+"%'";

    String sql1 = "select max(TASK_NUMBER) TASK_NUMBER from T_TK_TASKCENTER where TASK_NUMBER like '%"+date+"%'";

    List<Map> list = modelService.listDataSql(sql1);

    Map map = list.get(0);

    String taskNumber = (String)map.get("TASK_NUMBER");//获取今天数据库中的最大任务编号 

    String task = date;
   
    String number = null;//存入数据库的任务编号，为系统时间+“-”+3位流水
    ...
    else 
    {
      ...
      int a = Integer.parseInt(newString) + 1;
      if (a < 10)//判断a是几位数，流水保证为3位数
      {
        number = newStrings + "-00" + a;
      }
      else if (a>=10 && a < 100)
      {
        number = newStrings + "-0" + a;
      }
    ...
    taskTabke.getColMap().put("FACT_BEG_TIME",
      DateUtil.parseDate(factBegTime, "yyyy-MM-dd HH:mm:ss"));
    taskTabke.getColMap().put("SPEED_PROGRESS", speedProgress);
    taskTabke.getColMap().put("PROJECT_ID", projectId);
    taskTabke.getColMap().put("PLAN_OVER_TIME",
      DateUtil.parseDate(prapPlanOverTime, "yyyy-MM-dd HH:mm:ss"));
    taskTabke.getColMap().put("CREATE_TIME", DateUtil.getCurDate());
    taskTabke.getColMap().put("PLAN_BEGIN_TIME",
      DateUtil.parseDate(prapPlanBeginTime, "yyyy-MM-dd HH:mm:ss"));
    ...
    taskTabke.getColMap().put("TASK_NUMBER", number);
